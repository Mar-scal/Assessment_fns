...
with(bank.live[bank.live$year==2018,],text(lon,lat,round(pre, 1),cex=0.5))
...
with(bank.live[bank.live$year==2018,],text(lon,lat, round(rec, 1),cex=0.5))
...
with(bank.live[bank.live$year==2018,],text(lon,lat,round(com, 1),cex=0.5))
...
with(bank.live[bank.live$year==2018,],text(lon,lat,round(pre.bm, 1),cex=0.5))
...
with(bank.live[bank.live$year==2018,],text(lon,lat, round(rec.bm, 1),cex=0.5))
...
with(bank.live[bank.live$year==2018,],text(lon,lat,round(com.bm, 1),cex=0.5))
...
## Case by case Historical spatial plotting

case <- bank.dat$Sab
case$stratum <- NA
case$pre <- rowSums(case[,12:26])
case$rec <- rowSums(case[,27:28])
case$com <- rowSums(case[,29:51])
case$tot <- rowSums(case[,12:51])

source("E:/Offshore scallop/Assessment/Assessment_fns/Survey_and_OSAC/assign_strata.r")

detail.poly.surv <- subset(survey.detail.polys,label=="Sab")
attr(detail.poly.surv,"projection")<-"LL"

oldscheme <- assign.strata(case, polys=detail.poly.surv[detail.poly.surv$startyear == 1900,])
newscheme <- assign.strata(case, polys=detail.poly.surv[detail.poly.surv$startyear==2018,])

case <- oldscheme
case$new <- newscheme$Strata_ID

case$Strata_ID[is.na(case$Strata_ID)] <- "na1"
case$new[is.na(case$new)] <- "na1"

lost <- case[which(case$Strata_ID!=case$new),]

before<-ddply(.data=case, .(year),
      summarize,
      pre=sum(pre),
      rec=sum(rec),
      com=sum(com),
...
after<-ddply(.data=case[which(case$Strata_ID==case$new),], .(year),
      summarize,
      pre=sum(pre),
      rec=sum(rec),
      com=sum(com),
...
after$pre_diff <- before$pre-after$pre
after$rec_diff <- before$rec-after$rec
after$com_diff <- before$com-after$com
after$tot_diff <- before$tot-after$tot
after$tow_diff <- before$tows-after$tows

require(reshape2)
after <- melt(data=after, id.vars = c("year", "pre", "rec", "com", "tot", "tows", "tow_diff"))

poly <- fortify(PolySet2SpatialPolygons(as.PolySet(detail.poly.surv[detail.poly.surv$label=="Sab" & detail.poly.surv$startyear==1900,], projection="LL")))

pdf(onefile=T, file="E:/Offshore scallop/Assessment/Assessment_fns/One_off_scripts/2018/Assessing restrat.pdf", height=8.5, width=11)
ggplot() + 
  geom_polygon(data=poly,
               aes(long, lat, group=group), colour="grey", fill=NA)+
  geom_point(data=case, aes(lon, lat), size=1) +
  geom_point(data=lost,
             aes(lon, lat), colour="red", size=1)+
  facet_wrap(~year, nrow=5)+
  coord_map() + 
  theme_bw() + 
  theme(panel.grid=element_blank())

ggplot() + geom_point(data=after, aes(year, value, colour=variable)) + 
  ylab("Difference between estimates\nbefore and after restratification")+
  theme_bw() +
  scale_colour_discrete(labels=c("pre-recruits",
                               "recruits", 
                               "fully recruited",
                               "everything"))

ggplot() + geom_point(data=after, aes(year, tow_diff)) +
  ylab("Difference in sample size\nbefore and after restratification")+
  theme_bw()
dev.off()
