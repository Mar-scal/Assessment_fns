...
# Note that we've increased the number of extra stations we produce to 85 (from 80) in case we need a few in our back pocket due to
# halibut survey...
res <- Survey.design(yr = yr,banks = c("GBb"),direct = direct,export=T,relief.plots = F,fig="png",seed=219,text.points = F,add.extras=F,zoom=T)
table(res$Tows$Poly.ID)

aggregate(EID~Poly.ID,res$Tows,length)

100*(surv.polydata$area_km2[surv.polydata$label == "Sab"]/sum(surv.polydata$area_km2[surv.polydata$label == "Sab"]))


Survey.design(yr = yr,banks = c("Sab"),direct = direct,export=F,relief.plots = T,fig="screen",seed=yr-2000,text.points = T,ger.new = 80,add.extras=F,zoom=T)
...
# In 2018 we wanted to see where the halibut stations were located with respect to the German survey stations.
Survey.design(yr = yr,banks = c("Ger"),direct = direct,export=F,relief.plots = F,fig="screen",seed=yr-2000,text.points = T,ger.new = 80,add.extras=F,zoom=T)
# Bring in the halibut survey stations
hali <- read.csv(paste0(direct,"Data/Survey_data/",yr,"/Halibut_survey_stations_2018.csv"))
points(hali$lon.DecDeg,hali$lat.DecDeg,pch=21,bg="blue",cex=1)
# Get the stations nearby as well...
ScallopMap("WSS",plot.boundries = T,boundries = "offshore")
points(hali$lon.DecDeg,hali$lat.DecDeg,pch=21,bg="blue",cex=1)
# And get the stations out in the entire SS
ScallopMap("SS",plot.boundries = T,boundries = "offshore",offshore.names = T)
points(hali$lon.DecDeg,hali$lat.DecDeg,pch=21,bg="blue",cex=1)
