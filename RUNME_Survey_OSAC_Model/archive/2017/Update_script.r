...
update_JAGS(run.mod=T,preprocessed=T,make.figs=T,make.diag.figs = F,run.pred.eval.model = F,use.final=F,fig="pdf",
            make.pred.eval.figs = F,pred.eval.fig.type="box",nchains=8,bank="GBa",export.tables = T,
            make.update.figs=T,yr=2016,niter = 10000,nburn = 5000,make.diag=F,direct=direct,
            un=un.ID,pw=pwd.ID,db.con="ptran")
# Run the model and see how long it takes.)

# Here are a quick summary of the interesting model results which we report in the model update for BBn.
load(paste(direct,"/Data/Model/2016/BBn/Results/Model_results_and_diagnostics.RData",sep=""))
load(paste(direct,"/Data/Model/2016/GBa/Results/Model_testing_results.RData",sep=""))
...
# The fully recruited biomass for the current year and previous year
FR.bm
# Long term median fully recruited biomass (not including the most recent year)
FR.ltm
# Recruite Biomass for current and previous year
rec.bm
