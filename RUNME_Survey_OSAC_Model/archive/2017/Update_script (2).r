...
update_JAGS(run.mod=T,preprocessed=T,make.figs=F,make.diag.figs = F,run.pred.eval.model = F,use.final=F,final.run=F,fig="pdf",
            make.pred.eval.figs = F,pred.eval.fig.type="box",nchains=10,bank=c("GBa"),export.tables = T, 
            make.update.figs=F,yr=2016,niter = 150000,nburn = 100000,make.diag=T,direct=direct,
...
load(paste(direct,"/Data/Model/2017/BBn/Results/Model_results_and_diagnostics.RData",sep=""))
