...
# If you have run the whole analysis down to the end and just need to reload it this is the code to run.
# Note that for objects that are not bank specific (e.g. com.contours, rec.contours, etc) those lines of code will need to be re-run
# for each bank.  This object likely has the contours stored for Georges A (and B) but not necessarily.  Also at the start of the bank
# section you are interested in you need to reload those objects as they are not bank specific (RS, CS, years, and the plot titles)
...
#source(paste(direct,"Data/Survey_data/",yr,"/Survey_summary_output/GBasurveyObj.R",sep=""))    #Source31 
################################## End Load Functions   #######################################################
...
# survey data
    ...
    survAug.dat<-survAug.dat[which(!names(survAug.dat)%in%c("dis","brg",'totwt','baskets'))]
		...
		Summersurv2011$SHF <- Summersurv2011$SHF[which(!names(Summersurv2011$SHF)%in%c("dis","brg"))]
		...
		Springsurv2012$SHF <- Springsurv2012$SHF[which(!names(Springsurv2012$SHF)%in%c("dis","brg"))]
		...
		Summersurv2012$SHF <- Summersurv2012$SHF[which(!names(Summersurv2012$SHF)%in%c("dis","brg"))]
	  ...
	  # DK Note, I still need to get access to OSTOWS table for this to work
		...
		### DK NOTE This is a new way to get the temperature information
		...
		# Remove the temperature data so the survey data from before temperature data was collected lines up nicely.
		...
		Summersurv2015SHF<-subset(SurvDBSHF,cruise%in% c("LE01","LE02"))
		...
		#summerSurv.dat<-merge(summerSurv.dat,temps,all.x=T)
		...
		surv.TE10 <- subset(summerSurv.dat,cruise=="TE10" & state == "live")
...
## Slide 2 from Spring survey summary can be made here
		png(paste(direct,yr,"/Presentations/Survey_summary/spring_survey.png",sep=""),units="in",width = 11, height = 8.5,
...
ScallopMap(area="NL",plot.bathy=T,bathy.source = "quick",boundries = "offshore",shore="nwatlHR",
           ...
           title=paste("Offshore scallop spring survey results (",yr-1,")",sep=""),cex.mn=2,dec.deg = F,direct=direct)
dev.off()
...
yr <- 2014 # since we didn't do survey in 2015 for middle bank reset the year to 2014
##################  Bring in and ID the variables needed to create the plots ##################################
    ...
    clap.per.ts.title <- substitute(bold(paste("Clapper time series (% dead ",bank,")",sep="")),
		...
		#Source19 source("fn/contour.gen.r")
		pre.contours<-contour.gen(subset(survMidLive.dat,year==yr,c('tow','lon','lat','pre')),ticks=c(1,5,10,50,100),
		                          str.min=0,interp.method='gstat',points=T,blank=T,res=0.01,key='log.cont',
		                         color.fun=tim.colors,id.par=5,units='#/tow',
		                          plot=F,subscale=0.1,direct = direct)
		
		rec.contours<-contour.gen(subset(survMidLive.dat,year==yr,c('tow','lon','lat','rec')),ticks=c(1,5,10,50,100),
		                          str.min=0,interp.method='gstat',points=T,blank=T,res=0.01,key='log.cont',
		                          color.fun=tim.colors,id.par=5,units='#/tow',
		                          plot=F,subscale=0.1,direct = direct)
		...
		CF.Mid.current<-na.omit(merge(subset(na.omit(SurvDB$pos),bank == bnk & year==yr,c('tow','lon','lat')),
		                             Mid.SpatHtWt.fit$fit))
		names(CF.Mid.current)[4]<-"CF"
		CF.Mid.current<-merge(CF.Mid.current,subset(survMidLive.dat,year==yr,c('year','tow','lon','lat',
		...
		CF.Mid.current<-surv.by.tow(CF.Mid.current, yr, pre.ht=RS, rec.ht=CS, type='ALL', mw.par="CF")
		
		# CONDITION FACTOR and MEAT COUNT OF SCALLOP ON MIDDLE BANK in 2014
		#Source19  source("fn/contour.gen.r") 
		mc.contours<-contour.gen(subset(CF.Mid.current,select=c('tow','lon','lat','meat.count')),
		                         ticks='define',nstrata=7,str.min=0,place=2,id.par=3.5,
		                         interp.method='gstat',key='strata',blank=F,
		                         plot=F,subset.poly=Mid.survey.bound.poly,subset.eff=0,subscale=0.25,direct = direct)
		
		cf.contours<-contour.gen(subset(CF.Mid.current,select=c('tow','lon','lat','CF')),ticks='define',
...
##################  Create  plots for Middle Bank using the above data. ##################################
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/survey_strata.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/abundance_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
    ...
    dev.off()
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/biomass_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		survey.ts(survMid.obj[[1]],1984:yr,Bank=bnk,pdf=F,type='B', RS=RS-5, CS=CS-5,ys=.8,ht=6.5,wd=10,clr='blue',se=F,pch=16,
		          yl2=c(0.11,0.11,1.1),add.title=T,titl = survey.ts.BM.title,cx.mn=3,axis.cx = 1.5)
    dev.off()
			
		# SHELL HEIGHT FREQUENCY HISTORGRAMS MIDDLE BANK (slide 7 in presentation)
		#Source18 source("fn/shf.plt.r")
    png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/shell_height_freqency.png",sep=""),units="in",width = 8.5, 
		...
		shf.plt(ps.dat,survMid.obj,from='surv',yr=2008:yr,col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),rel=F,ymax=10,
		        recline=c(RS-5,CS-5),wd=7,ht=8,add.title = T,titl = SHF.title,cex.mn=3,adj=0.05,
		        sample.size = survMid.obj[[1]]$n[survMid.obj[[1]]$year %in% 2008:yr])	
		dev.off()
		
	
		
		# PRE RECRUIT SCALLOP DISTRIBUTION ON MIDDLE BANK	(slide 9 in presentation)	
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Prerecruits.png",sep=""),units="in",
		...
		ScallopMap(bnk,contour=list(preCont.poly,cont.data),  plot.bathy=T,bathy.source = "quick",plot.boundries = T,
		           title=pre.rec.title, xlab="", ylab="",
		           cex.mn=2,dec.deg = F,direct=direct)
		# Regular survey tows
		points(lat~lon,subset(survMidLive.dat,year== yr & random==1),pch=20,bg='black',cex=0.8)
		# Add other survey tows
		points(lat~lon,survMidLive.dat,subset=year==yr & random %in% c(2,3,4,5),pch=24,bg="darkorange",cex=0.8)
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		       fill=cont.data$col,title=N.tow.lab, title.adj = 0.2,inset=0.02,bg=NA,box.col=NA)
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(unique(subset(survMidLive.dat,year==yr & random==1)$tow)),")",sep=""),
		       inset=0.01,bg=NA,box.col=NA)
		dev.off()
		
		
		# RECRUIT SCALLOP DISTRIBUTION ON MIDDLE BANK	(NEW SLIDE in presentation)	
		lvls=c(1,5,seq(10,40,10))
		CL <- contourLines(rec.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		recCont.poly <- CP$PolySet
		cont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Recruits.png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		
		ScallopMap(bnk,contour=list(recCont.poly,cont.data),  plot.bathy=T,bathy.source = "quick",plot.boundries = T,
		           title=rec.title, xlab="", ylab="",
		           cex.mn=2,dec.deg = F,direct=direct)
		# Regular survey tows
		points(lat~lon,subset(survMidLive.dat,year== yr & random==1),pch=20,bg='black',cex=0.8)
		# Add other survey tows
...
#		windows(11,8.5)
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Fully_recruited.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/MWSH_and_CF_ts.png",sep=""),units="in",width = 13, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,2))
		...
		stdts.plt(merge(MidcfData$CFyrs,data.frame(year=min(years):yr+1),all=T),y='CF',pch=16,
		          ylab=cf.lab, mean.line=T,graphic='none',xlab='Year',ylim=c(8,18),las=1,titl = CF.ts.title,cex=2)		
		dev.off()
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
	#	windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/MC_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		# Add the legends.
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		       fill=cont.data$col,title=mc.lab,inset=0.02,bg=NA,box.col=NA)
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(unique(subset(survMidLive.dat,year==yr & random==1)$tow)),")",sep=""),
		       inset=0.01,bg=NA,box.col=NA)
    dev.off()
...
############################################################################################################
		...
		yr <- 2014 # Just for 2015 since no spring survey...
		...
		Sabmw.dat<-merge(subset(Sab.wgt.dat,month %in% 5:6,c("ID","year","lon","lat","depth","sh","wmw","tow")),
		                 subset(sab.mw,select=c("ID","year","lon","lat","depth","sh","wmw","tow")),all=T)
		#Write5
		...
		CF.Sab.current<-na.omit(merge(subset(na.omit(SurvDB$pos),bank==bnk&year==yr,c('tow','lon','lat')),Sab.SpatHtWt.fit$fit))
		names(CF.Sab.current)[4]<-"CF"
		CF.Sab.current<-merge(CF.Sab.current,subset(survSabLive.dat,
		                                          year==yr,c('year','tow','lon','lat',paste('h',seq(5,200,5),sep=''))))
		#Source14 source("fn/surv.by.tow.r") surv.by.tow calculates number and/or biomass of pre, rec and 
		#com size scallops in each tow + meat count.
		CF.Sab.current<-surv.by.tow(CF.Sab.current, yr, pre.ht=RS, rec.ht=CS)
		CF.Sab.current<-surv.by.tow(CF.Sab.current, yr, pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")
		# Meat count per 500g
		CF.Sab.current$meat.count<-0.5/(CF.Sab.current$com.bm/CF.Sab.current$com)
		
		# CONDITION FACTOR and MEAT COUNT OF SCALLOP ON SABLE BANK in 2014
		#Source19  source("fn/contour.gen.r")
		mc.contours<-contour.gen(subset(CF.Sab.current,select=c('tow','lon','lat','meat.count')),ticks='define',
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") 
    ...
    legend(-62,44.35,legend = as.numeric(with(subset(survSab.dat,year== yr & state=='live' & random==1),
    ...
    dev.off()
		...
		dev.off()
		median(survSab.obj[[1]]$NPR,na.rm=T)
		# Shell height frequency plots (slide 17, 2014 presentation)
		#Source18 source("fn/shf.plt.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/shell_height_freqency.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,survSab.obj,from='surv',yr=2008:yr,col1='grey80',type='sh',col2=1,col3=1, adj=0.05,
		        xl=c(0,200),rel=F,ymax=25,recline=c(RS-5,CS-5),wd=7,ht=8,add.title = T,titl = SHF.title,cex.mn=3,
		        sample.size = survSab.obj[[1]]$n[survSab.obj[[1]]$year %in% 2008:yr])	
		dev.off()
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Pre-recruits.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(unique(subset(survSab.dat,year==yr & random==1)$tow)),")",sep=""),
		       inset=0.01,bg=NA,box.col=NA)		
		legend("topleft",title=N.tow.lab, title.adj = 0.2,c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                       paste(lvls[length(lvls)],'+',sep='')),fill=precont.data$col,
		       inset=0.04,bg=NA,box.col=NA)
		dev.off()
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Recruits.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(unique(subset(survSab.dat,year==yr & random==1)$tow)),")",sep=""),
		       inset=0.01,bg=NA,box.col=NA)	
		legend("topleft",title=N.tow.lab, title.adj = 0.2,
		       c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		       fill=precont.data$col, inset=0.04,bg=NA,box.col=NA)
		dev.off()
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Fully_recruited.png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		#windows(11,8.5)
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		...
		#Source12 source("fn/shwt.plt1.r")
		#windows(15,8)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/MWSH_and_CF_ts.png",sep=""),units="in",
		    width = 13, height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,2))
		shwt.plt1(Sab.SpatHtWt.fit,lw=3,cx=1.2,titl=MWSH.title,cex.mn=2,las=1)
		# CONDITION FACTOR PLOT SABLE BANK (also slide 21, 2014 presentation)
		#Source16 source("fn/stdts.plt.R")
		stdts.plt(merge(SabcfData$CFyrs,data.frame(year=min(years):yr+1),all=T),y='CF',pch=16,
    ...
    dev.off()	
		...
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/CF_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		ScallopMap(bnk,contour=list(cont.poly,cont.data),title=cf.title, plot.boundries = T,direct=direct,
		           plot.bathy = T,bathy.source = "quick",xlab="",ylab="",cex.mn = 2,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,CF.Sab.current,pch=20,bg='black',cex=0.8)
		# Add other survey tows
		#points(lat~lon,survSab.dat,subset=year==yr & state=='live' & random %in% c(2,3,4,5),pch=24,bg="darkorange",cex=0.8)
		# Add the legends
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(CF.Sab.current$tow),")",sep=""),
		       inset=0.01,bg=NA,box.col=NA)	
		legend("topleft",title=cf.lab,
		       c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		       fill=cont.data$col,inset=0.04,bg=NA,box.col=NA)
		dev.off()
		...
		ScallopMap(bnk,contour=list(cont.poly,cont.data),title=mc.title, plot.boundries = T,direct=direct,
		           plot.bathy = T,bathy.source = "quick",xlab="",ylab="",cex.mn = 2,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,subset(CF.Sab.current),pch=20,bg='black',cex=0.8)
		# Add other survey tows
		# Add the legends
		legend("topright",pch=c(20), pt.bg = c("black"), title="Tow type",
		       legend = paste('regular (n =',length(unique(subset(survSab.dat,year==yr & random==1)$tow)),")",sep=""),
...
################################ SECTION 4 GERMAN BANK ########################################################
############################################################################################################
		yr = as.numeric(format(Sys.time(), "%Y"))  # Reset year... 
		...
		RS=100 # RS = Shell height 1 year previous to CS
	 	...
	 	years=1985:yr # note I removed 1984, the data that year don't line up nicely as the live/dead survey results don't align.
	 	...
	 	cf.title <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
	 	                       list(year=as.character(yr),bank=bnk))
	 	cf.title.last <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
	 	                       list(year=as.character(yr-1),bank=bnk))
	 	mc.title <- substitute(bold(paste("Meat count (" ,"">=m, " mm " , bank,"-",year,")",sep="")),
	 	                       list(m=as.character(CS-5),year=as.character(yr),bank=bnk))
	 	survey.ts.N.title <- substitute(bold(paste("Survey abundance time series (",bank,")",sep="")),
	 	                                list(year=as.character(yr),bank=bnk))
	 	...
	 	clap.per.ts.title <- substitute(bold(paste("Clapper time series (% dead ",bank,")",sep="")),
	 	                                list(bank=bnk))
	 	fully.rec.title.last <- substitute(bold(paste("Fully recruited scallops (" ,"">=a, " mm " , bank,"-",year,")",sep="")),
	 	                              list(a=as.character(CS-5),year=as.character(yr-1),bank=bnk))
	 	rec.title.last <- substitute(bold(paste("Recruit scallops (",b- a, " mm " , bank,"-",year,")",sep="")),
	 	                        list(a=as.character(CS-6),b=as.character(RS-5),year=as.character(yr-1),bank=bnk))
	 	pre.rec.title.last <- substitute(bold(paste("Pre-recruit scallops (" ,""<b, " mm " , bank,"-",year,")",sep="")),
	 	                            list(b=as.character(RS-5),year=as.character(yr-1),bank=bnk))
	 	...
	 	# Need to input the number of tows, file location, weighting scheme, seconds between readings
	 	#Source10 source("fn/getdis.r") 
	 	GerDis<-dist.coef(c(404,405,444,446,447,458,472,c(407:415),c(417:423),c(425:431),
	 	                    c(433:436),c(439:442),c(449:451),c(454:456),c(460:468),
	 	                    c(474:480),1:20),
	 	                  path=paste(direct,"Data/Tow_tracks/",yr,"/Spring/German/",sep=""),
	 	                  w=c(1:10,9:1),rule=8,smooth=T,plt=F)
	 	
	 	
		# Grab the German bank and the live scallops on German
	 	# DK note clean this up, this gets rid of the temperature data from summer Survey..
		survGer.dat<-rbind(subset(springSurv.dat,bank==bnk & year  %in% years),
		                   subset(summerSurv.dat,bank==bnk & year %in% years))
		
		survGerLive.dat<-subset(survGer.dat,state=='live')
		
		
		### WHICH TOWS ARE MATCHED WITH WHICH!!!!
		#new.matched <- subset(survGerLive.dat, year==yr & random==3,c("tow","slon","slat","lat","lon","random","year"))
		new.ger.tows <- subset(survGerLive.dat, year==yr,c("tow","slon","slat","lat","lon","random","year"))
		new.ger.tows$EID <- 1:nrow(new.ger.tows)
		last.ger.tows <- subset(survGerLive.dat, year==yr-1)
		# Now get all the tows that appear to overlap and they are our matched tows, search on slat and lat.
		for(i in 1:nrow(new.ger.tows))
		  {
  		  if(any(round(last.ger.tows$slat,digits=2) == round(new.ger.tows$slat[i],digits=2) & 
  		     round(last.ger.tows$slon,digits=2) == round(new.ger.tows$slon[i],digits=2)))
  		   new.ger.tows$EID[i] <- last.ger.tows$tow[round(last.ger.tows$slat,digits=2) == round(new.ger.tows$slat[i],digits=2) & 
  		                                            round(last.ger.tows$slon,digits=2) == round(new.ger.tows$slon[i],digits=2)]
  		  if(any(round(last.ger.tows$lat,digits=2) == round(new.ger.tows$lat[i],digits=2) & 
  		         round(last.ger.tows$lon,digits=2) == round(new.ger.tows$lon[i],digits=2)))
  		    new.ger.tows$EID[i] <- last.ger.tows$tow[round(last.ger.tows$lat,digits=2) == round(new.ger.tows$lat[i],digits=2) & 
  		                                               round(last.ger.tows$lon,digits=2) == round(new.ger.tows$lon[i],digits=2)]
		...
		# DK Note we need a better way of doing this!
		...
		Ger15.mw<-subset(SurvDB$MWs,bank=="Ger"&year==2015)
		Ger.mw<-rbind(Ger11.mw,Ger12.mw,Ger13.mw,Ger14.mw,Ger15.mw)
		# Convert shell height in 2014 to decimeters.
		Ger15.mwdm<-Ger15.mw
		Ger15.mwdm$sh<-Ger15.mw$sh/100
		Ger14.mwdm<-Ger14.mw
		...
		# Grab the relavent Meat-Weight Shell height data and make a flat file from it
		Germw.dat<-merge(subset(Ger.wgt.dat,month%in%5:6,c("ID","year","lon","lat","depth","sh","wmw","tow")),
		                 subset(Ger.mw,select=c("ID","year","lon","lat","depth","sh","wmw","tow")),all=T)
		...
		#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
		#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
		Ger.SpatHtWt.fit<-shwt.lme(Ger15.mwdm,random.effect='tow',b.par=3)
		Ger.SpatHtWt.last.fit<-shwt.lme(Ger14.mwdm,random.effect='tow',b.par=3)
		
		
		# For the condition factor we need to select the location and that the gam is going to predict on
		# This needs to be a static location for this gam so we at least are predicting at the same place each year
		# For German I am picking the location somewhat in the middle of where we do all the tows and
		# at a depth that will bias this CF prediction higher, but it is also a more stable location
		# for the CF data between years, I don't like predicting on the "unstable" part of the depth spline
		# which is what we'd be doing using 85-95 meter depths (which are probably more accurate representations of)
		# the mean...
		pred.loc <- list(lon = -66.25, lat= 43.15, depth = 80)
		## MODEL - This is the model used to esimate condition factor across German Bank
		# Same model as Sable, tho both are more complex than Middle Bank due to data constraints there
		#Source13 source("fn/condFac.r")
		GercfData<-condFac(Germw.dat,survGer.dat,pred.loc,model.type='gam_f',pred.loc = pred.loc,dirct=direct)
		...
		#Write6 - Out put these results
		write.table(survGer.dat,paste(direct,"Data/Survey_data/",yr,"/Spring/Ger/GerSurvey84",yr,".csv",sep=""),sep=',',row.names=F)
		
		# Subset the survey data into the clappers(dead) and live scallops. I Removed year 1984 because those data
		# have different numbers of live/clappers.
		survGerClap.dat<-subset(survGer.dat,state=='dead')
		survGerLive.dat<-subset(survGer.dat,state=='live')
		
		
		# Clappers on German bank
		survGerClap.dat$clap.prop<-survGerClap.dat$tot/(survGerLive.dat$tot+survGerClap.dat$tot)*100
		survGerClap.dat$clap.prop[is.na(survGerClap.dat$clap.prop)]<-0
		survGerClap.dat$clap.propCom<-survGerClap.dat$com/(survGerLive.dat$com+survGerClap.dat$com)*100
		survGerClap.dat$clap.propCom[is.na(survGerClap.dat$clap.propCom)]<-0
		survGerClap.dat$clap.propRec<-survGerClap.dat$rec/(survGerLive.dat$rec+survGerClap.dat$rec)*100
		survGerClap.dat$clap.propRec[is.na(survGerClap.dat$clap.propRec)]<-0
		survGerClap.dat$clap.propPre<-survGerClap.dat$pre/(survGerLive.dat$pre+survGerClap.dat$pre)*100
		survGerClap.dat$clap.propPre[is.na(survGerClap.dat$clap.propPre)]<-0
		survGerClap.dat$clap.prop[is.na(survGerClap.dat$clap.prop)]<-0
		...
		lined.obj<-simple.surv(lined.dat,years=2008:yr)
		clap.obj<-simple.surv(clap.dat,years=2008:yr)
		
		merged.obj<-merge(subset(spr.obj[[1]],year!=2011),subset(lined.obj[[1]],year==2011),all=T)
		
		# matched tows
		matched.tows<-rbind(subset(survGerLive.dat, year == (yr-1) & tow %in% spr.obj[[2]]$tow.y1),
		                    subset(survGerLive.dat, year == yr & tow %in% spr.obj[[2]]$tow.y2))
		matched.obj<-simple.surv(matched.tows, years=(yr-1):yr)
		
		
		# Now I'd like this output so I have some of the annual numbers for Survey Summary
		SS.summary.Ger <- lined.obj[[1]]
		SS.summary.Ger$bank <- "Ger"
		names(SS.summary.Ger) <- c("year","n","FR.BM","CV.FR.BM","R.BM","CV.R.BM","Pre.BM","CV.Pre.BM",
		                                       "FR_N", "CV.FR.N",  "R.N","CV.R.N","Pre.N", "CV.Pre.N","bank")
		write.csv(SS.summary.Ger,
		          file = paste(direct,"Data/Survey_data/",yr,"/Spring/Ger/German_annual_summary",yr,".csv",sep=""),row.names = F)
		# Same for the SHF data.
		SHF.summary.Ger <- cbind(lined.obj[[1]]$year,lined.obj[[2]]$n.yst)
		write.csv(SHF.summary.Ger,
		          file = paste(direct,"Data/Survey_data/",yr,"/Spring/Ger/German_annual_SHF_summary",yr,".csv",sep=""),row.names = F)
		
		### GENERATE THE CONTOURS for each of the Sable bank distribtuion maps ###

		# Pre-recruits (< 95 mm shell height)
		#Source19 source("fn/contour.gen.r")
		pre.contours<-contour.gen(subset(survGerLive.dat,year==yr,c('tow','lon','lat','pre')),ticks='define',
		                          nstrata=9,str.min=0,interp.method='gstat',points=T,blank=T,res=0.01,key='log.cont',
		                          title='',color.fun=tim.colors,id.par=3.5,
		                          units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		
		pre.contours.last<-contour.gen(subset(survGerLive.dat,year==yr-1,c('tow','lon','lat','pre')),ticks='define',
		                          nstrata=9,str.min=0,interp.method='gstat',points=T,blank=T,res=0.01,key='log.cont',
		                          title='',color.fun=tim.colors,id.par=3.5,
		                          units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		...
		#Source19 source("fn/contour.gen.r")
		rec.contours<-contour.gen(subset(survGerLive.dat,year==yr,c('tow','lon','lat','rec')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',points=T,
		                          blank=T,res=0.01,key='log.cont',title='',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		rec.contours.last<-contour.gen(subset(survGerLive.dat,year==yr-1,c('tow','lon','lat','rec')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',points=T,
		                          blank=T,res=0.01,key='log.cont',title='',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		# Commercial (>= 105 mm shell height)
		#Source19 source("fn/contour.gen.r")
		com.contours<-contour.gen(subset(survGerLive.dat,year==yr,c('tow','lon','lat','com')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.01,key='log.cont',title='',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		com.contours.last<-contour.gen(subset(survGerLive.dat,year==yr-1,c('tow','lon','lat','com')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.01,key='log.cont',title='',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,direct=direct)
		...
		# MEAT COUNT & CONDITION FACTOR OF SCALLOP ON German BANK (requires a bit of preprocessing)
		CF.Ger.current<-na.omit(merge(subset(na.omit(SurvDB$pos),
		                                        bank=="Ger"& year==yr,c('tow','lon','lat')),Ger.SpatHtWt.fit$fit))
		CF.Ger.last<-na.omit(merge(subset(na.omit(SurvDB$pos),
		                                        bank=="Ger"& year==yr-1,c('tow','lon','lat')),Ger.SpatHtWt.last.fit$fit))
		names(CF.Ger.current)[4]<-"CF"
		names(CF.Ger.last)[4]<-"CF"
		CF.Ger.current<-merge(CF.Ger.current,subset(survGerLive.dat,
		                                          year==yr,c('year','tow','lon','lat',paste('h',seq(5,200,5),sep=''))))
		CF.Ger.last<-merge(CF.Ger.last,subset(survGerLive.dat,
		                                                  year==yr-1,c('year','tow','lon','lat',paste('h',seq(5,200,5),sep=''))))
		
		#Source14 source("fn/surv.by.tow.r") surv.by.tow calculates number and/or biomass of pre, rec and 
		#com size scallops in each tow + meat count.                                                                       
		CF.Ger.current<-surv.by.tow(CF.Ger.current, yr, pre.ht=RS, rec.ht=CS)
		CF.Ger.current<-surv.by.tow(CF.Ger.current, yr, pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")
		CF.Ger.last<-surv.by.tow(CF.Ger.last, yr-1, pre.ht=RS, rec.ht=CS)
		CF.Ger.last<-surv.by.tow(CF.Ger.last, yr-1, pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")
		
		# Meat count per 500g
		CF.Ger.current$meat.count<-0.5/(CF.Ger.current$com.bm/CF.Ger.current$com)
		CF.Ger.last$meat.count<-0.5/(CF.Ger.last$com.bm/CF.Ger.last$com)
		
		# CONDITION FACTOR and MEAT COUNT OF SCALLOP ON GERMAN BANK in 2014
		#Source19 source("fn/contour.gen.r")
		mc.contours<-contour.gen(na.omit(subset(CF.Ger.current,select=c('tow','lon','lat','meat.count'))),
		...
		cf.contours<-contour.gen(subset(CF.Ger.current,select=c('tow','lon','lat','CF')),ticks='define',
		                         nstrata=7,str.min=0,place=2,id.par=5,units="scallops/500g",
		                         interp.method='gstat',key='strata',blank=F,plot=F,subset.poly='square',
		                         subset.eff=0,subscale=0.25,direct=direct)
		
		cf.contours.last<-contour.gen(subset(CF.Ger.last,select=c('tow','lon','lat','CF')),ticks='define',
		                         nstrata=7,str.min=0,place=2,id.par=5,units="scallops/500g",
		                         interp.method='gstat',key='strata',blank=F,plot=F,subset.poly='square',
		                         subset.eff=0,subscale=0.25,direct=direct)
		...
		# If I want to make one file with all the figures in it
		#pdf(file = paste(direct,"Data/Survey_data/",yr,"/Exploratory_figures/Ger_survey_summary.pdf",sep=""),
		...
		par(mfrow=c(1,1))
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/survey_strata.png",sep=""),units="in",
		 width = 11, height = 8.5,res=420,bg="transparent")
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		ScallopMap(bnk,title=survey.title,plot.boundries = T,boundries = "offshore",
		           plot.bathy=T,bathy.source = "usgs",isobath = c(seq(40,140,by=20)),xlab="",ylab="",cex.mn=2,
		           direct=direct,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr & random==1,pch=20,bg='black',cex=0.8)
		# Add the Matched tows
		points(lat~lon,survGerLive.dat,subset=year==yr & random==3,pch=22,bg="yellow",cex=0.8)
		# Add the legend
		legend('topleft',legend=
		        c(paste('regular (n =',length(unique(subset(survGerLive.dat,year==yr & random==1)$tow)),")", sep=""),
		          paste('repeated (n =',length(unique(subset(survGerLive.dat,year==yr & random==3)$tow)),")", sep="")),
		...
		#Source17 source("fn/survey.ts.r",local=T)
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/abundance_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg="transparent")
		survey.ts(unlined.obj[[1]],1993:yr,Bank=bnk,pdf=F, CS=CS-5, RS=RS-5, ymin=-5,
		...
		dev.off()
				...
				#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/biomass_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg="transparent")
		survey.ts(unlined.obj[[1]],1993:yr,Bank=bnk,pdf=F,type='B', CS=CS-5, RS=RS-5, ymin=-5,yl2=6.5,
		...
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/shell_height_freqency.png",sep="")
		    ,units="in",width = 8.5, height = 11,res=420,bg="transparent")
		shf.plt(ps.dat,lined.obj,from='surv',yr=2008:yr,col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),rel=F,
		        ymax=120,rows=8,mean.line=F,recline=c(RS-5,CS-5),adj=0.05,add.title = T,titl = SHF.title,cex.mn=3,
		        sample.size = lined.obj[[1]]$n[lined.obj[[1]]$year %in% 2008:yr])	
		dev.off()
		# SHF for matched tows on German bank from 2013-2014, Slide 29 in 2014 presentation
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/shell_height_freqency-repeated.png",sep="")
		    ,units="in",width = 11, height = 8.5,res=420,bg="transparent")
		shf.plt(ps.dat,matched.obj,from='surv',yr=(yr-1):yr,col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),rel=F,
		        rows=2,recline=c(RS-5,CS-5),ymax=120,mean.line=F,adj=0.1,add.title = T,titl = "SHF - Repeated Tows (Ger)",cex.mn=2,
		        sample.size = matched.obj[[1]]$n[matched.obj[[1]]$year %in% (yr-1):yr])	
		dev.off()
		...
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Pre-recruits.png",sep=""),units="in",width = 11, 
		...
		ScallopMap(bnk,contour=list(preCont.poly,precont.data),boundries="offshore",plot.boundries = T,
		           title=pre.rec.title,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		...
		legend('topleft',legend=
		       ...
		       pch=c(20,22), pt.bg = c("black","yellow"),bty='n',cex=1, inset = .02)
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		       fill=precont.data$col,title=N.tow.lab, title.adj = 0.2,bg='white',box.col='white',inset=0.02)
		dev.off()
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Recruits.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg="transparent")
		...
		ScallopMap(bnk,contour=list(recCont.poly,recCont.data),boundries="offshore",plot.boundries = T,
		           title=rec.title,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr & random==1,pch=20,bg='black',cex=0.8)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Fully_recruited.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg="transparent")
		...
		ScallopMap(bnk,contour=list(comCont.poly,comCont.data),boundries="offshore",plot.boundries = T,
		           title=fully.rec.title,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr & random==1,pch=20,bg='black',cex=0.8)
		...
		#windows(13,8)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/MWSH_and_CF_ts.png",sep=""),units="in",width = 13,
		    height = 8.5,res=420,bg="transparent")
		par(mfrow=c(1,2))
		shwt.plt1(Ger.SpatHtWt.fit,lw=3,cx=1.2,titl = MWSH.title,cex.mn = 2,las=1)
		...
		stdts.plt(GerCF.dat,y=c('CF','CF2'),pch=c(23,24),col=c('blue','red'),ylab=cf.lab,
		          mean.line=T,graphic='none',xlab='Year',ylim=c(8,18.5),las=1,titl = CF.ts.title,cex.mn=2,tx.ypos=4)
		legend("topright",c("unlined","lined"),pch=c(23,24),pt.bg = c("blue","red"),cex=1.5,lty=c(1,2),col=c("blue","red"),bty="n")
		dev.off()
		
		# CONDITION FACTOR DISTRIBUTION ON GERMAN BANK Slide 38 of 2014 Spring Summary presentation
		lvls=6:14
		range(contourLines(cf.contours$image.dat$z))
		CL <- contourLines(cf.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		# Note that this working is dependent on having the comCont.poly loaded from the Fully recruited spatial map!
		cont.poly <- joinPolys(CP$PolySet,subset(comCont.poly,PID==1&SID%in%c(1,7)))
		cont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),cf.col),border=NA,stringsAsFactors = F) 
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/CF_spatial.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg="transparent")
		par(mfrow=c(1,1))
		...
		ScallopMap(bnk,contour=list(cont.poly,cont.data),boundries="offshore",plot.boundries = T,
		           title=cf.title,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Grab the matched/regular tows from the condition factor data.
		matched.tw <- subset(survGerLive.dat,year==yr & random==3)$tow
		cf.matched <- subset(CF.Ger.current,tow %in% matched.tw)
		cf.regular <- CF.Ger.current[-which(CF.Ger.current$tow %in% matched.tw),]
	  ...
	  #points(lat~lon,survGerLive.dat,subset=year==2014 & random==1,pch=20,bg='black',cex=0.8)
		...
		legend('topleft',legend=
		         c(paste('regular (n =',length(cf.regular$tow),")", sep=""),
		           paste('repeated (n =',length(cf.matched$tow),")", sep="")),
		       pch=c(20,22), pt.bg = c("black","yellow"),bty='n',cex=1, inset = .02)
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),paste(lvls[length(lvls)],'+',sep='')),
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/MC_spatial.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg="transparent")
		
		par(mfrow=c(1,1))
		#windows(11,8.5)  
		ScallopMap(bnk,contour=list(cont.poly,cont.data),boundries="offshore",plot.boundries = T,
		           title=mc.title,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		...
		################################   PREVIOUS YEAR SPATIAL  PLOTS ############################################
		################################   PREVIOUS YEAR SPATIAL  PLOTS ############################################
		################################   PREVIOUS YEAR SPATIAL  PLOTS ############################################
		
		# Pre-recuits Distribution plots for all tows on German 2014
		lvls=c(2,5,10,20,50,100,200,500,1000) # Used for all spatial plots of abundance.
		CL <- contourLines(pre.contours.last$image.dat,levels=lvls)
		CP <- convCP(CL)
		preCont.poly <- joinPolys(CP$PolySet,GerBndry)
		precont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Pre-recruits",yr-1,".png",sep=""),
		    units="in",width = 11, height = 8.5,res=420)
		
		#windows(11,8.5) # Distribution of Pre recruits on German Bank 2014, Slide 34 (note 30-34 are 2010-2013 versions of this)	
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		ScallopMap(bnk,contour=list(preCont.poly,precont.data),boundries="offshore",plot.boundries = T,
		           title=pre.rec.title.last,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		# Add the Matched tows
		points(lat~lon,survGerLive.dat,subset=year==yr-1 & random==3,pch=22,bg="yellow",cex=0.8)
		# Add the legend
		legend('topleft',legend=
		         c(paste('regular (n =',length(unique(subset(survGerLive.dat,year==yr-1 & random==1)$tow)),")", sep=""),
		           paste('repeated (n =',length(unique(subset(survGerLive.dat,year==yr-1 & random==3)$tow)),")", sep="")),
		       pch=c(20,22), pt.bg = c("black","yellow"),bty='n',cex=1, inset = .02)
		...
		# Recruits Distribution plots for all tows on German 2014
		CL <- contourLines(rec.contours.last$image.dat,levels=lvls)
		CP <- convCP(CL)
		recCont.poly <- joinPolys(CP$PolySet,GerBndry)
		recCont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Recruits",yr-1,".png",sep=""),
		    units="in",width = 11, height = 8.5,res=420)
		
		#windows(11,8.5)  # Distribution of recruits on German Bank 2014, Slide 35
		ScallopMap(bnk,contour=list(recCont.poly,recCont.data),boundries="offshore",plot.boundries = T,
		           title=rec.title.last,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		...
		dev.off()
		# Commercial
		CL <- contourLines(com.contours.last$image.dat,levels=lvls)
		CP <- convCP(CL)
		comCont.poly <- joinPolys(CP$PolySet,GerBndry)
		comCont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		par(mfrow=c(1,1))
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Fully_recruited",yr-1,".png",sep=""),
		    units="in",width = 11, height = 8.5,res=420)
		
		#windows(11,8.5) # Distribution of commercial scallops on German Bank 2014, Slide 36
		ScallopMap(bnk,contour=list(comCont.poly,comCont.data),boundries="offshore",plot.boundries = T,
		           title=fully.rec.title.last,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		#points(lat~lon,survGerLive.dat,subset=year==2014,pch=16,cex=0.5)
		# Add the regular survey tows.
		points(lat~lon,survGerLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		...
		# CONDITION FACTOR DISTRIBUTION ON GERMAN BANK Slide 38 of 2014 Spring Summary presentation
		lvls=6:14
		range(contourLines(cf.contours.last$image.dat$z))
		CL <- contourLines(cf.contours.last$image.dat,levels=lvls)
		CP <- convCP(CL)
		# Note that this working is dependent on having the comCont.poly loaded from the Fully recruited spatial map!
		cont.poly <- joinPolys(CP$PolySet,subset(comCont.poly,PID==1&SID%in%c(1,7)))
		cont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),cf.col),border=NA,stringsAsFactors = F) 
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,"Presentations/",yr-1,"/Survey_summary/",bnk,"/CF_spatial.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420)
		par(mfrow=c(1,1))
		#windows(11,8.5) # Slide 39 of 2014 Spring Summary presentation
		ScallopMap(bnk,contour=list(cont.poly,cont.data),boundries="offshore",plot.boundries = T,
		           title=cf.title.last,plot.bathy = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Grab the matched/regular tows from the condition factor data.
		matched.tw <- subset(survGerLive.dat,year==yr-1 & random==3)$tow
		cf.matched <- subset(CF.Ger.last,tow %in% matched.tw)
		cf.regular <- CF.Ger.last[-which(CF.Ger.last$tow %in% matched.tw),]
		...
		################################   END PREVIOUS YEAR SPATIAL  PLOTS ############################################
		################################   END PREVIOUS YEAR SPATIAL  PLOTS ############################################
		################################   END PREVIOUS YEAR SPATIAL  PLOTS ############################################
...
################################################################################################################
################################ SECTION 5 BBs BANK ############################################################
### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs### BBs #####
################################ SECTION 5 BBs BANK ############################################################
################################################################################################################
...
bnk = "BBs" # use bnk for all bank specifics helps to generalize the code for later
		
survey.title <- substitute(bold(paste("Survey strata (",bank,"-",year,")",sep="")),
                           list(year=as.character(max(years)),bank=bnk))
...
CF.ts.title <- substitute(bold(paste("Condition factor time series (",bank,")",sep="")),
                          list(year=as.character(yr),bank=bnk))
BBs.survey.detail.poly<-subset(survey.detail.polys,label==bnk)
attr(BBs.survey.detail.poly,"projection")<-"LL"

#Read25 read removed... Get all the details of the survey strata
BBs.surv.info <- subset(survey.info,label== bnk)

survBBs.dat<- rbind(subset(springSurv.dat,bank==bnk))


# Meat weight data from BBs for 2011 - Current, stitch them all together.
BBs11.mw<-subset(Springsurv2011$MWs,bank==bnk)
BBs13.mw<-subset(SurvDB$MWs,bank==bnk&year==2013)
BBs.mw<-rbind(BBs11.mw,BBs13.mw)

#Convert shell height in current year to decimeters.
BBs13.mwdm<-BBs13.mw
BBs13.mwdm$sh<-BBs13.mw$sh/100
# MEAT WEIGHT DATA - hydration sampling, it contains data from 1983-2007, tidy up the ID 
BBs.wgt.dat<-subset(MW.dat,bank==bnk)
BBs.wgt.dat$ID<-paste(BBs.wgt.dat$year,BBs.wgt.dat$tow,sep='.')
# Grab the relavent Meat-Weight Shell height data and make a flat file from it
BBsmw.dat<-merge(subset(BBs.wgt.dat,month %in% 5:6,c("ID","year","lon","lat","depth","sh","wmw","tow")),
                 subset(BBs.mw,select=c("ID","year","lon","lat","depth","sh","wmw","tow")),all=T)
#Write9
#write.csv(BBsmw.dat,paste(direct,"Data/Survey_Data/",yr,"/Spring/BBs/BBsmwData.csv",sep=""),row.names=F)

# MODEL - This is the meat weight Shell height realationship.  
#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
BBs.SpatHtWt.fit<-shwt.lme(BBs13.mwdm,random.effect='tow',b.par=3)

## MODEL - This is the model used to esimate condition factor across BBs
# We have more data than we did for Sable so a more complex model is fit.  
#Source13 source("fn/condFac.r")
# We currently predict on the mean depth,lat,lon from 2005-2014.
BBscfData<-condFac(na.omit(BBsmw.dat),survBBs.dat,model.type='gam_f')

range(survBBs.dat$year)

# Output the predictions for BBs from the Condition Factor model
survBBs.dat<-BBscfData$pred.dat
# tidy up the ID and Pull out the ID and condition factor
survBBs.dat$ID<-paste(survBBs.dat$year,survBBs.dat$tow,sep='.')
tmp.dat<-subset(BBscfData$CF.data,select=c("ID","CF"))
# Rename CF to CFh
names(tmp.dat)[2]<-"CFh"
# merge the two data sets, keeping all survBBs.dat values
survBBs.dat<-merge(survBBs.dat,tmp.dat,all.x=T)
# Replace any NA's in CFh with the original Condition Factor.
survBBs.dat$CFh[is.na(survBBs.dat$CFh)]<-survBBs.dat$CF[is.na(survBBs.dat$CFh)]


# Get the survey estimates... First we need to re-stratify the survey strata...
#Source20 #source("fn/restratwp.r",local=T)
survBBs.dat<-restratwp(survBBs.dat,list(BBs.survey.detail.poly))
paste("Strata reassigned",
      with(subset(survBBs.dat,year==max(years) & state == 'live' & random == 1),
           round((1-sum(stratum == new.stratum, na.rm = T) / length(stratum))*100)),'%')

# Survey by tow...
survBBs.dat<-surv.by.tow(survBBs.dat, pre.ht=RS, rec.ht=CS)
survBBs.dat<-surv.by.tow(survBBs.dat,pre.ht=RS, rec.ht=CS, type='B', mw.par="CFh")

survBBsRandom.dat <- subset(survBBs.dat,random ==1)



# We need to drop 1984 and 1987 as there are insufficent data in both years to do any analysis...
survBBs.dat <- subset(survBBs.dat, year!=1987 & year != 1984)
BBssurvey.obj<-survey.dat(subset(survBBs.dat,state=="live"), RS=RS, CS=CS, bk=bnk, 
                          areas=BBs.surv.info[,c(1,5)], mw.par="CFh")	


# MEAT COUNT & CONDITION FACTOR OF SCALLOP ON BBn (requires a bit of preprocessing)
CF.BBs.current<-na.omit(merge(subset(na.omit(SurvDB$pos),bank==bnk & year == max(years),
                                     c('tow','lon','lat')),BBs.SpatHtWt.fit$fit))

                                  
names(CF.BBs.current)[4]<-"CF"
CF.BBs.current<-merge(CF.BBs.current,subset(survBBs.dat,year==max(years),c('year','tow','lon','lat',
                                                                       paste('h',seq(5,200,5),sep=''))))

#Source14 source("fn/surv.by.tow.r") surv.by.tow calculates number and/or biomass of pre, rec and 
#com size scallops in each tow + meat count.
CF.BBs.current<-surv.by.tow(CF.BBs.current, max(years), pre.ht=RS, rec.ht=CS)
CF.BBs.current<-surv.by.tow(CF.BBs.current, max(years), pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")

# Meat Count per 500 g
CF.BBs.current$meat.count<-0.5/(CF.BBs.current$com.bm/CF.BBs.current$com)

with(CF.BBs.current,weighted.mean(meat.count,com,na.rm=T))
###################################  BBs Plots  ##########################################################

# If I want to make one file with all these figures in it...
pdf(file = paste(direct,"Data/Survey_data/",max(years),"/Exploratory_figures/BBs_survey_summary.pdf",sep=""),onefile=T,
    width=15,height=8.5)
...
# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
png(paste(direct,"Presentations/",max(years),"/Survey_summary/",bnk,"/survey_strata.png",sep=""),
    units="in",width = 11, height = 8.5,res=420,bg = "transparent")
par(mfrow=c(1,1))
#windows(11,8.5)
ScallopMap(bnk,poly.lst=list(BBs.survey.detail.poly,BBs.surv.info),xlab="",ylab="",title=survey.title,
           bathy.source="usgs",isobath = c(seq(40,140,by=20)),
           plot.bathy = T,plot.boundries = T,boundries="offshore",direct=direct,cex.mn=2,dec.deg = F)
# Add the regular survey tows.
points(lat~lon,survBBs.dat,subset=year==max(years) & random==1 & state=="live",pch=20,bg='black',cex=0.8)
# Add the exploratory survey tows
#points(lat~lon,survBBs.dat,subset=year==max(years)  & random==5 & state=="live",pch=24,bg="darkorange",cex=0.8)
# Add the legends
legend("topright",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",
       legend = c(paste('regular (n =',length(unique(subset(survBBs.dat,year==max(years) & random==1 & state=="live")$tow)),")",sep=""),
                  paste('exploratory (n =',length(unique(subset(survBBs.dat,year==max(years) & random==5 & state=="live")$tow)),")",sep="")),
       inset=0.01,bg='white',box.col='white')
legend("bottomleft",legend=BBs.surv.info$PName,fill=BBs.surv.info$col,bty='n',cex=1, title = "Strata")
legend("bottom",legend = round(BBs.surv.info$area_km2),
       fill=c(BBs.surv.info$col),border=c(rep('black',length(BBs.surv.info$PName))),
       pch=c(rep(NA,length(BBs.surv.info$PName))),title = expression(paste("Area - ",km^2,"")),title.adj=0.1,
       pt.bg = c(rep(NA,length(BBs.surv.info$PName))),col='black',bty='n')

# I need to add a 0 here as we have never surveyed in the tiny strata 2...
tow.leg <- c(aggregate(tow~new.stratum,subset(survBBs.dat,year==max(years) & random==1 & state=="live"),length)$tow[1],0,
             aggregate(tow~new.stratum,subset(survBBs.dat,year==max(years) & random==1 & state=="live"),length)$tow[2:3])
legend("bottomright",legend = tow.leg,
       fill=c(BBs.surv.info$col),border=c(rep('black',length(BBs.surv.info$PName))),
       pch=c(rep(NA,length(BBs.surv.info$PName))),title = "Number of tows",title.adj=0.1,
       pt.bg = c(rep(NA,length(BBs.surv.info$PName))),col='black',bty='n')
...
###############		
png(paste(direct,"Presentations/",yr-2,"/Survey_summary/",bnk,"/abundance_ts.png",sep=""),units="in",width = 8.5, 
    height = 11,res=420,bg = "transparent")
par(mfrow=c(1,1))
survey.ts(BBssurvey.obj[[1]],min(years):yr,Bank=bnk,pdf=F, RS=RS-5, CS=CS-5,
          areas=BBs.surv.info$towable_area,ys=.99,clr='blue',se=T,pch=16,yl2=c(3000,800,800),
          add.title = T,titl = survey.ts.N.title,cx.mn=3,axis.cx = 1.5)
dev.off()
png(paste(direct,"Presentations/",yr-2,"/Survey_summary/",bnk,"/biomass_ts.png",sep=""),units="in",width = 8.5, 
    height = 11,res=420,bg = "transparent")
par(mfrow=c(1,1))
survey.ts(BBssurvey.obj[[1]],min(years):yr,Bank=bnk,pdf=F,type='B', 
          RS=RS-5, CS=CS-5,areas=BBs.surv.info$towable_area,ys=.99,clr='blue',se=T,pch=16,yl2=6,
          add.title = T,titl = survey.ts.BM.title,cx.mn=3,axis.cx = 1.5)
dev.off()
###############		

# Shell height frequency plot, 
#Source18 #source("fn/shf.plt.r")
png(paste(direct,"Presentations/",max(years),"/Survey_summary/",bnk,"/shell_height_freqency.png",sep=""),units="in",width = 8.5, 
    height = 11,res=420,bg = "transparent")
shf.plt(ps.dat,BBssurvey.obj,from='surv',yr=c(2007:2011,2013),col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),
        rel=F,recline=c(CS-5,RS-5),rows=length(c(2007:2011,2013)),mean.line=F,ymax=400,adj=0.1,add.title = T,titl = SHF.title,
        cex.mn=3, sample.size = BBssurvey.obj[[1]]$n[BBssurvey.obj[[1]]$year %in% 2007:max(years)])	
dev.off()


#windows(15,8)
png(paste(direct,"Presentations/",max(years),"/Survey_summary/",bnk,"/MWSH_and_CF_ts.png",sep=""),units="in",width = 13, 
    height = 8.5,res=420,bg = "transparent")
par(mfrow=c(1,2))
#Source12 source("fn/shwt.plt1.r") 
shwt.plt1(BBs.SpatHtWt.fit,lw=3,ht=10,wd=12,cx=1.5,titl = MWSH.title,cex.mn = 2,las=1)
# CONDITION FACTOR TIME SERIES (SLIDE 58, 2014 presentation)
#Source16 source("fn/stdts.plt.R")
stdts.plt(merge(BBscfData$CFyrs,data.frame(year=min(years):yr),all=T),y='CF',pch=16,ylab = cf.lab,
          mean.line=T,graphic='none',xlab='Year',ylim=c(2,10),las=1,
...
################################################################################################################
	  ...
	  clap.dis.title <- substitute(bold(paste("Clappers (% dead ","">=c, " mm ", bank,"-",year,")",sep="")),
		...
		#Source10 source("fn/getdis.r") 
		BBnDis<-dist.coef(c(1:100),path=paste(direct,"Data/Tow_tracks/",yr,"/Spring/BBn/",sep=""),
		                  w=c(1:10,9:1),rule=8,smooth=T,plt=F,meh=1000)
		BBn.extra.Dis<-dist.coef(c(950:970),path=paste(direct,"Data/Tow_tracks/",yr,"/Spring/BBnextras/",sep=""),
		                  w=c(1:10,9:1),rule=8,smooth=T,plt=F)
		
		
		# Here is the survey data for BBn, because of 2015 BBn being during the summer we need to do this...
		survBBn.dat<- rbind(subset(springSurv.dat,bank==bnk),subset(summerSurv.dat,bank==bnk))
		...
		paste("Strata reassigned",
		      with(subset(survBBn.dat,year==yr & state == 'live' & random == 1),
		           round((1-sum(stratum == new.stratum, na.rm = T) / length(stratum))*100)),'%')
	  test <-  subset(survGB.dat,year==yr & state=='live')$random
    strat.chk.BBn <- subset(survBBn.dat,year==yr & state=='live'& random==1)
    mis.match.BBn <- na.omit(strat.chk.BBn[strat.chk.BBn$stratum != strat.chk.BBn$new.stratum,
                                       c("year","cruise","bank","tow","stratum","new.stratum")])
    mis.match.BBn <- rbind(mis.match.BBn,strat.chk.BBn[is.na(strat.chk.BBn$new.stratum),
                           c("year","cruise","bank","tow","stratum","new.stratum")])
    
		# Meat weight data from BBn for 2011 - Current, stitch them all together.
    # DK Note, I need to make this less user inputy every year...
		...
		BBn15.mw<-subset(SurvDB$MWs,bank==bnk&year==2015)
		BBn.mw<-rbind(BBn11.mw,BBn12.mw,BBn13.mw,BBn14.mw,BBn15.mw)
		
		#Convert shell height in current year to decimeters.
		BBn15.mwdm<-BBn15.mw
		BBn15.mwdm$sh<-BBn15.mw$sh/100
		BBn14.mwdm<-BBn14.mw
		...
		# Grab the relavent Meat-Weight Shell height data and make a flat file from it
		BBnmw.dat<-merge(subset(BBn.wgt.dat,year > 1990 & month %in% 5:6,c("ID","year","lon","lat","depth","sh","wmw","tow")),
		                 subset(BBn.mw,select=c("ID","year","lon","lat","depth","sh","wmw","tow")),all=T)
		#Write9
		...
		#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
		#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
		BBn.SpatHtWt.fit<-shwt.lme(BBn15.mwdm,random.effect='tow',b.par=3)
		BBn.SpatHtWt.last.fit<-shwt.lme(BBn14.mwdm,random.effect='tow',b.par=3)
		...
		write.table(survBBn.dat,paste(direct,"Data/Survey_Data/",yr,"/Spring/BBn/BBnSurvey91",yr,".csv",sep=""),sep=',',row.names=F)
		...
		survBBnClap.dat<-subset(survBBn.dat,state=='dead' & year >= min(years))
		survBBnLive.dat<-subset(survBBn.dat,state=='live' & year >= min(years))
		survBBnRandom.dat<-subset(survBBn.dat,state=='live'& random==1)
		
		# Proportion of Clappers on BBn for each class and overall
		survBBnClap.dat$clap.prop<-survBBnClap.dat$tot/(survBBnLive.dat$tot+survBBnClap.dat$tot)*100
		survBBnClap.dat$clap.prop[is.na(survBBnClap.dat$clap.prop)]<-0
		survBBnClap.dat$clap.propCom<-survBBnClap.dat$com/(survBBnLive.dat$com+survBBnClap.dat$com)*100
		survBBnClap.dat$clap.propCom[is.na(survBBnClap.dat$clap.propCom)]<-0
		survBBnClap.dat$clap.propRec<-survBBnClap.dat$rec/(survBBnLive.dat$rec+survBBnClap.dat$rec)*100
		survBBnClap.dat$clap.propRec[is.na(survBBnClap.dat$clap.propRec)]<-0
		survBBnClap.dat$clap.propPre<-survBBnClap.dat$pre/(survBBnLive.dat$pre+survBBnClap.dat$pre)*100
		survBBnClap.dat$clap.propPre[is.na(survBBnClap.dat$clap.propPre)]<-0
		
		survBBnClap.dat$clap.prop.Rcom<- (survBBnClap.dat$com + survBBnClap.dat$rec) / 
		                                  (survBBnLive.dat$com +survBBnLive.dat$rec +survBBnClap.dat$com + survBBnClap.dat$rec)*100
		survBBnClap.dat$clap.prop.Rcom[is.na(survBBnClap.dat$clap.prop.Rcom)]<-0
		...
		dump(c('BBnsurvey.obj','BBnclap.obj'),
		     file = paste(direct,'Data/Survey_data/',yr,'/Survey_summary_output/BBnsurveyObj.R',sep=""))
		# If we want to skip all the processing we could just call in the survey object like so...
		#Source24 source('BBnsurveyObj.R')
		
		# Now I'd like this output so I have some of the annual numbers for Survey Summary in a nice formate
		SS.summary.BBn <- BBnsurvey.obj[[1]]
		SS.summary.BBn$bank <- "BBn"
		names(SS.summary.BBn) <- c("year","n","FR.BM","CV.FR.BM","R.BM","CV.R.BM","Pre.BM","CV.Pre.BM",
		                           "FR_N", "CV.FR.N",  "R.N","CV.R.N","Pre.N", "CV.Pre.N",
		                           "MW.mean","SH.mean","SH.k","MW.k","CF","FR.clap","R.clap","bank")
		# I want to get this into per tow 
		SS.summary.BBn[names(SS.summary.BBn) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] <-
		SS.summary.BBn[names(SS.summary.BBn) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] / 
		                                                               sum(BBn.surv.info$towable_area) * 10^6 
		write.csv(SS.summary.BBn,
		          file = paste(direct,"Data/Survey_data/",yr,"/Spring/",bnk,"/",bnk,"_annual_summary",yr,".csv",sep=""),
		          row.names = F)
		# Same for the SHF data.
		SHF.summary.BBn <- as.data.frame(cbind(BBnsurvey.obj[[1]]$year,BBnsurvey.obj[[2]]$n.yst))
		SHF.summary.BBn$bank <- "BBn"
		write.csv(SHF.summary.BBn,
		          file = paste(direct,"Data/Survey_data/",yr,"/Spring/",bnk,"/",bnk,"_annual_SHF_summary",yr,".csv",sep=""),
		          row.names = F)
		...
		C2.obj<-simple.surv(survBBnLive.dat[1:nrow(survBBnLive.dat) %in% key$EID,],years=2007:yr)
		key<-findPolys(box.dat,subset(BBboxes,SCALLOP_Group_ID=='201401C3'))
		C3.obj<-simple.surv(survBBnLive.dat[1:nrow(survBBnLive.dat) %in% key$EID,],years=2007:yr)
	  ...
	  # This year < 95
		...
		# Previous year spatial data 		# Previous year spatial data
		pre.contours.last<-contour.gen(subset(survBBnLive.dat,year==yr-1,c('tow','lon','lat','pre')),
		                          ticks='define',nstrata=9,str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.005,key='log.cont',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,
		                          subset.poly='square',subset.eff=0,subscale=0.25,direct=direct)
		# Recruit (95 - 105 mm shell height)
		rec.contours.last<-contour.gen(subset(survBBnLive.dat,year==yr-1,c('tow','lon','lat','rec')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.005,key='log.cont',color.fun=tim.colors,
		                          id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,subset.poly='square',
		                          subset.eff=0,subscale=0.25,direct=direct)
		# Commercial sized scallop (> 105 mm)
		com.contours.last<-contour.gen(subset(survBBnLive.dat,year==yr-1,c('tow','lon','lat','com')),
		                          ticks=c(1,5,10,50,100,500,1000,5000),str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.005,key='log.cont',color.fun=tim.colors,
		                          id.par=3.5,units='#/tow',plot=F,blank.dist=0.08,subset.poly='square',
		                          subset.eff=0,subscale=0.25,direct=direct)
		...
		# MEAT COUNT & CONDITION FACTOR OF SCALLOP ON BBn (requires a bit of preprocessing)
		CF.BBn.current <- na.omit(merge(subset(na.omit(SurvDB$pos),bank==bnk & year == yr,
		                                    c('tow','lon','lat')),BBn.SpatHtWt.fit$fit))
		CF.BBn.last<-na.omit(merge(subset(na.omit(SurvDB$pos),bank==bnk & year == yr-1,
		                                      c('tow','lon','lat')),BBn.SpatHtWt.last.fit$fit))
		names(CF.BBn.current)[4]<-"CF"
		names(CF.BBn.last)[4]<-"CF"
		CF.BBn.current<-merge(CF.BBn.current,subset(survBBnLive.dat,year==yr,c('year','tow','lon','lat',
		                                                                       paste('h',seq(5,200,5),sep=''))))
		CF.BBn.last<-merge(CF.BBn.last,subset(survBBnLive.dat,year==yr-1,c('year','tow','lon','lat',
		                                                                         paste('h',seq(5,200,5),sep=''))))
		
		#Source14 source("fn/surv.by.tow.r") surv.by.tow calculates number and/or biomass of pre, rec and 
		#com size scallops in each tow + meat count.
		CF.BBn.current<-surv.by.tow(CF.BBn.current, yr, pre.ht=RS, rec.ht=CS)
		CF.BBn.current<-surv.by.tow(CF.BBn.current, yr, pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")
		CF.BBn.last<-surv.by.tow(CF.BBn.last, yr-1, pre.ht=RS, rec.ht=CS)
		CF.BBn.last<-surv.by.tow(CF.BBn.last, yr-1, pre.ht=RS, rec.ht=CS, type='B', mw.par="CF")
		
		# Meat Count per 500 g
		CF.BBn.current$meat.count<-0.5/(CF.BBn.current$com.bm/CF.BBn.current$com)
		CF.BBn.last$meat.count<-0.5/(CF.BBn.last$com.bm/CF.BBn.last$com)
		
		# CONDITION FACTOR MEAT COUNT and Clappers BBn in 2015 
		#Source19 source("fn/contour.gen.r")
		mc.contours<-contour.gen(subset(CF.BBn.current,select=c('tow','lon','lat','meat.count')),ticks='define',
		...
		# Condition Factor
		cf.contours<-contour.gen(subset(CF.BBn.current,select=c('tow','lon','lat','CF')),ticks='define',
		                         nstrata=7,str.min=0,place=2,id.par=3.5,interp.method='gstat',direct=direct,
		                         key='strata',blank=F,plot=F,subset.poly='square',subset.eff=0,subscale=0.25)
		# Condition Factor
		cf.contours.last<-contour.gen(subset(CF.BBn.last,select=c('tow','lon','lat','CF')),ticks='define',
		                         nstrata=7,str.min=0,place=2,id.par=3.5,interp.method='gstat',direct=direct,
		                         key='strata',blank=F,plot=F,subset.poly='square',subset.eff=0,subscale=0.25)
...
### BBn FIGURES### BBn FIGURES### BBn FIGURES### BBn FIGURES### BBn FIGURES### BBn FIGURES### BBn FIGURES### BBn FIGURES
		...
		# If I want to make one file with all these figures in it...
		pdf(file = paste(direct,"Data/Survey_data/",yr,"/Exploratory_figures/BBn_survey_summary.pdf",sep=""),onefile=T,
		    width=15,height=8.5)
		...
		# SURVEY INFO ON BBn was too sparse.
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/survey_strata.png",sep=""),
		     units="in",width = 11, height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		# Add the exploratory survey tows
		points(lat~lon,survBBnLive.dat,subset=year==yr  & random==5,pch=24,bg="darkorange",cex=0.8)
		# Add the legends
		legend("topleft",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",
		       legend = c(paste('regular (n =',length(unique(subset(survBBnLive.dat,year==yr & random==1)$tow)),")",sep=""),
		                  paste('exploratory (n =',length(unique(subset(survBBnLive.dat,year==yr & random==5)$tow)),")",sep="")),
		       inset=0.01,bg='white',box.col='white')
		...
		legend("bottomright",legend = as.numeric(with(subset(survBBnLive.dat,year== yr & random ==1),
		                                            tapply(tow,new.stratum,length))),
		       fill=c(BBn.surv.info$col),border=c(rep('black',length(BBn.surv.info$PName))),
		...
		shf.plt(ps.dat,BBnsurvey.obj,from='surv',yr=2007:yr,col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),
		        rel=F,recline=c(85,95),rows=9,mean.line=F,ymax=730,adj=0.1,add.title = T,titl = SHF.title,cex.mn=3,
		        sample.size = BBnsurvey.obj[[1]]$n[BBnsurvey.obj[[1]]$year %in% 2007:yr])	
		dev.off()
		
		# Pre-recruits for BBn in 2015 (< 95 mm shell height) Slide 48 2014 presentation
		range(pre.contours$image.dat$z)
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		CL <- contourLines(pre.contours$image.dat,levels=lvls1)
		CP <- convCP(CL)
		preCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		...
		if(max(pre.contours$image.dat$z)>=lvls2[1])
		{
		...
		if(max(pre.contours$image.dat$z)<lvls2[1])
		{
		  preCont.poly<-preCont1.poly
		  precont.data<-precont1.data
		  lvls <- lvls1
		} # end if(max(rec.contours$image.dat$z)<lvls2[1])
		...
		ScallopMap(bnk,contour=list(preCont.poly,precont.data),title=pre.rec.title,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBnLive.dat,subset=year==yr & random==1,pch=20,bg='black',cex=0.8)
		# Add the exploratory survey tows
		...
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		       ...
		       border="black",pch=c(rep(NA,length(lvls))),
		...
		range(rec.contours$image.dat$z) # are the levels reasonable?
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		CL <- contourLines(rec.contours$image.dat,levels=lvls1)
		CP <- convCP(CL)
		recCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		...
		if(max(rec.contours$image.dat$z)>=lvls2[1])
		{
		  ...
		  CP <- convCP(CL)
		  recCont2.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		...
		# Fully Recruited Scallops on BBn in 2014 (Slide 52 in 2014 presentation)
				...
				range(com.contours$image.dat$z) # are the levels reasonable?
				lvls1=c(2,5,10,20,50,100,200,500,1000)
				CL <- contourLines(com.contours$image.dat,levels=lvls1)
				CP <- convCP(CL)
				comCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
				...
				if(max(com.contours$image.dat$z)>=lvls2[1])
				{
				  ...
				  CP <- convCP(CL)
				  comCont2.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		...
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk,contour=list(comCont.poly,comcont.data),title=fully.rec.title,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBnLive.dat,subset=year==yr & random==1,pch=20,bg='black',cex=0.8)
		...
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                      paste(lvls[length(lvls)],'+',sep='')),fill=c(comcont.data$col),title=N.tow.lab, title.adj = 0.2,
		       border="black",pch=c(rep(NA,length(lvls))),
		       pt.bg = c(rep(NA,length(lvls))),inset=0.01,bg='white',box.col='white')
				addPolys(BBboxes,lty=2,lwd=2)
		dev.off()
...
############################ SEEDBOXES############################ SEEDBOXES############################ SEEDBOXES#####
		...
		#Source18 #source("fn/shf.plt.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/C2-shell_height_freqency.png",sep=""),units="in",
		    width = 8.5, height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,C2.obj,from='surv',yr=2007:yr,col1='grey80',type='sh',col2=1,col3=1,
		        xl=c(0,200),rel=F,recline=c(85,95),rows=9,mean.line=F,ymax=3600,adj=0.1,
		        add.title=T,cex.mn=3,titl= paste("Shell height frequency (Seedbox C2-",bnk,")",sep=""),
		        sample.size = C2.obj[[1]]$n[C2.obj[[1]]$year %in%2007:yr ])
		dev.off()
		
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/C3-shell_height_freqency.png",sep=""),units="in",
		    width = 8.5, height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,C3.obj,from='surv',yr=2007:yr,col1='grey80',type='sh',col2=1,col3=1,
		        xl=c(0,200),rel=F,recline=c(85,95),rows=9,mean.line=F,adj=0.1,ymax=3600,
		        add.title=T,cex.mn=3,titl=  paste("Shell height frequency (Seedbox C3-",bnk,")",sep=""),
		        sample.size = C3.obj[[1]]$n[C3.obj[[1]]$year %in%2007:yr ])
		dev.off()
		...
		BBn.tows.2015 <- subset(survBBnLive.dat,year==yr,select=c('tow','lon','lat'))
		names(BBn.tows.2015)<-c("EID","X","Y")
		C2 <-findPolys(BBn.tows.2015,subset(BBboxes,ID=='C2-012014'))
		C3 <-findPolys(BBn.tows.2015,subset(BBboxes,ID=='C3-012014'))
		
		
		
		# Revising to show all size classes in the seedbox like we do in GBa, repalces Slide 48 2014 presentation
		########### I am putting this plot first in 2015 as there is no big overview slide this year since we 
		# just have the one seedbox.
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/C2-Seedbox_spatial.png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(2,2),omi=c(0.1,0.2,0.5,0.5),xpd=F)
		#windows(11,8.5)
		ScallopMap(ylim=c(42.797,42.867),xlim=c(-66.282,-66.12),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           contour=list(comCont.poly,comcont.data),plot.bathy = T,plot.boundries = T,direct=direct,
		           title="",dec.deg = F,ylab="",xlab="",cex.mn=1.3)
		title(substitute(bold(paste("Fully recruited scallops (","">=a, " mm " ,")",sep="")),
		                 list(a=as.character(CS-5),year=as.character(yr))),cex.main=1.3,line=1.1)
		addPolys(BBboxes,lty=2,lwd=2)
		addLines(BBnDis[[2]],col='blue')
		addLines(BBn.extra.Dis[[2]],col='blue')
		# Add the regular survey tows.
		points(slat~slon,survBBn.dat,subset=year==yr & state=='live'& random==1,pch=20,bg='black',cex=1)
		# Add the exploratory survey tows
		points(slat~slon,survBBn.dat,subset=year==yr&state =='live' & random==5,pch=24,bg="darkorange",cex=1)
		#Recruits
		ScallopMap(ylim=c(42.797,42.867),xlim=c(-66.282,-66.12),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           contour=list(recCont.poly,reccont.data),plot.bathy = T,plot.boundries = T,direct=direct,
		           title="",dec.deg = F,ylab="",xlab="",cex.mn=1.3)
		title(substitute(bold(paste("Recruit scallops (",a,"-",b, " mm)",sep="")),
		                 list(a = as.character(RS-5),b=as.character(CS-6))),cex.main=1.3,line=1.1)
		...
		ScallopMap(ylim=c(42.797,42.867),xlim=c(-66.282,-66.12),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           ...
		           title="",dec.deg = F,ylab="",xlab="",cex.mn=1.3)
		title(substitute(bold(paste("Pre-recruit scallops (",""<a, " mm)",sep="")),
		                 list(a=as.character(RS-5),year=as.character(yr))),cex.main=1.3,line =1.1)
		addPolys(BBboxes,lty=2,lwd=2)
		addLines(BBnDis[[2]],col='blue')
		addLines(BBn.extra.Dis[[2]],col='blue')
		# Add the regular survey tows.
		...
		legend("topright",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",inset=0.01,
		       legend = c(paste('regular (n =',
		                ...
		                paste('exploratory (n =',
		                    length(subset(survBBn.dat,year==yr & state=='live'& random==5 & tow %in% C2$EID)$tow),")",sep="")),
		       bg=NA,box.col=NA,bty="n")
		title(paste("Seedbox C2 (",bnk,"-",yr,")",sep=""),cex.main=2,outer=T,line=-1)
		dev.off()
		
		
		
		
		#SeedBoxes Revised to plot all of C3 data...(Formerly the sidebar on Slide 54, 2014 Presentation)
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		lvls2=c(2000,5000,10000,20000,50000)
		# If counts are high enough to need lvls2 we use it, if not we just use lvls1.
		if(max(pre.contours$image.dat$z)>=lvls2[1])
		...
		##############
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/C3-Seedbox_spatial.png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		
		par(mfrow=c(2,2),omi=c(0.1,0.2,0.5,0.2),xpd=F)
		#windows(11,8.5)
		ScallopMap(ylim=c(42.78,42.885),xlim=c(-66.05,-65.94),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           contour=list(comCont.poly,comcont.data),plot.bathy = T,plot.boundries = T,direct=direct,
		           title="",dec.deg = F,ylab="",xlab="",cex.mn=1.3)
		title(substitute(bold(paste("Fully recruited scallops (","">=a," mm)",sep="")),
		           list(a=as.character(CS-5))),cex.main=1.3,line=1.1)
		addPolys(BBboxes,lty=2,lwd=2)
		addLines(BBnDis[[2]],col='blue')
		addLines(BBn.extra.Dis[[2]],col='blue')
		# Add the regular survey tows.
    ...
    title(substitute(bold(paste("Recruit scallops (",a,"-",b, " mm)",sep="")),
		...
		#Pre-recuits
		ScallopMap(ylim=c(42.78,42.885),xlim=c(-66.05,-65.94),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           contour=list(preCont.poly,precont.data),plot.bathy = T,plot.boundries = T,direct=direct, cex.mn=1.3,
		           title="", dec.deg = F,ylab="",xlab="")
		title(substitute(bold(paste("Pre-recruit scallops (",""<a, " mm)",sep="")),
		                 list(a=as.character(RS-5),year=as.character(yr))),cex.main=1.3,line=1.1)
		...
		points(slat~slon,survBBn.dat,subset=year==yr&state =='live' & random==5,pch=24,bg="darkorange",cex=1)
		par(xpd=T)
		plot(1:10,type='n',axes=F,xlab='',ylab='',main="",cex.main=2)
		legend("left",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                paste(lvls[length(lvls)],'+',sep='')),fill=c(precont.data$col),
		       border="black",pch=c(rep(NA,length(lvls))),title = N.tow.lab,title.adj = 0.2,
		       pt.bg = c(rep(NA,length(lvls2))),bg=NA,bty="n")
		...
		legend("topright",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",inset=0.01,
		       legend = c(paste('regular (n =',
		            ...
		            paste('exploratory (n =',
		                 length(subset(survBBn.dat,year==yr & state=='live'& random==5 & tow %in% C3$EID)$tow),")",sep="")),
		       bg=NA,box.col='white')
		title(paste("Seedbox C3 (",bnk,"-",yr,")",sep=""),cex.main=2,outer=T,line=-1)
		dev.off()
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Clapper_abund_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Clapper_per_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		Clap3.plt(Clap.dat,years=1991:yr,yl=c(0,40),add.title = T,cex.mn = 3,
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Clapper_spatial.png",sep=""),units="in",width = 11,
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		       ...
		       border="black",pch=c(rep(NA,length(lvls))),
		       pt.bg = c(rep(NA,length(lvls))),inset=0.01,bg=NA,box.col=NA,bty="n")
				addPolys(BBboxes,lty=2,lwd=2)
				dev.off()
				
		
		# MEAT WEIGHT SHELL HEIGHT PLOT BBn (slide 58, 2014 presentation)
		...
		#CONDITION FACTOR on BBn for 2015. SLide 59 from 2014 presenation 
		lvls=8:16
		CL <- contourLines(cf.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		...
		exp.tw <- subset(survBBnLive.dat,year==yr & random==5)$tow
		reg.tw <- subset(survBBnLive.dat,year==yr & random==1)$tow
		cf.exp <- subset(CF.BBn.current,tow %in% exp.tw)
		cf.reg <- CF.BBn.current[which(CF.BBn.current$tow %in% reg.tw),]
		...
		# MEAT COUNT on BBn for current year.  Slide 60 of 2014 presentation
		lvls=seq(5,40,5)
		div=3
		CL <- contourLines(mc.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		...
		# Pre-recruits for BBn in 2015 (< 95 mm shell height) Slide 48 2014 presentation
		range(pre.contours.last$image.dat$z)
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		CL <- contourLines(pre.contours.last$image.dat,levels=lvls1)
		CP <- convCP(CL)
		preCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		precont1.data<- data.frame(PID=1:length(lvls1),col=brewer.pal(length(lvls1),N.col),border=NA,stringsAsFactors = F) 
		lvls2=c(2000,5000,10000,20000,50000)
		# If counts are high enough to need lvls2 we use it, if not we just use lvls1.
		if(max(pre.contours.last$image.dat$z)>=lvls2[1])
		{
		  CL <- contourLines(pre.contours.last$image.dat,levels=lvls2)
		  CP <- convCP(CL)
		  preCont2.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		  preCont2.poly$PID<-preCont2.poly$PID+length(lvls1)
		  preCont.poly<-rbind(preCont1.poly,preCont2.poly)
		...
		#This is the prerecruit plot in the presentation.  There is also a 2013 version of this in the ppt.
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Pre-recruits",yr-1,".png",sep=""),units="in",width = 11,
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk,contour=list(preCont.poly,precont.data),title=pre.rec.title.last,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBnLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		# Add the exploratory survey tows
		points(lat~lon,survBBnLive.dat,subset=year==yr-1  & random==4,pch=24,bg="darkorange",cex=0.8)
		# Add the legends
		legend("topleft",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",
		       legend = c(paste('regular (n =',length(unique(subset(survBBnLive.dat,year==yr-1 & random==1)$tow)),")",sep=""),
		                  paste('exploratory (n =',length(unique(subset(survBBnLive.dat,year==yr-1 & random==4)$tow)),")",sep="")),
		       inset=0.01,bg='white',box.col='white')
		...
		############
		# Recruit Scallops on BBn in 2014 (Slide 50 in 2014 presentation)
		range(rec.contours.last$image.dat$z) # are the levels reasonable?
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		CL <- contourLines(rec.contours.last$image.dat,levels=lvls1)
		CP <- convCP(CL)
		recCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		reccont1.data<- data.frame(PID=1:length(lvls1),col=brewer.pal(length(lvls1),N.col),border=NA,stringsAsFactors = F)
		lvls2=c(2000,5000,10000,20000,50000)
		# If counts are high enough to need lvls2 we use it, if not we just use lvls1.
		if(max(rec.contours.last$image.dat$z)>=lvls2[1])
		{
		  CL <- contourLines(rec.contours.last$image.dat,levels=lvls2)
		  CP <- convCP(CL)
		  recCont2.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		  recCont2.poly$PID<-recCont2.poly$PID+length(lvls1)
		  recCont.poly<-rbind(recCont1.poly,recCont2.poly)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Recruits",yr-1,".png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk,contour=list(recCont.poly,reccont.data),title=rec.title.last,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBnLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		...
		range(com.contours.last$image.dat$z) # are the levels reasonable?
		lvls1=c(2,5,10,20,50,100,200,500,1000)
		CL <- contourLines(com.contours.last$image.dat,levels=lvls1)
		CP <- convCP(CL)
		comCont1.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		comcont1.data<- data.frame(PID=1:length(lvls1),col=brewer.pal(length(lvls1),N.col),border=NA,stringsAsFactors = F) 
		lvls2=c(2000,5000,10000,20000,50000)
		# If counts are high enough to need lvls2 we use it, if not we just use lvls1.
		if(max(com.contours.last$image.dat$z)>=lvls2[1])
		{
		  CL <- contourLines(com.contours.last$image.dat,levels=lvls2)
		  CP <- convCP(CL)
		  comCont2.poly <- joinPolys(CP$PolySet,BBn.survey.bound.poly)
		  comCont2.poly$PID<-comCont2.poly$PID+length(lvls1)
		  comCont.poly<-rbind(comCont1.poly,comCont2.poly)
		...
		#Source9
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk,"/Fully_recruited",yr-1,".png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk,contour=list(comCont.poly,comcont.data),title=fully.rec.title.last,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBnLive.dat,subset=year==yr-1 & random==1,pch=20,bg='black',cex=0.8)
		...
		legend("topleft",pch=c(20,24), pt.bg = c("black","darkorange"), title="Tow type",
		  legend = c(paste('regular (n =',length(unique(subset(survBBnLive.dat,year==yr-1 & random==1)$tow)),")",sep=""),
		              paste('exploratory (n =',length(unique(subset(survBBnLive.dat,year==yr-1 & random==4)$tow)),")",
		                        sep="")),inset=0.01,bg='white',box.col='white')
		
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                      paste(lvls[length(lvls)],'+',sep='')),fill=c(comcont.data$col),title=N.tow.lab, 
		       border="black",pch=c(rep(NA,length(lvls))),title.adj = 0.2,
		       pt.bg = c(rep(NA,length(lvls))),inset=0.01,bg='white',box.col='white')
		addPolys(BBboxes,lty=2,lwd=2)
		dev.off()
		############
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,"Presentations/",yr-1,"/Survey_summary/",bnk,"/CF_spatial.png",sep=""),units="in",
		    width = 11, height = 8.5,res=420,bg = "transparent")
		
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk,contour=list(cont.poly,cont.data),title=cf.title.last,
		           bathy.source="usgs",isobath = c(seq(40,140,by=20)),plot.bathy=T,plot.boundries=T,boundries="offshore",
		           direct=direct,cex.mn=2,xlab="",ylab="",dec.deg = F)
		# Grab the exploratory/regular tows from the condition factor data.
		exp.tw <- subset(survBBnLive.dat,year==yr-1 & random==4)$tow
		reg.tw <- subset(survBBnLive.dat,year==yr-1 & random==1)$tow
		cf.exp <- subset(CF.BBn.last,tow %in% exp.tw)
		cf.reg <- CF.BBn.last[which(CF.BBn.last$tow %in% reg.tw),]
...
################  NOT in presentation but likely of interest  ############################################
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		windows(12,8)
		ScallopMap(ylim=c(42.5,42.95),xlim=c(-66.5,-65.7),bathy.source="usgs",isobath = c(seq(40,140,by=20)),
		           direct=direct,contour=list(preCont.poly,precont.data),plot.bathy = T,
		           title="BBn 2012 pre-recruits (< 85 mm shell height)",cex=1.2,plot.boundries = T,
		           boundries="offshore",dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survBBn.dat,subset=year==yr & state=='live'& random==1,pch=20,bg='black',cex=0.8)
		# Add the exploratory survey tows
		points(lat~lon,survBBn.dat,subset=year==yr&state =='live' & random==5,pch=24,bg="darkorange",cex=0.8)
		# Add the legend
		...
		# Shell height for knife edge recruitment based on portsampling data DK note: These are 5 higher than reality.
		# Note how these have changed over time...
		# Correctly specifying the years here really matters since the RS and CS are changing with time, begs for a 
		# better method!!
		# 1981-1985 CS = 80, RS = 65
		# From 1986-1995 CS = 90, RS= 80
		# From 1996-current CS= 100, RS = 90
		# The first few years here are based on starting at 1984
		CS = c(rep(80,2),rep(90,10),rep(100,yr-1995))
		# RS = Shell height 1 year previous to CS
		RS = c(rep(65,2),rep(80,10),rep(90,yr-1995))	
		...
		tow.track.title <- substitute(bold(paste("Tow tracks (",bank,"-Spring ",year,")",sep="")),
		                              list(year=as.character(yr),bank=bnk[1]))
		fully.rec.title <-substitute(bold(paste("Fully recruited scallops (" ,"">=a, " mm " , bank,"-Spring ",year,")",
		                                        sep="")),list(a=as.character(CS-5),year=as.character(yr),bank=bnk[1]))
		...
		# The 2015 data is strictly from the C1 seedbox and is not to be used as anything more than a comparison of interest
		# There was no complete spring survey in 2015 due to vessel issues!!!!
		surv_2015 <- subset(SurvDB$SHF,cruise == "PS01")
		surv_2015$lon <- with(surv_2015,apply(cbind(elon,slon),1,mean))
		surv_2015$lat <- with(surv_2015,apply(cbind(elat,slat),1,mean))
		survGBMay.dat <- rbind(survGBMay.dat,surv_2015)
		...
		GB11.mw<-subset(Springsurv2011$MWs,bank %in% bnk)
		GB12.mw<-subset(Springsurv2012$MWs,bank %in% bnk)
		GB13.mw<-subset(SurvDB$MWs,bank %in% bnk & cruise=="TE15")
		GB14.mw<-subset(SurvDB$MWs,bank %in% bnk & cruise=="TE17")
		# The 2015 data is strictly from the C1 seedbox and is not to be used as anything more than a comparison of interest
		# There was no complete spring survey in 2015 due to vessel issues!!!!
		GB15.mw <- subset(SurvDB$MWs, cruise=="PS01")
		GBMay.mw<-rbind(GB11.mw,GB12.mw,GB13.mw,GB14.mw,GB15.mw)
		GBMay.mw$month<-5
		# Convert shell height in 2014 to decimeters.
		GB15.mwdm<-GB15.mw
		...
		# Grab the relavent Meat-Weight Shell height data and make a flat file from it
		GBmwMay.dat<-merge(subset(GB.wgt.dat,month %in% 5:6 & tow%in% 301:324,
		                          c("ID","year","lon","lat","depth","sh","wmw","tow","month")),
		                   subset(GBMay.mw,tow %in% 301:324,
		                          c("ID","year","lon","lat","depth","sh","wmw","tow","month")),all=T)
		#Write11 
		...
		GB.spring.SpatHtWt.fit<-shwt.lme(GB15.mwdm,random.effect='tow',b.par=3)
		
		## MODEL - This is the model used to esimate condition factor across GB spring
		# We have more data than we do for Sable so a more complex model is fit.  
		#Source13 source("fn/condFac.r")
		# If you don't have the summer GB data yet you will need to change "yr" to to "yr-1"
		source(paste(direct,"Data/Survey_data/",yr,"/Survey_summary_output/GBcfData.R",sep=""))
		#Source13 source("fn/condFac.r") Calculate the condition factor in May
		# We currently predict on the mean depth,lat,lon from 2005-2014 SUMMER survey so I need to bring these data in...
		GBAug.mw<-rbind(subset(Summersurv2011$MWs[,-grep("stratum",names(Summersurv2011$MWs))],bank %in% bnk[2:3]),
		                na.omit(subset(Summersurv2012$MWs[,-grep("stratum",names(Summersurv2012$MWs))],bank %in% bnk[2:3])),
		                subset(SurvDB$MWs,bank %in% bnk[2:3] & cruise =="TE16"),
		                subset(SurvDB$MWs,bank %in% bnk[2:3] & cruise =="TE18"),
		                subset(SurvDB$MWs,bank %in% bnk[2:3] & cruise =="LE02"))
		GBAug.mw$month<-8
		...
		pred.loc.GB.summer <- NULL
		pred.loc.GB.summer[["depth"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$depth,na.rm=T)
		pred.loc.GB.summer[["lat"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$lat,na.rm=T)
		pred.loc.GB.summer[["lon"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$lon,na.rm=T)
		
		GBcfDataMay<-condFac(na.omit(GBmw.dat[GBmw.dat$month %in% c(5,6),]),survGBMay.dat,model.type='gam_f',
		                     pred.loc = pred.loc.GB.summer)
		# ID the may data
		...
		CFdata	<- merge(CFdata,data.frame(year=2016),all=T)
		...
		write.table(survGBMay.dat,paste(direct,"Data/Survey_data/",yr,"/Spring/GB/GBMaySurvey",min(years),"-",max(years),".csv"
		                                ,sep=""),sep=',',row.names=F)
		...
		ScallopMap(bnk[1],xlab="",ylab="",title=survey.title,nafo='all',nafo.bord = T, plot.EEZ = T,
		           bathy.source="quick",plot.bathy = T,plot.boundries = T,
		...
		# Add the exploratory survey tows
		points(lat~lon,survGBLive.dat,subset = year==2014 & state =='live' & random %in% c(2,4,5),
		       pch=24,bg="darkorange",cex=0.8)
		# Add the legend
		legend("bottomleft",
		...
		#Source18 source("fn/shf.plt.r") Shell Height Frequency data for GB spring, this is abundance (slide 64 2014)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		windows(11,8.5)
		ScallopMap(bnk[1],contour=list(preCont.poly,precont.data), xlab="",ylab="",title=pre.rec.title,nafo='all',
		           nafo.lab = F,nafo.bord = T, plot.EEZ = T, bathy.source="quick",plot.bathy = T,plot.boundries = T,
		           boundries="offshore",direct=direct,cex.mn=2,dec.deg = F)
		
		# Add the comparative survey tows.
		points(slat~slon,survGBLive.dat,subset = year == 2014 & state=='live' & random == 3 , pch=23,bg='BLUE',cex=0.8)
		# Add the exploratory survey tows
		points(slat~slon,survGBLive.dat,subset = year==2014 & state =='live' & random %in% c(2,4,5), 
		       pch=24,bg="darkorange",cex=0.8)
		# Add the legend
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
    ...
    shwt.plt1(GB.spring.SpatHtWt.fit,lw=3,cx=1.5,titl = MWSH.title,cex.mn = 2,las=1)
		...
		# SEEDBOXES:  All of this is required to make something similar to Slide 68 in 2014 presentation,
		#  slightly modified to work with new seedbox data and added other information.
		...
		## Here we plot the maps for each of the seed banks, first we do Seedbox for the 3 size classes. 
		#Slide 69, 2014 Presentation
		#lvls=c(1,100,200,500,1000,2000,5000,10000)
		...
		windows(11,8.5)
		par(mfrow=c(2,2))
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r") The Fully recruited scallops
		...
		legend("topright",pch=c(24,23),pt.bg = c("darkorange","Blue"), title = "Tow type",
		       ...
		       paste('comparative (n =',length(unique(subset(survGBLive.dat,year==yr & 
		                                      random ==3 & tow %in% SB$EID)$tow)),")",sep="")),
...
##################  Figures potentially of interest but not included in presentation ##################
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		windows(12,8)
		ScallopMap(ylim=c(41.2,42.2),xlim=c(-67.3,-65.6),bathy.source="quick",bathcol=rgb(0,0,1,0.3),
		           contour=list(comCont.poly,cont.data),plot.bathy = T,plot.boundries = T,
		           title="GBa 2014 fully recruited scallops (>= 95 mm shell height)",dec.deg = F)
		# Add the comparative survey tows.
		...
		windows(12,8)
		ScallopMap(ylim=c(41.2,42.2),xlim=c(-67.3,-65.6),bathy.source="quick",bathcol=rgb(0,0,1,0.3),
		           plot.bathy = T,plot.boundries = T,contour=list(recCont.poly,cont.data),
		           title="GBa 2014 recruit scallops (85 - 95 mm shell height)",dec.deg = F)
		# Add the comparative survey tows.
		points(slat~slon,survGBLive.dat,subset = year == 2014 & state=='live' & random == 3 , pch=24,bg='BLUE',cex=0.8)
		# Add the exploratory survey tows
		points(slat~slon,survGBLive.dat,subset = year==2014 & state =='live' & random %in% c(2,4,5), 
...
###################################  SUMMER SURVEY!!!!!!  SUMMER SURVEY!!!!!!  SUMMER SURVEY!!!!!!   #################				
		...
		ScallopMap(area="WOB",plot.bathy=T,bathy.source = "quick",boundries = "offshore", plot.strata = T,strata="offshore",
		           bound.color = T,plot.boundries = T,label.boundries = F,offshore.names=T,xlab="",ylab="", un= un.ID,
		           title=paste("Offshore scallop summer survey results (",yr,")",sep=""),
		           pw=pwd.ID,cex.mn=2,direct=direct,dec.deg = F)
		######################## SPECIAL PLOTS ######################## SPECIAL PLOTS  #################################
		######################## SPECIAL PLOTS ######################## SPECIAL PLOTS  #################################
    ...
    # Correctly specifying the years here really matters since the RS and CS are changing with time,
		...
		rec.title <- substitute(bold(paste("Recruit scallops (",b- a, " mm " , bank,"-",year,")",sep="")),
		                        list(a=as.character(CS[length(CS)]-6),b=as.character(RS[length(RS)]-5),
		                             year=as.character(yr),bank=bnk[2]))
		pre.rec.title <- substitute(bold(paste("Pre-recruit scallops (" ,""<b, " mm " , bank,"-",year,")",sep="")),
		                            list(b=as.character(RS[length(RS)]-5),year=as.character(yr),bank=bnk[2]))
		cf.title <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
		                       list(year=as.character(yr),bank=bnk[2]))
		cf.title.last <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
		                            list(year=as.character(yr-1),bank=bnk[2]))
		mc.title <- substitute(bold(paste("Meat count (" ,"">=m, " mm " , bank,"-",year,")",sep="")),
		...
		CF.ts.title <- substitute(bold(paste("Condition factor time series (",bank,")",sep="")),
		                          list(year=as.character(yr),bank=bnk[2]))
		clap.dis.title <- substitute(bold(paste("Clappers (% dead ","">=c, " mm ", bank,"-",year,")",sep="")),
		                             list(c=as.character(85),bank=bnk[2],year=as.character(yr)))
		clap.ts.title <- substitute(bold(paste("Clapper time series (",bank,")",sep="")),
		                            list(bank=bnk[2]))
		clap.per.ts.title <- substitute(bold(paste("Clapper time series (% dead ",bank,")",sep="")),
		                                list(bank=bnk[2]))
		clap.abund.ts.title <- substitute(bold(paste("Clapper abundance time series (",bank,")",sep="")),
		                                list(bank=bnk[2]))
		fully.rec.title.last <- substitute(bold(paste("Fully recruited scallops (" ,"">=a, " mm " , bank,"-",year,")",sep="")),
		                              list(a=as.character(CS[length(CS)]-5),year=as.character(yr-1),bank=bnk[2]))
		rec.title.last <- substitute(bold(paste("Recruit scallops (",b- a, " mm " , bank,"-",year,")",sep="")),
		                        list(a=as.character(CS[length(CS)]-6),b=as.character(RS[length(RS)]-5),
		                             year=as.character(yr-1),bank=bnk[2]))
		pre.rec.title.last <- substitute(bold(paste("Pre-recruit scallops (" ,""<b, " mm " , bank,"-",year,")",sep="")),
		                            list(b=as.character(RS[length(RS)]-5),year=as.character(yr-1),bank=bnk[2]))
		
		## Set up plot titles
		
		# Subset the seed-boxes for GBa
		GBa.boxes <- as.PolySet(subset(seedboxes,Bank==bnk[2] & Open >= paste(yr,"-01-01",sep="")), projection = "LL")
		GBa.boxes.last <- as.PolySet(subset(seedboxes,Bank==bnk[2] & Open >= paste(yr-1,"-01-01",sep="")), projection = "LL")
		...
		GBaDis<-dist.coef(c(1:200),path=paste(direct,"Data/Tow_tracks/",yr,"/Summer/GBa/",sep=""),
		                  w=c(1:10,9:1),rule=8,smooth=T,plt=F,meh=1000)
		...
		# summer and don't want to re-run the spring mw data. 
		...
		# DK Note I really need to make this code better...
		...
		#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
		#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
		GB.summer.SpatHtWt.fit<-shwt.lme(GB15.mwdm,random.effect='tow',b.par=3)
		GB.summer.SpatHtWt.last.fit<-shwt.lme(GB14.mwdm,random.effect='tow',b.par=3)
		#GB.spring.SpatHtWt.fit<-shwt.lme(GBMay.mw,random.effect='tow',b.par=3)
		# If feeling up to it compare the models across a few years.
		#GB.summer.SpatHtWt.fit.2014<-shwt.lme(GB14.mwdm,random.effect='tow',b.par=3)
		#GB.summer.SpatHtWt.fit.2013<-shwt.lme(GB13.mwdm,random.effect='tow',b.par=3)
		#GB.summer.SpatHtWt.fit.2013$summary
		#GB.summer.SpatHtWt.fit.2014$summary
		#GB.summer.SpatHtWt.fit$summary
		...
		# We currently predict on the mean depth,lat,lon from 2005-2014.
		# DO NOT CHANGE THIS UNLESS YOU KNOW WHAT YOU ARE DOING, 100% for sure!!!!
		pred.loc.GB.summer <- NULL
		pred.loc.GB.summer[["depth"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$depth,na.rm=T)
		pred.loc.GB.summer[["lat"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$lat,na.rm=T)
		pred.loc.GB.summer[["lon"]] <- mean(subset(GBmw.dat, year >=2005 & year <2015 & month %in% c(8,"08"))$lon,na.rm=T)
		
		GBcfData <- condFac(na.omit(subset(GBmw.dat,month%in% c(8,"08"))),survGB.dat,model.type='gam_f',
		                    pred.loc = pred.loc.GB.summer)
		...
		#Source30  This is where the GBcfData.R object is created, each year it will be updated 
		# DK Note:  In 2015 this gave a warning the dump may have been incomplete so care with it!.
		dump('GBcfData',file = paste(direct,"Data/Survey_data/",yr,"/Survey_summary_output/GBcfData.R",sep=""))
    ...
    strat.chk.GBa <- subset(survGBa.dat,year==yr & state=='live'& random==1)
		mis.match.GBa <- strat.chk.GBa[strat.chk.GBa$stratum != strat.chk.GBa$new.stratum,
		                           c("year","cruise","bank","tow","stratum","new.stratum")]
		...
		write.table(survGBa.dat,paste(direct,
          "data/Survey_data/",yr,"/Summer/GBa/GBaSurvey",min(years),"-",yr,".csv",sep=""),sep=',',row.names=F)
		
		
		
		# MEAT WEIGHT for GBa from 2011-2014, stitch them together. Drop the stratum column from 2011-2012.
		GBa11.mw<-subset(Summersurv2011$MWs[,-grep("stratum",names(Summersurv2011$MWs))],bank %in% bnk[2])
		GBa12.mw<-na.omit(subset(Summersurv2012$MWs[,-grep("stratum",names(Summersurv2012$MWs))],bank %in% bnk[2]))
		GBa13.mw<-subset(SurvDB$MWs,bank %in% bnk[2] & cruise =="TE16")
		GBa14.mw<-subset(SurvDB$MWs,bank %in% bnk[2] & cruise =="TE18")
		GBa15.mw<-subset(SurvDB$MWs,bank %in% bnk[2] & cruise =="LE02")
		GBaAug.mw<-rbind(GBa11.mw,GBa12.mw,GBa13.mw,GBa14.mw,GBa15.mw)
		GBaAug.mw$month<-8
		# Convert Shell height to decimeters for 2015
		GBa15.mwdm <- GBa15.mw
		GBa15.mwdm$sh<-GBa15.mw$sh/100
		GBa14.mwdm <- GBa14.mw
		GBa14.mwdm$sh<-GBa14.mw$sh/100
		# MEAT WEIGHT DATA - hydration sampling, it contains data from 1983-2007
		GBa.wgt.dat<-subset(MW.dat,bank %in% bnk[2])
		GBa.wgt.dat$ID<-paste(GBa.wgt.dat$year,GBa.wgt.dat$tow,sep='.')
		# Combine the Spring Meat Weight data with the summer survey data
		GBa.mw<-merge(subset(GBaAug.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")), 
		             subset(GBMay.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),all=T)
		# And the hydration sampling data with the more recent data.
		GBa.mw.dat<-merge(subset(GBa.wgt.dat,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),
		                subset(GBa.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),all=T)
		
		#Write14 Output the meat weight data
		write.csv(GBa.mw.dat,paste(direct,"data/Survey_data/",yr,"/Summer/GBa/GBamwData.csv",sep=""),row.names=F)
		
		# MODEL - This is the meat weight Shell height realationship.  
		#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
		#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
		GBa.summer.SpatHtWt.fit<-shwt.lme(GBa15.mwdm,random.effect='tow',b.par=3)
		GBa.summer.SpatHtWt.last.fit<-shwt.lme(GBa14.mwdm,random.effect='tow',b.par=3)
		...
		# We currently predict on the mean depth,lat,lon from 2005-2014.
		# DO NOT CHANGE THIS UNLESS YOU KNOW WHAT YOU ARE DOING, 100% for sure!!!!
		GBacfData <- condFac(na.omit(subset(GBa.mw.dat,month %in% c(8,"08"))),survGBa.dat,
		                     model.type='gam_f',pred.loc = pred.loc.GB.summer)
		
		# Output the predictions and give a new ID
		survGBa.dat<-GBacfData$pred.dat
		survGBa.dat$ID<-paste(survGBa.dat$year,survGBa.dat$tow,sep='.')
		# Pull out the ID and condition factor
		tmp.dat<-subset(GBacfData$CF.data,select=c("ID","CF"))
		# Rename CF to CFh
		names(tmp.dat)[2]<-"CFh"
		# merge the two data sets, keeping all x values
		survGBa.dat<-merge(survGBa.dat,tmp.dat,all.x=T)
		# Replace any NA's in CFh with the original Condition Factor.
		survGBa.dat$CFh[is.na(survGBa.dat$CFh)]<-survGBa.dat$CF[is.na(survGBa.dat$CFh)]
		
    # Put GBa summer together with May data.
		CF.GBa.data<-merge(GBacfData$CFyrs,GBcfDataMay$CFyrs[,-(2:4)],all=T)
		# Add a year column which offsets the year slightly (for plotting purposes)
		CF.GBa.data$y2<-CF.GBa.data$year-0.25
		CF.GBa.data	<- merge(CF.GBa.data,data.frame(year=yr+1),all=T)
		# 2015 should be removed as it is not a 'real' survey, just 6 tows quickly done on C1 seedbox.
		CF.GBa.data$CFMay[CF.GBa.data$year == 2015] <- NA
		...
		# A temporary mistake on these data needs cleaned up, won't be necessary forever!
		survGBa.dat$random[survGBa.dat$year == 2014 & survGBa.dat$tow >=900] <- 2
		...
		dump(c('survey.obj','clap.obj'),
		     file = paste(direct,"Data/Survey_data/",yr,"/Survey_summary_output/GBasurveyObj.R",sep=""))
		# If we want to skip all the processing we could just call in the survey object like so...
		#Source31 source(paste(direct,"Assessment_fns/Data/Survey_data/",yr,"/Survey_summary_output/surveyObj.R",sep="")) 
		
		# Now I'd like this output so I have some of the annual numbers for Survey Summary in a nice formate
		SS.summary.GBa <- survey.obj[[1]]
		SS.summary.GBa$bank <- "GBa"
		names(SS.summary.GBa) <- c("year","n","FR.BM","CV.FR.BM","R.BM","CV.R.BM","Pre.BM","CV.Pre.BM",
		                           "FR_N", "CV.FR.N",  "R.N","CV.R.N","Pre.N", "CV.Pre.N",
		                           "MW.mean","SH.mean","SH.k","MW.k","CF","FR.clap","R.clap","bank")
		# I want to get this into per tow 
		SS.summary.GBa[names(SS.summary.GBa) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] <-
		  SS.summary.GBa[names(SS.summary.GBa) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] / 
		  sum(GBa.surv.info$towable_area) * 10^6 
		write.csv(SS.summary.GBa,
		          file = paste(direct,"Data/Survey_data/",yr,"/Summer/",bnk[2],"/",bnk[2],"_annual_summary",yr,".csv",sep=""),
		          row.names = F)
		# Same for the SHF data.
		SHF.summary.GBa <- cbind(survey.obj[[1]]$year,survey.obj[[2]]$n.yst)
		write.csv(SHF.summary.GBa,
		          file = paste(direct,"Data/Survey_data/",yr,"/Summer/",bnk[2],"/",bnk[2],"_annual_SHF_summary",yr,".csv",sep=""),
		          row.names = F)
		...
		survGBaClap.dat$clap.prop.Rcom<-(survGBaClap.dat$com + survGBaClap.dat$rec)  / 
		                         (survGBaLive.dat$rec+survGBaLive.dat$com+survGBaClap.dat$com+ survGBaClap.dat$rec)*100

		survGBaClap.dat$clap.prop.Rcom[is.na(survGBaClap.dat$clap.prop.Rcom)]<-0
		
		
		
		# Calculate the CF and meat counts
		# Calculate the CF and meat counts
		CF.GBa.current <- subset(GBacfData$CF.data,year==yr,c('ID','lon','lat','CF',"tow"))
		CF.GBa.current <- merge(subset(CF.GBa.current,select=c("CF","tow")),	
		                     subset(survGBaLive.dat, year == yr),by="tow")
		CF.GBa.current$meat.count <- 0.5/(CF.GBa.current$com.bm/CF.GBa.current$com)
		CF.GBa.last <- subset(GBacfData$CF.data,year==yr-1,c('ID','lon','lat','CF',"tow"))
		
		CF.GBa.last <- merge(subset(CF.GBa.last,select=c("CF","tow")),	
		                        subset(survGBaLive.dat, year == yr-1),by="tow")
		CF.GBa.last$meat.count <- 0.5/(CF.GBa.last$com.bm/CF.GBa.last$com)
		
		
		############## END GBa Specific Results ############## END GBa Specific Results ################ END GBa Specific Results
		############## END GBa Specific Results ############## END GBa Specific Results ################ END GBa Specific Results
		subset(survGBaLive.dat, year == yr,select=c("tow","CF","com","com.bm"))
		...
		paste("Strata reassigned",
		      with(subset(survGBb.dat,year==yr&state=='live'&random==1),
		           round((1-sum(stratum==new.stratum,na.rm=T)/length(stratum))*100)),'%')
		
     strat.chk.GBb <- subset(survGBb.dat,year==yr & state=='live'& random==1)
     mis.match.GBb <- strat.chk.GBb[strat.chk.GBb$stratum != strat.chk.GBb$new.stratum,
                                c("year","cruise","bank","tow","stratum","new.stratum")]

     # Now as a one off I can output all of the re-assigned tows to Alan to see what's happening
     #reassigned.tows <- rbind(mis.match.BBn,mis.match.GBa,mis.match.GBb)
     #write.csv(reassigned.tows,
     #           file=paste(direct,"Data/Survey_data/2015/Survey_summary_output/Data_checks/reassigned_tows.csv",sep=""))
    # surv.by.tow calculates number or biomass of pre, rec and com size scallops in each tow
		...
		# MEAT WEIGHT for GBb from 2011-2014, stitch them together. Drop the stratum column from 2011-2012.
		GBb11.mw<-subset(Summersurv2011$MWs[,-grep("stratum",names(Summersurv2011$MWs))],bank %in% bnk[3])
		GBb12.mw<-na.omit(subset(Summersurv2012$MWs[,-grep("stratum",names(Summersurv2012$MWs))],bank %in% bnk[3]))
		GBb13.mw<-subset(SurvDB$MWs,bank %in% bnk[3] & cruise =="TE16")
		GBb14.mw<-subset(SurvDB$MWs,bank %in% bnk[3] & cruise =="TE18")
		GBb15.mw<-subset(SurvDB$MWs,bank %in% bnk[3] & cruise =="LE02")
		GBbAug.mw<-rbind(GBb11.mw,GBb12.mw,GBb13.mw,GBb14.mw,GBb15.mw)
		GBbAug.mw$month<-8
		# Convert Shell height to decimeters for 2015
		GBb15.mwdm <- GBb15.mw
		GBb15.mwdm$sh<-GBb15.mw$sh/100
		GBb14.mwdm <- GBb14.mw
		GBb14.mwdm$sh<-GBb14.mw$sh/100
		
		# MEAT WEIGHT DATA - hydration sampling, it contains data from 1983-2007
		GBb.wgt.dat<-subset(MW.dat,bank %in% bnk[3])
		GBb.wgt.dat$ID<-paste(GBb.wgt.dat$year,GBb.wgt.dat$tow,sep='.')
		# Combine the Spring Meat Weight data with the summer survey data
		GBb.mw<-merge(subset(GBbAug.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")), 
		              subset(GBMay.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),all=T)
		# And the hydration sampling data with the more recent data.
		GBb.mw.dat<-merge(subset(GBb.wgt.dat,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),
		                  subset(GBb.mw,select=c("ID","year","month","lon","lat","depth","sh","wmw","tow")),all=T)
		
		#Write14 Output the meat weight data
		write.csv(GBb.mw.dat,paste(direct,"data/Survey_data/",yr,"/Summer/GBb/GBbmwData.csv",sep=""),row.names=F)
		
		# MODEL - This is the meat weight Shell height realationship.  
		#MEAT WEIGHT SHELL HEIGHT RELATIONSHIP 2014 
		#Source11 source("fn/shwt.lme.r") note thtat the exponent is set as a parameter here b=3
		GBb.summer.SpatHtWt.fit<-shwt.lme(GBb15.mwdm,random.effect='tow',b.par=3)
		...
		# DK Note: And this was done incorrectly for GB in survey summary forever which messes up
		# everything that uses GBcfData, which includes...all CF plots, MC plots, 
		# We currently predict on the mean depth,lat,lon from 2005-2014 surveys
		# DO NOT CHANGE THIS UNLESS YOU KNOW WHAT YOU ARE DOING, 100% for sure!!!!
		GBbcfData <- condFac(na.omit(subset(GBb.mw.dat,month %in% c(8,"08"))),survGBb.dat,model.type='gam_f')
		# Output the predictions and give a new ID
		survGBb.dat<-GBbcfData$pred.dat
		survGBb.dat$ID<-paste(survGBb.dat$year,survGBb.dat$tow,sep='.')
		# Pull out the ID and condition factor
		tmp.dat<-subset(GBbcfData$CF.data,select=c("ID","CF"))
		# Rename CF to CFh
		names(tmp.dat)[2]<-"CFh"
		# merge the two data sets, keeping all x values
		survGBb.dat<-merge(survGBb.dat,tmp.dat,all.x=T)
		# Replace any NA's in CFh with the original Condition Factor.
		survGBb.dat$CFh[is.na(survGBb.dat$CFh)]<-survGBb.dat$CF[is.na(survGBb.dat$CFh)]
		
		# Put GBa summer together with May data.
		CF.GBb.data<-merge(GBbcfData$CFyrs,GBbcfData$CFyrs,all=T)
		# Add a year column which offsets the year slightly (for plotting purposes)
		CF.GBb.data$y2<-CF.GBb.data$year-0.25
		CF.GBb.data	<- merge(CF.GBb.data,data.frame(year=yr+1),all=T)
		...
		write.table(survGBb.dat,paste(direct,
		                   "data/Survey_data/",yr,"/Summer/GBb/GBbSurvey",min(years),"-",yr,".csv",sep=""),sep=',',row.names=F)
		...
		survGBbClap.dat$clap.propCom<-survGBbClap.dat$com/(survGBbLive.dat$com+survGBbClap.dat$com)*100
		survGBbClap.dat$clap.propCom[is.na(survGBbClap.dat$clap.propCom)]<-0
		survGBbClap.dat$clap.propRec<-survGBbClap.dat$rec/(survGBbLive.dat$rec+survGBbClap.dat$rec)*100
		survGBbClap.dat$clap.propRec[is.na(survGBbClap.dat$clap.propRec)]<-0
		survGBbClap.dat$clap.propPre<-survGBbClap.dat$pre/(survGBbLive.dat$pre+survGBbClap.dat$pre)*100
		survGBbClap.dat$clap.propPre[is.na(survGBbClap.dat$clap.propPre)]<-0
		survGBbClap.dat$clap.prop[is.na(survGBbClap.dat$clap.prop)]<-0
		survGBbClap.dat$clap.prop.Rcom<-(survGBbClap.dat$com + survGBbClap.dat$rec)  / 
		  (survGBbLive.dat$rec+survGBbLive.dat$com+survGBbClap.dat$com+ survGBbClap.dat$rec)*100
		
		survGBbClap.dat$clap.prop.Rcom[is.na(survGBbClap.dat$clap.prop.Rcom)]<-0
		...
		# Now I'd like this output so I have some of the annual numbers for Survey Summary in a nice formate
		SS.summary.GBb <- surveyGBb.obj[[1]]
		SS.summary.GBb$bank <- "GBb"
		names(SS.summary.GBb) <- c("year","n","FR.BM","CV.FR.BM","R.BM","CV.R.BM","Pre.BM","CV.Pre.BM",
		                           "FR_N", "CV.FR.N",  "R.N","CV.R.N","Pre.N", "CV.Pre.N",
		                           "MW.mean","SH.mean","SH.k","MW.k","CF","FR.clap","R.clap","bank")
		# I want to get this into per tow 
		SS.summary.GBb[names(SS.summary.GBb) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] <-
		  SS.summary.GBb[names(SS.summary.GBb) %in% c("FR.BM","R.BM","Pre.BM","FR_N",  "R.N","Pre.N")] / 
		  sum(GBb.surv.info$towable_area) * 10^6 
		write.csv(SS.summary.GBb,
		          file = paste(direct,"Data/Survey_data/",yr,"/Summer/",bnk[3],"/",bnk[3],"_annual_summary",yr,".csv",sep=""),
		          row.names = F)
		# Same for the SHF data.
		SHF.summary.GBb <- cbind(surveyGBb.obj[[1]]$year,surveyGBb.obj[[2]]$n.yst)
		write.csv(SHF.summary.GBb,
		          file = paste(direct,"Data/Survey_data/",yr,"/Summer/",bnk[3],"/",bnk[3],"_annual_SHF_summary",yr,".csv",sep=""),
		          row.names = F)
		
		
		# Calculate the CF and meat counts
		CF.GBb.current <- subset(GBbcfData$CF.data,year==yr,c('ID','lon','lat','CF',"tow"))
		CF.GBb.current <- merge(subset(CF.GBb.current,select=c("CF","tow")),	
		                     subset(survGBbLive.dat, year == yr),by="tow")
		CF.GBb.current$meat.count <- 0.5/(CF.GBb.current$com.bm/CF.GBb.current$com)
		CF.GBb.last <- subset(GBbcfData$CF.data,year==yr-1,c('ID','lon','lat','CF',"tow"))
		CF.GBb.last <- merge(subset(CF.GBb.last,select=c("CF","tow")),	
		                        subset(survGBbLive.dat, year == yr-1),by="tow")
		CF.GBb.last$meat.count <- 0.5/(CF.GBb.last$com.bm/CF.GBb.last$com)
		...
		# DK note I don't quite love this yet as we are using the start point of the tows
		# it might be better to use the mid-point of the tows or maybe even the tow tracks
		# to determine what tows are inside/outside the boxes?
	  ...
	  #survGBMay.dat<-read.csv(paste(direct,"data/Survey_data/",yr,"/Spring/GB/GBMaySurvey1984-",yr,".csv",sep=""),
		# stringsAsFactors = F)
		...
		GBa.surv.0715<-merge(survGBAug.dat,subset(survGBMay.dat,year>2006),all=T)
		# Make some new columns
		GBa.surv.0715$EID<-1:nrow(GBa.surv.0715)
		GBa.surv.0715$X<-GBa.surv.0715$slon
		GBa.surv.0715$Y<-GBa.surv.0715$slat
		# This will change each year and will effect what is plotted below.
		C1.surv <- findPolys(GBa.surv.0715,subset(GBa.boxes,ID=='C1-012014'))
		#SB.surv <- findPolys(GBa.surv.0715,subset(GBa.boxes,ID=='Seed box (2012 modified)'))
		#AB.surv <- findPolys(GBa.surv.0715,subset(GBa.boxes,ID=='A/B line closure'))
		# Survey results from each seedbox for 2011 to 2014
		#Source15 source("fn/simple.surv.r")
		C1line.obj<-simple.surv(subset(GBa.surv.0715,EID %in% C1.surv$EID),years=seq(2012.7,2015.7,0.5))
		#SBline.obj<-simple.surv(subset(GBa.surv.0715,EID%in%SB.surv$EID),years=seq(2011.7,2014.7,1))
		#ABline.obj<-simple.surv(subset(GBa.surv.0715,EID%in%AB.surv$EID),years=seq(2011.7,2014.7,1))
		
		#################################################################################################################
		# END SEEDBOXES # END SEEDBOXES # END SEEDBOXES # END SEEDBOXES # END SEEDBOXES # END SEEDBOXES # END SEEDBOXES # 
		###################################################################################################################
		...
		con.dat<-rbind(subset(CF.GBa.current,select = c('tow','lon','lat','meat.count')),
		               subset(CF.GBb.current,select = c('tow','lon','lat','meat.count')))
		...
		# Bottom temp  
		temp.contours<-contour.gen(na.omit(subset(temps,year==yr & bank %in% bnk[c(2,3)],c('tow','slon','slat','mean.temp'))),
		...
		con.dat<-rbind(subset(survGBaClap.dat,year==yr,c('tow','lon','lat','clap.prop.Rcom')),
		               subset(survGBbClap.dat,year==yr,c('tow','lon','lat','clap.prop.Rcom')))
		...
		#### Spatial data from 2014 
		con.dat.last<-rbind(subset(survGBaLive.dat,year==yr-1,c('tow','lon','lat','pre')),
		               subset(survGBbLive.dat,year==yr-1,c('tow','lon','lat','pre')))
		pre.contours.last<-contour.gen(con.dat.last,ticks='define',nstrata=14,str.min=0,interp.method='gstat',
		                          points=T,blank=T,res=0.01,key='log.cont',title='pre-recruits (< 85 mm shell height)',
		                          color.fun=tim.colors,id.par=3.5,units='#/tow',plot=F,subscale=0.1,blank.dist=0.08,direct = direct)
		# Recruits (85 - 95 mm shell height)
		con.dat.last<-rbind(subset(survGBaLive.dat,year==yr-1,c('tow','lon','lat','rec')),
		               subset(survGBbLive.dat,year==yr-1,c('tow','lon','lat','rec')))
		rec.contours.last<-contour.gen(con.dat.last,ticks='define',nstrata=14,str.min=0,interp.method='gstat',points=T,blank=T,
		                          res=0.01,key='log.cont',title='recruits(85 - 95 mm shell height)',color.fun=tim.colors,id.par=3.5,
		                          units='#/tow',plot=F,subscale=0.1,blank.dist=0.08,direct = direct)
		# Fully Recruited (>= 95 mm shell height)
		con.dat.last<-rbind(subset(survGBaLive.dat,year==yr-1,c('tow','lon','lat','com')),
		               subset(survGBbLive.dat,year==yr-1,c('tow','lon','lat','com')))
		com.contours.last<-contour.gen(con.dat.last,ticks='define',nstrata=14,str.min=0,interp.method='gstat',points=T,blank=T,
		                          res=0.01,key='log.cont',title='fully recruited (>= 95 mm shell height)',
		                          color.fun=tim.colors,id.par=5,units='#/tow',plot=F,subscale=0.1,blank.dist=0.08,direct = direct)
		# Condition Factor 2014 note I do not specify blank distance, allow the blank.blank function to determine optimal given data.
		cf.contours.last<-contour.gen(subset(GBcfData$CF.data,year==yr-1,c('ID','lon','lat','CF')),interp.method='gstat',
		                         ticks='define',nstrata=7,str.min=0,place=2,id.par=3.5,units="scallops/500g",direct = direct,
		                         key='strata',blank=T,plot=F,subset.eff=0,subscale=0.25,subset.poly='square')
		...
		#pdf(file = paste(direct,"Data/Survey_data/",yr,"/Exploratory_figures/GB_survey_summary.pdf",sep=""),
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/survey_strata.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(bnk[1],poly.lst=list(GBa.survey.detail.poly,GBa.surv.info),direct = direct,ylab="",xlab="",
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), cex.mn=2,
		...
		legend(-66,41.45,legend = as.numeric(with(subset(survGBaLive.dat,year==yr & random==1),tapply(tow,new.stratum,length))),
		       fill=c(GBa.surv.info$col),border=c(rep('black',length(GBa.surv.info$PName))),
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/abundance_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/biomass_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/shell_height_freqency.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,survey.obj,from='surv',yr=2008:yr,col1='grey80',type='sh',col2=1,col3=1,xl=c(0,200),adj=0.05,
		        rel=F,recline=c(RS[length(years)]-5,CS[length(years)]-5),ymax=310,mean.line=F,titl=SHF.title,cex.mn = 3,
		        add.title = T,rows=8,sample.size = survey.obj[[1]]$n[survey.obj[[1]]$year %in% 2008:yr])	
		dev.off()
		############
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Pre-recruits.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1],contour=list(preCont.poly,precont.data),direct = direct, title=pre.rec.title,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Recruits.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		############
				...
				# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
				png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Fully_recruited.png",sep=""),units="in",width = 11, 
				    height = 8.5,res=420,bg = "transparent")
				par(mfrow=c(1,1))
				#windows(11,8.5)
				ScallopMap(area=bnk[1],contour=list(comCont.poly,comcont.data),direct = direct,title=fully.rec.title,
				           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",cex.mn=2,
				           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg=F)
				# Add the regular survey tows.
				points(lat~lon,survGBaLive.dat,subset=year==yr & state=='live'& random==1,pch=20,bg='black',cex=0.8)
				# Add the exploratory survey tows
				...
				#windows(15,8)
				png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/MWSH_and_CF_ts.png",sep=""),units="in",width = 13,
				    height = 8.5,res=420,bg = "transparent")
				par(mfrow=c(1,2))
				shwt.plt1(GBa.summer.SpatHtWt.fit,lw=3,ht=10,wd=12,cx=1.5,titl = MWSH.title,cex.mn = 2,las=1)
				# If any outliers identify them and send this information off to be checked.
				#identify(GB.summer.SpatHtWt.fit$data$sh,GB.summer.SpatHtWt.fit$data$wmw)
				#outliers <- GB.summer.SpatHtWt.fit$data[c(401,1090,2444,2657,2589,554,2680,1071,1439,
				#                                   1785,1893,2691,2695,1859,620,3288,2634,3546,3246,3933,1992,2018,4013,3562,273),]
				#write.csv(outliers,
				#         file=paste(direct,"Data/Survey_data/2015/Survey_summary_output/Data_checks/MWSH_outliers.csv",sep=""))
				
				############
				#Source16 source("fn/stdts.plt.R") Condition Factor time series slide 14 in 2014 presentation 
				
				stdts.plt(CF.GBa.data[-(1:3),],x=c('year','y2'),y=c('CF','CFMay'),pch=16,ylab=cf.lab,las=1,col=c("blue","red"),
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/CF_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		# Grab the exploratory/regular tows from the condition factor data.
		tmp <- subset(GBcfData$CF.data,year==yr)
		reg.tw <- subset(survGBaLive.dat,year==yr & random==1)$tow
		exp.tw <- subset(survGBaLive.dat,year==yr & random==2)$tow
		...
		#Source16 source("fn/stdts.plt.R")
				# DK Note:  How is average meat weight staying approximately the same when CF and SH are dropping?
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/SH_MW_CF_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		...
		stdts.plt(subset(GBacfData$CFyrs,year %in% 1995:2015),y="CF",pch=17,lty=1,las=1, ylab=cf.lab,
		          mean.line=T,graphic='none',xlab='',labcx=1.2,axis.cx=1.2)
		...
		title(paste("Shell height, Condition factor, Meat weight (",bnk[2],")",sep=""), cex.main=3,outer=T)
		dev.off()
		############
		...
		range(mc.contours$image.dat$z)
		lvls=c(seq(5,30,5),33)
		div=2
		CL <- contourLines(mc.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/MC_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1],contour=list(MCcont.poly,MCcont.data),direct = direct, title=mc.title,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg = F)
		# Grab the exploratory/regular tows from the condition factor data.
		tmp <- subset(GBcfData$CF.data,year==yr)
		...
		############bookmark
		#Source26 Clapper abundance plot for Georges B, new slide in 2015 presentation source("fn/Clap.plt.R")
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Clapper_abund_ts.png",sep=""),
		    units="in",width = 8.5, height = 11,res=420,bg = "transparent")
		survey.ts(clap.obj[[1]],1991:yr,Bank=bnk[2],pdf=F, RS=RS[length(RS)]-5, CS=CS[length(CS)]-5,
		          titl = clap.abund.ts.title,add.title=T, cx.mn=3,areas=GBa.strata.areas$towable_area,
		          ys=.8,ht=7,wd=10,clr='blue',se=T,pch=16, plots=c("pre",'rec','com'))
		dev.off()
		############
		
		############
		#Source26 Clapper plot for Georges A, this replaces slide 22 in 2014 presentation source("fn/Clap.plt.R")
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Clapper_per_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		Clap3.plt(subset(survGBaClap.dat,random==T),years=years,yl=c(0,17),
		         add.title = T,titl=clap.per.ts.title,cex.mn=3,lab.cx=1.2,axis.cx=1,CS=CS[length(years)]-5,RS=RS[length(years)]-5)
		dev.off()
		############
		...
		range(clap.contours$image.dat$z)
		lvls=c(1,2,5,10,15,20,30)
		CL <- contourLines(clap.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		clapCont.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
		cont.data<- data.frame(PID=1:length(lvls),
		                       col=brewer.pal(length(lvls),clap.col),border=NA,stringsAsFactors = F)
		#Clappers in Georges A slide 23
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r").
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Clapper_spatial.png",sep=""),units="in",width = 11, 
		    height = 8.5, res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk,contour=list(clapCont.poly,cont.data),direct = direct, title=clap.dis.title,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGBaLive.dat,subset=year==yr & state=='live'& random==1,pch=20,bg='black',cex=0.8)
		...
		#############  C1-012014 Plots ################
				...
				if(max(rec.contours$image.dat$z)<lvls2[1])
				{
				  ...
				  CL <- contourLines(rec.contours$image.dat,levels=lvls)
				  CP <- convCP(CL)
				  recCont.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
				  reccont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
				} # end if(max(com.contours$image.dat$z)<lvls2[1])
				...
				if(max(com.contours$image.dat$z)>=lvls2[1])
				{
				  ...
				  CL <- contourLines(com.contours$image.dat,levels=lvls2)
				  CP <- convCP(CL)
				  comCont2.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
				  comCont2.poly$PID<-comCont2.poly$PID+length(lvls1)
				  comCont.poly<-rbind(comCont1.poly,comCont2.poly)
				  lvls<-c(lvls1,lvls2)
				  comcont2.data<- data.frame(PID=(length(lvls1)+1):(length(lvls1)+length(lvls2)),
				...
				if(max(com.contours$image.dat$z)<lvls2[1])
				{
				  ...
				  CL <- contourLines(com.contours$image.dat,levels=lvls)
				  CP <- convCP(CL)
				  comCont.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
				  comcont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
				} # end if(max(com.contours$image.dat$z)<lvls2[1])
				# The pre-recruits
				# If counts are high enough to need lvls2 we use it, if not we just use lvls1.
				...
				if(max(pre.contours$image.dat$z)<lvls2[1])
				{
				  ...
				  CL <- contourLines(pre.contours$image.dat,levels=lvls)
				  CP <- convCP(CL)
				  preCont.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
				  precont.data<- data.frame(PID=1:length(lvls),col=brewer.pal(length(lvls),N.col),border=NA,stringsAsFactors = F) 
				} # end if(max(com.contours$image.dat$z)<lvls2[1])
				...
				#  source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
				# Retain this window size as the plot goes bad with other sized windows.
				#windows(15,8.5)
				png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/C1-seedbox_spatial.png",sep=""),units="in",width = 11, 
				    height = 8.5,res=420,bg = "transparent")
				par(mfrow=c(2,2),omi=c(0.2,0.2,0.2,0.5))
				# Fully recruited
				ScallopMap(ylim=c(42.05,42.14),xlim=c(-67.14,-66.95),bathy.source="usgs",isobath = c(seq(40,140,by=20)), 
				           plot.bathy=T,plot.boundries = T,
				           bathcol=rgb(0,0,1,0.3),contour=list(comCont.poly,comcont.data),xlab="",ylab="",cex.mn=2,
				           title= "", direct=direct,dec.deg = F)
				title(substitute(bold(paste("Fully recruited scallops (","">=a, " mm " ,")",sep="")),
				                 list(a=as.character(CS[length(CS)]-5),year=as.character(yr))),cex.main=1.3,line=1.1)
				addLines(GBextrasDis[[2]],col='blue',lwd=2)
				addLines(GBaDis[[2]],col='blue',lwd=2)
				addPolys(GBa.boxes,lty=2,lwd=2)
				
				# Add the regular survey tows in Seedbox 2012
				points(slat~slon,survGBaLive.dat,subset=year==yr & state=='live'& random==1,pch=20,bg='black',cex=0.8)
				# Add the exploratory survey tows
				points(slat~slon,survGBaLive.dat,subset=year==yr&state =='live' & random==2,pch=24,bg="darkorange",cex=0.8)
				# Recruits
				ScallopMap(ylim=c(42.05,42.14),xlim=c(-67.14,-66.95),plot.bathy=T,plot.boundries = T,bathy.source = "usgs",
				           contour=list(recCont.poly,reccont.data),cex.mn=2,xlab="",ylab="",isobath = c(seq(40,140,by=20)), 
				           title= "", direct=direct,dec.deg = F)
				title(substitute(bold(paste("Recruit scallops (",a,"-",b, " mm)",sep="")),
				                 list(a = as.character(RS[length(RS)]-5),b=as.character(CS[length(CS)]-6))),cex.main=1.3,line=1.1)
				...
				ScallopMap(ylim=c(42.05,42.14),xlim=c(-67.14,-66.95),bathy.source="usgs",isobath = c(seq(40,140,by=20)), 
				           plot.bathy=T,plot.boundries = T,
				           contour=list(preCont.poly,precont.data),cex.mn=2,xlab="",ylab="",
				           title= "", direct=direct,dec.deg = F)
	
				title(substitute(bold(paste("Pre-recruit scallops (",""<a, " mm)",sep="")),
				                 list(a=as.character(RS[length(RS)]-5),year=as.character(yr))),cex.main=1.3,line =1.1)
				addLines(GBextrasDis[[2]],col='blue',lwd=2)
				addLines(GBaDis[[2]],col='blue',lwd=2)
        addPolys(GBa.boxes,lty=2,lwd=2)
				...
				legend("left",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
				                paste(lvls[length(lvls)],'+',sep='')),fill=c(precont.data$col),title=N.tow.lab,title.adj = 0.2,
				       border=c(rep('black',length(lvls))),pch=c(rep(NA,length(lvls))),
				       pt.bg = c(rep(NA,length(lvls))),inset=0.01,bty="n",cex=1.1)
				legend("topright",
				       legend = c(paste('exploratory (n =',
				                        length(unique(subset(survGBaLive.dat,year==yr & random==2 & (tow %in% C1$EID ))$tow)),
				                        ")",sep=""),
				                  paste('regular (n =',
				                        length(unique(subset(survGBaLive.dat,year==yr & random==1 & (tow %in% C1$EID ))$tow)),
				                        ")",sep="")),title="Tow type",
				       pt.bg = c("darkorange","black"),pch=c(24,20),bty="n",cex=1.1)		###########
				title(paste("Seedbox C1 (",bnk[2],"-",yr,")",sep=""),cex.main=2,outer=T,line=-1)
		...
		C1.t<-c("Aug 2012","May 2013","Aug 2013","May 2014","Aug 2014","May 2015","Aug 2015")
		#Source18 Georges A, C1-012014, slide 31, very cool! source("fn/shf.plt.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/C1-shell_height_freqency.png",sep=""),units="in",width = 8.5, 
			    height = 11,res=420,bg = "transparent")
		par(xpd=F)
				shf.plt(ps.dat,C1line.obj[[2]]$n.yst,from='special',yr=C1.t,col1='grey80',
		        col2=1,col3=1,xl=c(0,150),rel=F,rows=7,ht=12,sample.size=C1line.obj[[1]]$n,
		        recline=c(RS[length(RS)]-5,CS[length(CS)]-5),ymax=600,add.title = T,
		        titl = paste("Shell height frequency (Seedbox C1-",bnk[2],")",sep=""),cex.mn=3)
				dev.off()
		###########
		
  	############
	  # Plot for Bottom Temperature across GB, This is slide 21 from presentation and covers all of the bank, moving it to the end
	  # to make it a transition between GBa and GBb.
		range(temp.contours$image.dat$z,na.rm=T)
	  lvls=seq(4,14,1)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Temperature.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		############  PREVIOUS YEAR SPATIAL  PLOTS  PREVIOUS YEAR SPATIAL  PLOTS PREVIOUS YEAR SPATIAL  PLOTS ####
		############  PREVIOUS YEAR SPATIAL  PLOTS  PREVIOUS YEAR SPATIAL  PLOTS PREVIOUS YEAR SPATIAL  PLOTS ####
		############  PREVIOUS YEAR SPATIAL  PLOTS  PREVIOUS YEAR SPATIAL  PLOTS PREVIOUS YEAR SPATIAL  PLOTS ####
		...
		# This produces figure on slide 12 for 2014 pre recruts
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Pre-recruits",yr-1,".png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1],contour=list(preCont.poly,precont.data),direct = direct, title=pre.rec.title.last,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGBaLive.dat,subset=year==yr-1 & state=='live'& tow < 900,pch=20,bg='black',cex=0.8)
		# Add the exploratory survey tows
		points(lat~lon,survGBaLive.dat,subset=year==yr-1 & state =='live' & tow >= 900,pch=24,bg="darkorange",cex=0.8)
		# Add the legend
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                      paste(lvls[length(lvls)],'+',sep='')),fill=c(precont.data$col),title=N.tow.lab,
		       border="black",
		...
		legend("topright",
		       legend = c(paste('exploratory (n =',
		                        length(unique(subset(survGBaLive.dat,year==yr-1 & tow >= 900))$tow) ,
		                        ")",sep=""),
		                  paste('regular (n =',
		                        length(unique(subset(survGBaLive.dat,year==yr-1 & tow <900))$tow),
		                        ")",sep="")),title="Tow type",
		       pt.bg = c("darkorange","black"),pch=c(24,20),bg = "white",inset=0.01,box.col="white")
		# Add seedboxes
		addPolys(GBa.boxes.last,lty=2,lwd=2)
		dev.off()
		############
		...
		if(max(rec.contours.last$image.dat$z)>=lvls2[1])
		{
		  ...
		  reccont2.data<- data.frame(PID=(length(lvls1)+1):(length(lvls1)+length(lvls2)),
		                             col=rev(brewer.pal(8,X.lvl))[(1:length(lvls2)+1)],border=NA,stringsAsFactors = F)
		  reccont.data<-rbind(reccont1.data,reccont2.data)
		} # end if(max(rec.contours.last$image.dat$z)>=lvls2[1])
		if(max(rec.contours.last$image.dat$z)<lvls2[1])
		{
		  recCont.poly<-recCont1.poly
		  reccont.data<-reccont1.data
		...
		# GBa This is the recruit scallops, slide 10 of the powerpoint.
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Recruits",yr-1,".png",sep=""),
		    units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1], contour=list(recCont.poly,reccont.data),direct = direct, title=rec.title.last,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGBaLive.dat,subset=year==yr-1 & state=='live'& tow < 900,pch=20,bg='black',cex=0.8)
		...
		if(max(com.contours.last$image.dat$z)>=lvls2[1])
		{
		  ...
		  comcont2.data<- data.frame(PID=(length(lvls1)+1):(length(lvls1)+length(lvls2)),
		                             col=rev(brewer.pal(8,X.lvl))[(1:length(lvls2)+1)],border=NA,stringsAsFactors = F) 
		  comcont.data<-rbind(comcont1.data,comcont2.data)
		} # end if(max(com.contours.last$image.dat$z)>=lvls2[1])
		if(max(com.contours.last$image.dat$z)<lvls2[1])
		{
		  comCont.poly<-comCont1.poly
		  comcont.data<-comcont1.data
		...
		# GBa This is the fully recruited scallops, slide 8 of the powerpoint.
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[2],"/Fully_recruited",yr-1,".png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1],contour=list(comCont.poly,comcont.data),direct = direct,title=fully.rec.title.last,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",cex.mn=2,
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg=F)
		# Add the regular survey tows.
		points(lat~lon,survGBaLive.dat,subset=year==yr-1 & state=='live'& tow < 900,pch=20,bg='black',cex=0.8)
		...
		# DK Note:  This is not very similar to the 2014 plot, points are diferent/missing in places...
		# Mostly due to missing points in the seedboxes.
		range(cf.contours.last$image.dat$z)
		lvls=12:20
		CL <- contourLines(cf.contours.last$image.dat,levels=lvls)
		CP <- convCP(CL)
		cont.poly <- joinPolys(CP$PolySet,GBa.survey.bound.poly)
		...
		#Condition factor for Georges A, slide 16 in powerpoint
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,"Presentations/",yr-1,"/Survey_summary/",bnk[2],"/CF_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[1],contour=list(cont.poly,cont.data),direct = direct, title=cf.title.last,cex.mn=2,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,dec.deg = F)
		# Add the survey tows in GBa, somewhat messy way to get there...
		# Grab the exploratory/regular tows from the condition factor data.
		tmp <- subset(GBcfData$CF.data,year==yr-1)
		reg.tw <- subset(survGBaLive.dat,year==yr-1 & random==1)$tow
		exp.tw <- subset(survGBaLive.dat,year==yr-1 & random==2)$tow
		cf.reg <- subset(tmp, tow %in% reg.tw)
		cf.exp <- subset(tmp, tow %in% exp.tw)
		# Add the regular survey tows.
		points(lat~lon,cf.reg,pch=20,bg='black',cex=0.8)
		...
		############## END PREVIOUS YEAR SPATIAL  PLOTS END PREVIOUS YEAR SPATIAL  PLOTS #######################
		############## END PREVIOUS YEAR SPATIAL  PLOTS END PREVIOUS YEAR SPATIAL  PLOTS #######################
		############## END PREVIOUS YEAR SPATIAL  PLOTS END PREVIOUS YEAR SPATIAL  PLOTS #######################
		...
		cf.title <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
		                       list(year=as.character(yr),bank=bnk[3]))
		cf.title.last <- substitute(bold(paste("Condition factor (", bank,"-",year,")",sep="")),
		                       list(year=as.character(yr-1),bank=bnk[3]))
		mc.title <- substitute(bold(paste("Meat count (" ,"">=m, " mm " , bank,"-",year,")",sep="")),
		...
		CF.ts.title <- substitute(bold(paste("Condition factor time series (",bank,")",sep="")),
		                          list(year=as.character(yr),bank=bnk[3]))
		clap.dis.title <- substitute(bold(paste("Clappers (% dead ","">=c, " mm ", bank,"-",year,")",sep="")),
		                             list(c=as.character(85),bank=bnk[3],year=as.character(yr)))
		...
		clap.abund.ts.title <- substitute(bold(paste("Clapper abundance time series (",bank,")",sep="")),
		                                  list(bank=bnk[3]))
		fully.rec.title.last <- substitute(bold(paste("Fully recruited scallops (" ,"">=a, " mm " , bank,"-",year,")",sep="")),
		                              list(a=as.character(CS[length(CS)]-5),year=as.character(yr-1),bank=bnk[3]))
		rec.title.last <- substitute(bold(paste("Recruit scallops (",b- a, " mm " , bank,"-",year,")",sep="")),
		                  list(a=as.character(CS[length(CS)]-6),b=as.character(RS[length(RS)]-5),year=as.character(yr-1),bank=bnk[3]))
		pre.rec.title.last <- substitute(bold(paste("Pre-recruit scallops (" ,""<b, " mm " , bank,"-",year,")",sep="")),
		                            list(b=as.character(RS[length(RS)]-5),year=as.character(yr-1),bank=bnk[3]))
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/survey_strata.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/abundance_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/abundance_ts_rescaled.png",sep=""),units="in",width = 8.5,
		    height = 11,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		survey.ts(surveyGBb.obj[[1]],years,Bank=bnk[3],pdf=F, CS=CS[length(years)]-5,RS=RS[length(years)]-5,
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/biomass_ts.png",sep=""),units="in",width = 8.5, height = 11,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/shell_height_freqency.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,surveyGBb.obj,from='surv',yr=2008:yr,col1='grey80',type='sh',rows=8,
		        col2=1,col3=1,xl=c(0,200),rel=F,recline=c(RS[length(years)]-5,CS[length(years)]-5),
		        ymax=410,mean.line=F,adj=0.02,add.title = T,titl=SHF.title,cex.mn = 3,
		        sample.size= surveyGBb.obj[[1]]$n[surveyGBb.obj[[1]]$year %in% 2008:yr])	
		dev.off()
		################
		
		################
		# Shell height frequency histograms, same as previous but rescaled to put this new cohort into perspective, slide 39
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/shell_height_freqency_rescaled.png",sep=""),units="in", 
		    width = 8.5,height = 11,res=420,bg = "transparent")
		shf.plt(ps.dat,surveyGBb.obj,from='surv',yr=2008:yr,col1='grey80',type='sh', rows = 8,
		        col2=1,col3=1,xl=c(0,200),rel=F,recline=c(RS[length(years)]-5,CS[length(years)]-5),
		        ymax=4100,mean.line=F,adj=0.02,add.title = T,titl=SHF.title,cex.mn = 3,cx.axis=1.3,
		        sample.size= surveyGBb.obj[[1]]$n[surveyGBb.obj[[1]]$year %in% 2008:yr])
		dev.off()
		################
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Pre-recruits.png",sep=""),units="in",width = 11,
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Recruits.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Fully_recruited.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		#Source12 Meat Height Shell weight plot on Slide 13  source("fn/shwt.plt1.r") 
		#windows(15,8)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/MWSH_and_CF_ts.png",sep=""),units="in",width = 13,
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,2))
		shwt.plt1(GBb.summer.SpatHtWt.fit,lw=3,ht=10,wd=12,cx=1.5,titl = MWSH.title,cex.mn = 2,las=1)
		# If any outliers identify them and send this information off to be checked.
		#identify(GB.summer.SpatHtWt.fit$data$sh,GB.summer.SpatHtWt.fit$data$wmw)
		#outliers <- GB.summer.SpatHtWt.fit$data[c(401,1090,2444,2657,2589,554,2680,1071,1439,
		#                                         1785,1893,2691,2695,1859,620,3288,2634,3546,3246,3933,1992,2018,4013,3562,273),]
		...
		stdts.plt(CF.GBb.data[-(1:3),],x=c('year'),y=c('CF'),pch=16,ylab=cf.lab,las=1,col=c("blue"),
		          mean.line=T,graphic='none',xlab='Year',ylim=c(8,16),titl=CF.ts.title,cex.mn=2)
		#legend('bottomleft',c("August","May"),lty=1:2,pch=c(16,NA),bty='n',inset=0.02,col=c("blue","red"))		
		############
		dev.off()		
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/CF_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		...
		legend("topright",legend = c(paste('exploratory (n =',
		                                   ...
		                                   ")",sep=""),
		                             paste('regular (n =',
		                                   length(unique(subset(temp,EID %in% CF.B$EID)$EID)),
		                                   ")",sep="")),title="Tow type",
		       pt.bg = c("darkorange","black"),pch=c(24,20),bg = "white",inset=0.01,box.col="white")		############
		dev.off()
		############
		...
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/MC_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		# Add the legend
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                      paste(lvls[length(lvls)],'+',sep='')),fill=c(MCcont.data$col),title=mc.lab,title.adj =0.5,
		       border=c(rep('black',length(lvls))),pch=c(rep(NA,length(lvls))),
		       pt.bg = c(rep(NA,length(lvls))),inset=0.01,bty="n")
		legend("topright",legend = c(paste('exploratory (n =',
		                                   length(unique(subset(survGBbLive.dat,year==yr & random==2)$tow)),
		...
		############
		#Source26 Clapper abundance plot for Georges A, new slide in 2015 presentation source("fn/Clap.plt.R")
		#windows(11,8.5)
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Clapper_abund_ts.png",sep=""),
		    units="in",width = 8.5, height = 11,res=420,bg = "transparent")
		survey.ts(clapGBb.obj[[1]],1991:yr,Bank=bnk[3],pdf=F, RS=RS[length(RS)]-5, CS=CS[length(CS)]-5,
		          titl=clap.abund.ts.title,add.title = T, cx.mn=3,areas=GBb.strata.areas$towable_area,
		          ys=.8,ht=7,wd=10,clr='blue',se=T,pch=16, plots=c("pre",'rec','com'))
		dev.off()
		############
		...
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Clapper_per_ts.png",sep=""),units="in",width = 8.5, 
		    height = 11,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		Clap3.plt(subset(survGBbClap.dat,random==T),years=years,yl=c(0,50), RS=RS[length(RS)]-5, CS=CS[length(CS)]-5,
		         add.title = T,titl=clap.per.ts.title,cex.mn=3,lab.cx=1.2,axis.cx=1)
		dev.off()
		############
		
		############
		# Clappers in George B, slide 49 
		# The peak clapper numbers on GBb are less than 30% in 2015. (It is up to 36% in GBa at a maximum)
		lvls=c(1,2,5,10,15,20,30)
		CL <- contourLines(clap.contours$image.dat,levels=lvls)
		CP <- convCP(CL)
		clapCont.poly <- joinPolys(CP$PolySet,GBb.strata.bound.poly)
		#range(clap.contours$image.dat$z) # Make sure the range chosen is reasonable for the levels.
		cont.data<- data.frame(PID=1:length(lvls),
		                       col=brewer.pal(length(lvls),clap.col),border=NA,stringsAsFactors = F) 
		# Clappers in George B, slide 49 
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Clapper_spatial.png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		...
		legend("topright",legend = c(paste('exploratory (n =',
		                             ...
		                             paste('regular (n =',
		                                   ...
		                                   ")",sep="")),title="Tow type",
		       pt.bg = c("darkorange","black"),pch=c(24,20),bg = "white",inset=0.01,box.col="white")	
		dev.off() # close the pdf device if I opened it.
		
		
		##############  PREVIOUS YEAR SPATIAL PLOTS PREVIOUS YEAR SPATIAL PLOTS
		...
		# This is the Georges B prerecruits slide 42
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Pre-recruits",yr-1,".png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[3],contour=list(preCont.poly,precont.data),direct = direct,title=pre.rec.title.last,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,cex.mn=2,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGBbLive.dat,subset=year==yr-1 & state=='live'& random==1,pch=20,bg='black',cex=0.8)
		# Add the exploratory survey tows
		points(lat~lon,survGBbLive.dat,subset=year==yr-1 & state =='live' & random==2,pch=24,bg="darkorange",cex=0.8)
		# Add the legend
		legend("bottomleft",c(paste(lvls[-length(lvls)],'-',lvls[-1],sep=''),
		                      paste(lvls[length(lvls)],'+',sep='')),fill=c(precont.data$col),title=N.tow.lab,title.adj=0.2,
		       border=c(rep('black',length(lvls))),pch=c(rep(NA,length(lvls))),
		       pt.bg = c(rep(NA,length(lvls))),inset=0.03,bty="n")
		legend("topright",legend = c(paste('exploratory (n =',
		                                   length(unique(subset(survGBbLive.dat,year==yr-1 & random==2)$tow)),
		                                   ")",sep=""),
		                             paste('regular (n =',
		                                   length(unique(subset(survGBbLive.dat,year==yr-1 & random==1)$tow)),
		                                   ")",sep="")),title="Tow type",
		       pt.bg = c("darkorange","black"),pch=c(24,20),bg = "white",inset=0.01,box.col="white")	
		dev.off()
		############
		...
		#Source9 This is recruits again, Georges B, slide 41
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Recruits",yr-1,".png",sep=""),units="in",width = 11,
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[3],contour=list(recCont.poly,reccont.data),direct = direct,title=rec.title.last,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,cex.mn=2,dec.deg = F)
		# Add the regular survey tows.
		points(lat~lon,survGBbLive.dat,subset=year==yr-1 & state=='live'& random==1,pch=20,bg='black',cex=0.8)
		...
		#Fully recruited for Georges B slide 40, 
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,yr,"/Presentations/Survey_summary/",bnk[3],"/Fully_recruited",yr-1,".png",sep=""),units="in",width = 11, 
		    height = 8.5,res=420,bg = "transparent")
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[3],contour=list(comCont.poly,comcont.data),direct = direct,title=fully.rec.title.last,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,cex.mn=2,dec.deg = F)
		...
		#Source9 Condition factor for Georges B, slide 44.  
		# Source9 source(paste(direct,"Assessment_fns/Maps/ScallopMap_test.r") #source("fn/ScallopMap.r")
		png(paste(direct,"Presentations/",yr-1,"/Survey_summary/",bnk[3],"/CF_spatial.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		
		
		par(mfrow=c(1,1))
		#windows(11,8.5)
		ScallopMap(area=bnk[3],contour=list(cont.poly,cont.data),direct = direct,title=cf.title.last,
		           plot.bathy=T,plot.boundries = T,bathy.source="usgs",isobath = c(seq(40,140,by=20)), xlab="",ylab="",
		           nafo.bord = T,nafo="all",nafo.lab = F,cex.mn=2,dec.deg = F)
		# Add the survey tows in GBb, somewhat messy way to get there...
		temp <- subset(GBcfData$CF.data,year==yr-1,c('ID','lon','lat','CF'))
		names(temp) <- c("EID","X","Y","CF")
		temp$EID <- 1:nrow(temp)
		CF.B <-findPolys(temp,GBb.strata.bound.poly)
		addPoints(subset(temp,EID %in% CF.B$EID),pch=20,bg='black',cex=0.8)
		...
		## Seedbox on GBa, because there is only 1 in 2015 there isn't much point of this figure
		## But is could be handy someday!
		...
		################################################################################################################
		################################ SECTION 8 The Occasional Banks ########################################################
		###  Banquereau ###  Banquereau###  Banquereau###  Banquereau###  Banquereau###  Banquereau###  Banquereau###  Banquereau
		############################### SECTION 8 The Occasional Banks ########################################################
		############################################################################################################
		
		
		# Grab any Banquereau data...
		survBan.dat<-subset(springSurv.dat,bank %in% "Ban")
		
		# A plot for Banquereau
		png(paste(direct,"2012/Presentations/Survey_summary/Ban/Survey_strata.png",sep=""),units="in",width = 11, height = 8.5,
		    res=420,bg = "transparent")
		ScallopMap(area="Ban",plot.bathy=T,bathy.source = "quick",boundries = "offshore",shore="nwatlHR",
		           bound.color = F,plot.boundries = T,label.boundries = F,offshore.names=T,xlab="",ylab="",
		           title=paste("Survey (Ban-2012)",sep=""),cex.mn=2,dec.deg = F,direct=direct)
		
		points(slat~slon,survBan.dat,subset = year == 2012 & state=='live' & random == 1,
		       pch=20,bg='black',cex=0.6)
		
		legend("topright",
		       legend = paste('regular (n =',
		                        length(unique(subset(survBan.dat,year==2012 & random==1 & state=='live'))$tow) ,
		                        ")",sep=""),title="Tow type",
