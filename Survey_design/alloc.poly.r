         ...
         repeated.tows=NULL,lplace='bottomleft',show.pool=F,seed = NULL,repo = 'github')
{
  ...
  for(i in 1:nrow(strata))
  {
    ...
    # strataTows.lst[[i]]<-data.frame(subset(pool.EventData,EID %in% LocSet$EID)[1:towsi[strata[i]],c("EID","X","Y")],
    #                                 Poly.ID=poly.lst[[2]]$PID[poly.lst[[2]]$PName==strata[i]],
    #                                 STRATA=strata[i])
  } # end for(i in 1:length(strata))
