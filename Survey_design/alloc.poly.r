         ...
         repeated.tows=NULL,lplace='bottomleft',show.pool=F,seed = NULL,repo = 'github')
{
  ...
  if(!is.null(repeated.tows))
  {
    ...
    # flip to sf
    repeated.tows <- st_as_sf(repeated.tows, coords=c("X", "Y"), remove=F, crs=4326)
    ...
    tmp <- rbind(Tows,repeated.tows[, c("EID", "X", "Y", "geometry", "Poly.ID")])
    ...
    repeated.tows<-subset(tmp,nndist > mindist & EID>1000)
