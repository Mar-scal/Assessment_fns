...
# Minor tweak to function agrument line (removed "bnk" and make it "Ger")
# May 2nd, 2017 Removed the Tow ID from the relief plot so to minimze confusion, Temp ID is added, and title now "Preliminary Tow #".
#####################################  Function Summary ########################################################
####  
...
for(i in 1:nrow(tows))
{
	...
	if(fig=="pdf") pdf(paste(save.dir,"Preliminary_",tows$EID[i],".pdf",sep=''),width = 8.5, height = 8.5,bg = "transparent")
  if(fig=="png") png(paste(save.dir,"Preliminary_",tows$EID[i],".png",sep=''),width = 8.5, units="in", res=420,height = 8.5,bg = "transparent")
      ...
      # First make the topography from the location data given.
			...
			if(length(towMB.lst$x)>1 && length(towMB.lst$y)>1)
			{
			  # Make a perspective plot based on the towMB data.
				...
				title(paste("Temp ID",tows$EID[i]),cex=1.5,adj=0.1)
