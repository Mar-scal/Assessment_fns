...
hot_tows <- st_intersection(stations_bbn, BBn_hot)
hot_tows19 <- st_intersection(stations2019, BBn_hot)

ggplot() + geom_sf(data=BBn_hot) + geom_sf(data=hot_tows) + geom_label(data=hot_tows, aes(lon, lat, label=ID))
...
map2019 <- pecjector(area="BBn", gis.repo = "github", repo=direct_fns, add_layer=list(survey=c("offshore", "outline"))) +
  #scale_fill_manual(values=rep(NA, 9)) +
  geom_sf(data=BBn_hot, fill=NA, colour="red")+
  geom_text(data=stations2019, aes(lon, lat, label=tow, colour=com), size=2) +
  coord_sf(xlim = c(-66.4, -65.8), ylim=c(42.5, 42.9)) 
ggplotly(map2019)
add_ons <- c("2018.284", "2018.270", "2018.285", "2019.233", "2019.237", "2019.279", "2019.269", "2019.256", "2019.264", "2019.257", "2019.270")
...
#Add tows 274, 276, 277 from 2019 (one of which was sampled)
sampled2019 <- offshore_data$mw.dat.all$BBn %>% 
  filter(year==2019 & tow %in% towsfr2019$tow) %>%
  distinct(tow) # 13

sampled2018 <- offshore_data$mw.dat.all$BBn %>% 
  filter(year==2018 & tow %in% towsfr2018$tow) %>%
  distinct(tow) # 1

sampled2018$ID <- paste0("2018.", sampled2018$tow)
sampled2019$ID <- paste0("2019.", sampled2019$tow)

sampled <- rbind(sampled2018, sampled2019)
sampled$sampled <- "Y"

BBn_tows <- left_join(BBn_tows, dplyr::select(sampled, ID, sampled))
BBn_tows$sampled[is.na(BBn_tows$sampled)] <- "N"

table(BBn_tows$sampled)
```
...
map_stations_B <- pecjector(area="BBn", gis.repo = "github", repo=direct_fns, add_layer=list(survey=c("offshore", "outline")))  + 
  geom_point(data=dplyr::select(BBn_tows, lon, lat), aes(lon, lat), colour="black", shape=21, size=3) +
   # geom_text(data=dplyr::select(stations2020, lon, lat, sampled, tow), aes(lon, lat, label=tow), colour="black", size=3, hjust=0.5, vjust=0.25) +
  scale_fill_viridis_d(begin=0.4, end=0.9) +
  annotate(x=-65.8, y=42.95, geom = "text", label="Total = 30", hjust=0) 
 # coord_sf(ylim=c(41.8, 42.2)) +
 # geom_hline(yintercept=41.83333, linetype="dashed")
...
write.csv(dplyr::select(BBn_tows, lat, lon, tow2020, bank), file = "C:/Users/keyserf/Documents/Version_control_pandemic/Offshore/Survey/2020/BBn_stations2020_proposed.csv")
```
...
# overlay fishery (not done for BBn yet, code below is just copied from GB)
```{r}
# GB_fish <- offshore_data$new.log.dat
# GB_fish1920 <- subset(GB_fish, year%in% c(2019,2020) & sfa %in% c("27A", "27B"))
# 
