  ...
  if(tow==TRUE) {
    ...
    towchecks <- function(tows, icetow=NULL){
      ...
      if(length(unique(tows$SPECIES_ID))>1) {
        stop("\nThere are multiple species in the tow file. Please split into two separate files like so: Ban2012tows.csv and Ban2012icetows.csv")
      }
...
Check the MGT_AREA_CD values for the following tows:")
  ...
  if(mwsh==TRUE){
    ...
    mwshchecks<- function(mwshs){
      ...
      print("Are there empty rows at the bottom of the MWSH dataframe? If so, go back to your spreadsheet and delete empty rows.")
      print(tail(mwshs))
