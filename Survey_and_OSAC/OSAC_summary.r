...
#low:       For the banks with lower catch rates (Sab,Mid,Ban,Ger,BBs) what is a good catch in a cell.  Default = 1 tonnes
...
OSAC_summary <- function(yr = as.numeric(format(Sys.time(), "%Y"))-1, direct = "Y:/Offshore scallop/Assessment",
                         ...
                         poly.brd = "black", save.fig = F, add.titles = T,low = 1, 
                         low.banks = c("Ban","Mid","Sab","Ger","BBs","SPB"),
...
fnames <- list.files(path = paste0(direct, "Data/Port_Sampling/", yr, "/"))
fnames <- fnames[-grep("~",fnames)]
mctable <- meat.count.table(filenames=fnames, year=yr)
