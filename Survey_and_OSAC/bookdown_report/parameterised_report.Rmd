...
Abundance and biomass throughout the survey time series are presented using numbers or kilograms of scallop per standardized tow, respectively (Figures `r paste0(params$banknum * 2, " & ", params$banknum * 3)`). Survey tows are standardized to 800 metres. `r if(params$bank %in% c("Ban", "BanIce", "Mid", "GB")) fig1foot` Tables `r paste0(params$banknum * 1, " & ", params$banknum * 2)` provide the plotted values for the current and previous surveys, and long term medians, by size class. For `r bankname`, pre-recruits are scallops with shell heights less than `r sizes$RS[sizes$bank==params$bank]$size` mm, recruits have shell heights from `r sizes$RS[sizes$bank==params$bank]$size` mm to `r sizes$CS[sizes$bank==params$bank]$size` mm, and fully-recruited scallop have shell heights greater than `r sizes$CS[sizes$bank==params$bank]$size` mm. Recruits are expected to grow to commercial size in the next year.
...
For `r bankname` in `r year`, most of the scallops ranged from `r highlights$thisyear[highlights$variable=="sizerange75" & highlights$bank==params$bank]` mm (Figure `r params$banknum * 4`). In the previous survey, the 75% size range was from `r highlights$lastyear[highlights$variable=="sizerange75" & highlights$bank==params$bank]`. None of the bins had more than `r ceiling(as.numeric(highlights$thisyear[highlights$variable=="maxbin" & highlights$bank==params$bank]))` individuals/tow. The most frequent size class `r highlights$word[highlights$variable=="maxbin" & highlights$bank==params$bank]` mm.
...
On `r bankname`, 75% of the `r year` survey tows had `r prthisyear` pre-recruits per tow (Figure `r params$banknum * 6`). The maximum tow had approximately `r highlights$thisyear[highlights$variable=="maxPRtow" & highlights$bank=="Ban"]` pre-recruits.

On `r bankname`, 75% of the `r year` survey tows had `r rthisyear` recruits per tow (Figure `r params$banknum * 7`). The maximum tow had approximately `r highlights$thisyear[highlights$variable=="maxRtow" & highlights$bank=="Ban"]` pre-recruits.

On `r bankname`, 75% of the `r year` survey tows had `r frthisyear` fully-recruited scallop per tow (Figure `r params$banknum * 8`). The maximum tow had approximately `r highlights$thisyear[highlights$variable=="maxCtow" & highlights$bank=="Ban"]` pre-recruits.
...
On `r bankname`, condition factor (Figure `r params$banknum * 9`) was between `r highlights$thisyear[highlights$variable=="minCF" & highlights$bank==params$bank]` and `r highlights$thisyear[highlights$variable=="maxCF" & highlights$bank==params$bank]`.
...
Patterns in natural mortality are represented by the percentage of clappers (dead scallop, with empty hinged shells) caught in the survey tows. The percentage is calculated as the number of clappers in each size class relative to the total of dead and live scallop caught in that size class (Figure `r params$banknum * 11`). Survey tows are standardized to 800 metres. `r if(params$bank %in% c("Ban", "BanIce", "Mid", "GB")) fig1foot` Table `r params$banknum * 3` provides the plotted values for the current and previous surveys, and long term medians, by size class. 
...
Figure `r params$banknum * 12` provides an overview of how biomass and meat count are associated with each other, by showing the biomass for each shell height bin and the meat count for each shell height bin. Scallop with shell heights of `r highlights$thisyear[highlights$variable=="sh_for_mcreg" & highlights$bank==params$bank]` mm and above at the time of the survey would result in meat counts at or below the regulation (`r sizes$mc[sizes$bank==params$bank]$size` mm).


```{r, echo=FALSE, message=F, out.height="90%", out.width="90%", fig.align="center", fig.cap = "The relationship between biomass and meat count for the current survey. Shell height (5 mm bins) is on the x-axis. Biomass (kilograms per standardized tow, kg/tow) is on the left y-axis and corresponds to the grey bars, while meat count is on the right y-axis and corresponds to the red points and solid line. The blue vertical lines represent the recruit size range. The dashed red lines delineate the size range at wich you would have a meat count of `r sizes$mc[sizes$bank==params$bank]$size` mm or below."}
