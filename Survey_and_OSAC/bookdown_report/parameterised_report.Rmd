...
For `r bankname` in `r year`, the size bin with the most scallop per tow was `r maxbin` mm (Figure `r params$banknum * 4`). Table `r params$banknum * 3` provides the shell height range of 75% of the scallops in each size class.


```{r, echo=F}
table <- highlights[highlights$bank==params$bank & highlights$variable %in% c("sizerange75PR", "sizerange75Rec", "sizerange75FR"), c("variable", "lastyear", "thisyear")]
table$variable <- c(paste0("Pre-recruits (<", sizes$RS[sizes$bank==params$bank]$size, "mm)"),
                    paste0("Recruits (", sizes$RS[sizes$bank==params$bank]$size, "-", sizes$RS[sizes$bank==params$bank]$size, "mm)"),
                    paste0("Fully-recruited (>", sizes$CS[sizes$bank==params$bank]$size, "mm)"))
names(table) <- c("Size class", "Previous survey", "Current survey")

knitr::kable(table, format = "latex", row.names=F, booktabs=T, caption="Shell height ranges (in millimetres, mm) for 75\\% of the scallops in each size class, in the current survey and previous survey.") %>%
  kableExtra::kable_styling(latex_options = "HOLD_position")
...
```{r, echo=F}
prthisyear <- strsplit(highlights$thisyear[highlights$variable=="PR75" & highlights$bank==params$bank], " ")[[1]][1]
rthisyear <- strsplit(highlights$thisyear[highlights$variable=="R75" & highlights$bank==params$bank], " ")[[1]][1]
frthisyear <- strsplit(highlights$thisyear[highlights$variable=="C75" & highlights$bank==params$bank], " ")[[1]][1]

```


On `r bankname`, 75% of the `r year` survey tows had `r prthisyear` pre-recruits per tow (Figure `r params$banknum * 6`). The tow with the most pre-recruits had `r highlights$thisyear[highlights$variable=="maxPRtow" & highlights$bank==params$bank]` pre-recruits. The tow with the most recruits had approximately `r highlights$thisyear[highlights$variable=="maxRtow" & highlights$bank==params$bank]` individuals, and 75% of the `r year` survey tows had `r rthisyear` recruits per tow (Figure `r params$banknum * 7`). The tow with the most fully-recruited scallop had approximately `r highlights$thisyear[highlights$variable=="maxCtow" & highlights$bank==params$bank]` individuals, and 75% of the `r year` survey tows had `r frthisyear` fully-recruited scallop per tow (Figure `r params$banknum * 8`).

On `r bankname`, condition factor was between `r highlights$thisyear[highlights$variable=="minCF" & highlights$bank==params$bank]` and `r highlights$thisyear[highlights$variable=="maxCF" & highlights$bank==params$bank]` (Figure `r params$banknum * 9`) . The average meat count was `r highlights$thisyear[highlights$variable=="meanMC" & highlights$bank==params$bank]`, ranging from `r highlights$thisyear[highlights$variable=="minMC" & highlights$bank==params$bank]` to `r highlights$thisyear[highlights$variable=="maxMC" & highlights$bank==params$bank]`  (Figure `r params$banknum * 10`).
