...
```{r, echo=FALSE, message=F, out.height="90%", out.width="90%", fig.align="center", fig.cap=paste0("Abundance time series, represented by number of scallop per standardized tow (y-axis). Pre-recruits (less than ", sizes$RS[sizes$bank==params$bank]$size, "mm shell height) are shown in the top panel; recruit-sized (", sizes$RS[sizes$bank==params$bank]$size, " to ", sizes$CS[sizes$bank==params$bank]$size, "mm shell height) scallop in the middle panel; and fully-recruited (greater than ", sizes$CS[sizes$bank==params$bank]$size,"mm shell height) scallop in the bottom panel. Recruits are expected to grow to commercial size next year. The grey dashed line is the long term median.")}
...
if(params$bank %in% c("Ban", "BanIce", "Mid", "GB")) fig1foot <- paste0("Note: For the abundance and biomass time series figures (Figures ", which(params$bank %in% params) * 2, " & ", which(params$bank %in% params) * 3, "), error bars are not calculated for fixed station survey designs, as they do not properly account for uncertainty.")
```

`r if(params$bank %in% c("Ban", "BanIce", "Mid", "GB")) fig1foot`
...
```{r, echo=FALSE, message=F, out.height="90%", out.width="90%", fig.align="center", fig.cap=paste0("Biomass time series, represented by kilograms (kg) of scallop per standardized tow (y-axis). Pre-recruits (less than ", sizes$RS[sizes$bank==params$bank]$size, "mm shell height) are shown in the top panel; recruit-sized (", sizes$RS[sizes$bank==params$bank]$size, " to ", sizes$CS[sizes$bank==params$bank]$size, "mm shell height) scallop in the middle panel; and fully-recruited (greater than ", sizes$CS[sizes$bank==params$bank]$size,"mm shell height) scallop in the bottom panel. Recruits are expected to grow to commercial size next year.")}
