...
# 10: INLA:     What to you want to do with the spatial modelling.  Three options, the default is INLA= "quick"  
#               1: "quick"      This runs the INLA models with a simple mesh and no prediction on a grid.  The quick runs will generally provide results
#                               within several minutes for all the banks.  These are generally fine for preliminary runs and looking at your data as the
#                               the final results generally won't differ much, these plots will have some funny artifacts near boundaries
#               2: "run.full"   The full spatial models will be run, THIS WILL RUN AND SAVE the models need for ALL spatial figures, not just the
#                               models specified by the "plots" arguement above.  If you have "s.res = 'high'" this can take hours for a bank
#                               the intention is that this would only be run once you are satisfied with the data.  For each banks a file
#                               is saved with the Bank name and spatial resolution in the file name so that high and low runs can be saved and 
...
#               3: "load.full"  This loads in the spatial models saved and avoids having to re-run the "run.full" option whenever you want to
...
survey.figs <- function(plots = c("PR-spatial","Rec-spatial","FR-spatial","CF-spatial","MC-spatial","Clap-spatial","Survey","MW-SH",
                       ...
                       add.title = T,fig="screen",season="both",INLA = "quick" ,contour =F)
{ 
...
################  Next up are the rest of the spatial plots ###########################
 ############  Now start getting the spatial data for this year for the specific bank.
  ...
  if((length(spatial.maps > 0) || any(plots %in% c("seedboxes","user.SH.bins"))))
  {
    ...
    # This section only needs run if we are running the full or quick INLA models.
    if(INLA != "load.full")
    {
      ...
      # If running the full models this is how the mesh and A matrix are constructed
      if(INLA == "run.full")
      {
        ...
        # This make a prediction grid for projection onto our mesh,
        proj <- inla.mesh.projector(mesh,xlim=xyl[1, ], ylim=xyl[2,],dims = s.res) # 500 x 500 gives very fine results but is slow.                       
        # What we do now is find which points of this projection actually fall within our survey area...
        ...
        # And make a prediction based A matrix.
        A.pred <- proj$proj$A[which(pred.in),]
        A.pred.cf <- proj$proj$A[which(pred.in),]
        ...
        # Typically I'd do this below where we build the estimation stack for each variable, but this is identical for all the
        # model runs so no point of repeating it.  This is necessary to get our prediction on the specific part of the bank we are interested in...
        stk.p <- inla.stack(tag="pred",data=list(y = rep(NA,sum(pred.in)), link=1L),
                            effects=list(data.frame(a0 = rep(1, sum(pred.in))), s = 1:spde$n.spde),
                            A = list(1, A.pred))
        stk.p.cf <- inla.stack(tag="pred",data=list(y = rep(NA,sum(pred.in)), link=1L),
                               effects=list(data.frame(a0 = rep(1, sum(pred.in))), s = 1:spde$n.spde),
                               A = list(1, A.pred.cf))
      } # end if(INLA == "run.full")
      ...
      #browser()
      ### A whole bunch of logical statements to make sure we run the INLA models on the data we want to run it on...
      ...
      if(any(plots %in% "seedboxes") || INLA == 'run.full') seed.n.spatial.maps <- 
                                                                      c("PR-spatial","Rec-spatial","FR-spatial","CF-spatial","MC-spatial","Clap-spatial")
      ...
      # If we don't want the seedboxes and we aren't saving the INLA results then we just do this for the plots we are interested in
      if(length(grep("seedboxes",plots)) == 0 && INLA != 'run.full') seed.n.spatial.maps <- spatial.maps
      ...
      if(length(seed.n.spatial.maps > 0))
      {
        ...
        for(k in 1:length(seed.n.spatial.maps))
        {
          ...
          if(seed.n.spatial.maps[k] == "PR-spatial")    
          {
            ...
            stk.e <- inla.stack(tag="est",data=list(y = tmp.dat$pre, link=1L),
                            effects=list(data.frame(a0=rep(1, nrow(tmp.dat))), s = 1:spde$n.spde),
                              A = list(1, A))
            # Put the prediction and estimation staks together for the model
            if(INLA == "run.full") stk <- inla.stack(stk.e, stk.p)
            if(INLA == "quick") stk <- stk.e # Or just use the estimation stack
            # stk <- stk.e
          ...
          if(seed.n.spatial.maps[k] == "Rec-spatial")        
          {
            ...
            stk.e <- inla.stack(tag="est",data=list(y = tmp.dat$rec, link=1L),
          ...
          {
            # This is the stack for the INLA model
            stk.e <- inla.stack(tag="est",data=list(y = tmp.dat$com, link=1L),
                              effects=list(a0 = rep(1, nrow(tmp.dat)), s = 1:spde$n.spde),
                              A = list(1, A))
            # Put the prediction and estimation staks together for the model
            if(INLA == "run.full") stk <- inla.stack(stk.e, stk.p)
          ...
          {
            # This is the stack for the INLA model
            stk.e <- inla.stack(tag="est",data=list(y = tmp.cf$CF, link=1L),
          ...
          {
            # This is the stack for the INLA model
            stk.e <- inla.stack(tag="est",data=list(y = tmp.cf$meat.count, link=1L),
                              effects=list(a0 = rep(1, nrow(tmp.cf)), s = 1:spde$n.spde),
                              A = list(1, A.cf))
            # Put the prediction and estimation staks together for the model
            if(INLA == "run.full") stk <- inla.stack(stk.e, stk.p)
          ...
          {
            # This is the stack for the INLA model
            stk.e <- inla.stack(tag="est",data=list(y = tmp.clap$clap.prop, link=1L),
          ...
          if(INLA == "run.full") mod.res[[seed.n.spatial.maps[k]]][!pred.in] <- NA
    ...
    if(any(plots == "user.SH.bins") || INLA == 'run.full')
    {
      ...
      # Now run through each bin...
        for(k in 1:num.bins)
        {
          ...
          stk.e <- inla.stack(tag="est",data=list(y = tmp.dat[,pick], link=1L),
                            effects=list(a0 = rep(1, nrow(tmp.dat)), s = 1:spde$n.spde),
                            A = list(1, A))
          # Put the prediction and estimation staks together for the model
          if(INLA == "run.full") stk <- inla.stack(stk.e, stk.p)
          ...
          if(INLA == "run.full") mod.res[[bin.names[k]]][!pred.in] <- NA
    ...
    if(INLA =='run.full') 
    {
      #save.image(file = paste(direct,"Data/Survey_data/", yr, "/Survey_summary_output/" ,banks[i],"_figures_res_",s.res[1],"-",s.res[2], ".RData",sep=""))
      save(mod.res,proj,
