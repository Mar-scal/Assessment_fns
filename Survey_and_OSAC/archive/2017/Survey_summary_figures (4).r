...
# June 2017:  Revised to work with new method of inputing the strata, other minor tweaks/updgrades during the month 
#####################################  Function Summary ########################################################
####  
    ...
    if(banks[i] == "GBa") 
      {
        ...
        legend(-67.15,41.82,legend = round(surv.info$area_km2),
               ...
               pt.bg = c(rep(NA,length(surv.info$PName))),col='black',bty='n')
        legend(-66.12,41.55,legend = as.numeric(with(subset(surv.Live[[banks[i]]],year==yr),tapply(tow,Strata_ID,length))),
               fill=c(surv.info$col),border=c(rep('black',length(surv.info$PName))),
