                    ...
                    mc=40,title=paste("Breakdown",yr),cx.axs=1.5,cx.lab=1.5,cx.mn=2,add.title=T)
{
  ...
  # Set up the plot margins
	...
	y.ax <- axTicks(4)
	mn.ax <- (y.ax[2]-y.ax[1])/2
	axis(4,at=y.ax,las=1,cex.axis=cx.axs,tcl=0.6)
	axis(4,at=seq(mn.ax,(max(y.ax)+mn.ax),2*mn.ax), las=1,cex.axis=cx.axs,tcl=0.3,labels=F)
	# Add the axis labels...
	mtext("Shell height (mm)", 1, line=3, cex = cx.lab)	
	mtext(expression(frac(kg,tow)), 2, line=3, cex = cx.lab,las=1)	
	mtext(expression(paste("MC:",bgroup("(",frac(N,"500 g"),")"))) , 4, line=3, cex = cx.lab,las=1)
