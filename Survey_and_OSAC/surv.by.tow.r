                      ...
                      B=htwt.fit$B, mw.par='fixed',bar.ht='com', mc=33)
{
  ...
  for(i in 1:length(years))
  {
    ...
    if(type=='B' || type == "ALL")
    {
      ...
      if(mw.par!='annual'&& mw.par!='fixed' && mw.par!="weight.matrix") mw[[i]]<-sweep(matrix((seq(2.5,200,5)/100)^3,nrow(ann.dat),40,byrow=T,dimnames=list(ann.dat$tow,mw.bin)),
                                                                                       1,FUN='*',ann.dat[,mw.par])
      # new method from 2024 framework
      # use the weight.matrix object to multiply the shf.dat and get a biomass matrix
      ...
      shf.dat <- dplyr::arrange(shf.dat, year, tow, state)
      
      # Now we can grab the biomass of the pre-recruits
      # this multiplies mw * shf and then adds them up all at once
      ...
      # Now we can grab the biomass of the commercial scallops
  ...
  if(type=='MC' & !mw.par=="weight.matrix")
  {
