                      ...
                      ypos=1, add.title = F, cx.mn = 1, titl = "", axis.cx=1,user.bins = NULL, ...)
{
  ...
  for(i in 1:length(mn.tmp))
  {
    ...
    if(!is.null(user.bins))
    {
      if(i == 1) plot.names[[i]] <- paste("< ",user.bins[i]," mm",sep = "")
      if(i > 1 && i < length(mn.tmp)) plot.names[[i]] <- paste(user.bins[i-1],"-",user.bins[i], " mm",sep="")
      if(i == length(mn.tmp)) plot.names[[i]] <- paste("> ", user.bins[i-1], " mm",sep="")
    } # end if(!is.null(user.bins))
    if(is.null(user.bins))
    {
