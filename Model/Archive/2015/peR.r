# This function is used for the Prediction Evaluation, commented and checked by DK in January 2016
...
peR <- function(model, input, priors, years, pe, n = 80000, burn = 40000, thin = 10, graphic="R",run=T,TAC=100,plot=1,full.ests=F,
                proj.from="BUGS",lab='',g2,gR2,debug=F,dtable=T,path=''){
  ...
  # rename the years.
	...
	# If running the model to get the estimates
	if(run==T)
	  {
  		# Create an out object
  	  out<-list(NULL)
  		# Get the input data (model input list) and turn it into a dataframe, exclude NY, NC, and C.p if they exist.
  		input.dat<-data.frame(Year=years,input[names(input) != "NY" & names(input) != "NC" & names(input) != "C.p"])
      
  		# Loop through each year we want the predictive evaluation done "pe"
  		for(i in 1:length(pe))
  		  {
  				# Select the years
    			yrs<-years[1]:pe[i]
    			# Subset the input by years and remove the first column (the years)
    			input.lst<-as.list(subset(input.dat,Year %in% yrs,-1))
    			input.lst$NY <- length(input.lst$C)
    			# If alternative growth is 
    			if(!missing(g2))input.lst$g[input.lst$NY]<-g2[input.lst$NY]
    			if(!missing(gR2))input.lst$gR[input.lst$NY]<-gR2[input.lst$NY]
    		
    			# Run model		
    			#browser()
     			DD<-delayBUGS(model, input.lst, priors, yrs, n = n, burn = burn, thin = thin,
     			              debug=debug,pe=T,parameters=c(names(priors),'K','P','B','R','mu'))
	    ...
	    dump('out',paste(lab,'peOutList.R',sep=''))
	  }
	
	if(!run)source(paste(lab,'peOutList.R',sep=''))
	...
	if(1%in%plot){
		...
		if(graphic=='pdf')pdf(paste(path,"pe1",lab,".pdf",sep=''), width=6.5, height=5)
	...
	if(3%in%plot){
		...
		for(i in 1:length(out)){
			pB<-out[[i]][[2]]/1000
			#browser()
			#test<-boxplot(pB,range=0,plot=F)
			#test$stats[1,]<-quantile(pB,0.1)
			#test$stats[5,]<-quantile(pB,0.9)
			#bxp(test,add=T,at=years[i]+1,axes=F,range=0,lty=1)
