biomass.plt <- function(out.data, years, graphic='pdf',avg.line=NULL,ht=7,wd=6.5,Rymax,Bymax,TAC,pred=1,kt=T,refs=c("URP","LRP","zones"),index,URP,LRP,URP2=8000,LRP2=3000,out.data2,lab='',alpha=c(0.05,0.2),path=''){
...
if(missing(Bymax))Bymax <- max(c(apply(out.data$sims.list$B, 2, quantile, 1-alpha[1]/2),quantile(out.data$sims.list$B.p[,TACI],1-alpha[2]/2)))
#browser()
...
text(2010, 100, "CRITICAL", col='firebrick1', cex=1.5)
text(2010, 10000, "CAUTIOUS", col='goldenrod1',cex=1.5)
text(2010, 50100, "HEALTHY", col='chartreuse2',cex=1.5)
