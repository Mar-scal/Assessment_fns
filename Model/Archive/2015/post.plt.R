post.plt <- function(model.out, priors, years, nr=4, nc=2, wd=8, ht=11, post.labs=NULL, graphic='R',xl.type=1, multi=T,path=''){

	if(graphic=='pdf')pdf(paste0(path,"post_single.pdf"), width = wd, height = ht, pointsize = 16)
	if(graphic=='R')windows(record=T,wd, ht)
	...
	if(multi){
		...
		if(graphic=='pdf')pdf(paste0(path,"post_annual.pdf"), width = wd, height = ht, pointsize = 12)
		for (i in 1:length(posts)){
...
#		browser()
				...
				if(names(multiposts)[i]%in%names(priors))p<-get(priors[[names(multiposts)[i]]]$d)(x,  priors[[names(multiposts)[i]]]$a,  priors[[names(multiposts)[i]]]$b)
