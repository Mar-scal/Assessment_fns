...
# diagnostics
if(graphic=="pdf")pdf(paste(path, name, ".pdf", sep=""), width = 8.5, height = 11, pointsize=16)
...
if("qU"%in%names(data$median))par(mfrow=c(4, 1), mar = c(1, 4, 1, 1), omi = c(0.2, 0.1, 0.2, 0.2))
	...
	if("qU"%in%names(data$median)){
		...
		plot(years, data$median$sresid.U, las = 1, pch = 16, ylab = "",  ylim = c(-3, 3), mgp = c(0.5, 0.5, 0), xlab = "", tcl = -0.3,xlim=range(years))
		axis(4, lab = F, tcl = -0.3)
		mtext("CPUE", 2, 2.5)
		abline(h = 0, lty = 2)
		sapply(1:length(years), function(i){lines(c((years[1]-1) + i, (years[1]-1) + i), with(data$sims.list, c(quantile(sresid.U[,i], 0.025), quantile(sresid.U[,i], 0.975))))})
	}
}
...
if("qU"%in%names(data$median))par(mfrow=c(4, 1), mar = c(1, 4, 1, 1), omi = c(0.2, 0.1, 0.2, 0.2))
	...
	if("qU"%in%names(data$median)){
		ymin <- min(sapply(1:length(years), function(i){with(data$sims.list, quantile(sresid.U[,i], 0.025))}))
		ymax <- max(sapply(1:length(years), function(i){with(data$sims.list, quantile(sresid.U[,i], 0.975))}))
		plot(years, data$median$sresid.U, las = 1, pch = 16, ylab = "",  ylim = c(-1, 1), mgp = c(0.5, 0.5, 0), xlab = "", tcl = -0.3,xlim=range(years))
		axis(4, lab = F, tcl = -0.3)
		mtext("CPUE", 2, 2.5)
		abline(h = 0, lty = 2)
		sapply(1:length(years), function(i){lines(c((years[1]-1) + i, (years[1]-1) + i), with(data$sims.list, c(quantile(sresid.U[,i], 0.025), quantile(sresid.U[,i], 0.975))))})
