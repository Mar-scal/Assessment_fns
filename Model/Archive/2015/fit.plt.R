...
if(graphic=='pdf')pdf(paste(path,"fit", name, ".pdf", sep=""))
...
if("qU"%in%names(data.out$median)){
	...
	ymax<-ifelse(CI,max(pmax(apply(sweep(data.out$sims.list$B,2,FUN='*',data.out$median$qU/Uadj), 2, quantile, 1-alpha/2), data.out$data$U)),max(c((data.out$median$B[1:length(data.out$data$iyr)]*data.out$median$qU)/Uadj,data.out$U)))
	...
	#segments(data.out$data$iyr, data.out$U+data.out$U.cv*data.out$U, data.out$data$iyr, data.out$U-data.out$U.cv*data.out$U,col='red')
	}
