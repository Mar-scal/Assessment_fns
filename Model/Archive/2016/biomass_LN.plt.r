biomass.plt <- function(out.data, years, graphic='pdf',avg.line=NULL,ht=7,wd=6.5,Rymax,Bymax,TAC,
                        pred=1,kt=T,refs=c("URP","LRP","zones"),index,URP,LRP,URP2=8000,LRP2=3000,out.data2,lab='',alpha=c(0.05,0.2),path='')
{
...
if(missing(Bymax))Bymax <- max(c(apply(out.data$sims.list$B, 2, quantile, 1-alpha[1]/2),
                                 quantile(out.data$sims.list$B.p[,TACI],1-alpha[2]/2)))
#browser()
plot(years, out.data$median$B, type = 'b', pch = 16, ylim = c(0, Bymax), ylab = "", las = 1, 
...
text(max(years)-4, 100, "CRITICAL", col='firebrick1', cex=1.5)
text(max(years)-4, 10000, "CAUTIOUS", col='goldenrod1',cex=1.5)
text(max(years)-4, 50100, "HEALTHY", col='chartreuse2',cex=1.5)
if(is.null(refs))if(!is.null(avg.line))abline(h=avg.line(out.data$median$B[-length(out.data$median$B)]),lty=2,col='blue',lwd=2)
...
if(missing(URP))URP1<-median(out.data$median$B[index])*0.8
else URP1<-URP
if(missing(LRP))LRP1<-min(out.data$median$B[index])
else LRP1<-LRP
...
if(!missing(out.data2)){
	...
	if(is.null(refs))if(!is.null(avg.line))abline(h=avg.line(out.data2$median$B[-length(out.data$median$B)]),lty=2,col='blue',lwd=2)
	...
	if(missing(index))index<-1:length(out.data$median$B)
	if(missing(URP))URP1<-mean(out.data2$median$B[index])*0.8
	else URP1<-URP
