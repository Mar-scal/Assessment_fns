# DK modified in Jan 2016 to allow easier flipping between 2 and 3 year mortality averages...

projections.2015<-function(model.out,C.p=c(200,300),mort=3)
  {
...
if(mort == 3) mean.m<-apply(d$m[,27:29],1,mean)
if(mort == 2) mean.m<-apply(d$m[,28:29],1,mean)

  for(i in 1:length(C.p)){

		# year 1
#		Pmed.p[[i]]<- log(exp(-d$m[,d$NY])*(d$g[d$NY])*(d$P[,d$NY]-C.p[i]/d$K)+exp(-d$m[,d$NY])*(d$gR[d$NY])*d$r[,d$NY])
    Pmed.p[[i]]<- log(exp(-mean.m)*(d$g[d$NY])*(d$P[,d$NY]-C.p[i]/d$K)+exp(-mean.m)*(d$gR[d$NY])*d$r[,d$NY])
