...
load(paste(direct,"Data/Survey_data/",2018,"/Survey_summary_output/Survey_all_results.Rdata",sep=""))
...
q <- 0.25
round.conversion <- 
# Here's the mean number per m2 of each size in each strata in each year on GBa, from the survey and corrected for catchability (very conservatively using 0.25 which will lead to high estiamtes of N and B per tow)
...
#windows(11,11)
require(ggplot2)
pdf(paste(direct,"2018/Presentations/OSAC/GBa/N_by_strata.pdf",sep=""),width=12,height = 6,onefile = T)
...
pdf(paste(direct,"2018/Presentations/OSAC/GBa/Biomass_by_strata.pdf",sep=""),width=12,height = 6,onefile = T)
