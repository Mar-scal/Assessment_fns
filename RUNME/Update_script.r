...
for(b in banks){
  for(it in imputetype){
    ...
    pred.eval(input = DD.lst[[b]], priors = DD.out[[b]]$priors, pe.years= yr:(yr-6), growth="both",
              ...
              parameters = DD.out[[b]]$parameters, niter = 175000/1000, nburn = 100000/1000, nthin = 20, nchains=8/2,
              direct=direct, save.res=paste0(direct, "/Data/Model/", yr+1, "/", b, "/Results/", it,"/"))
    ...
    out.tmp[["2020"]] <- NULL
    #2021
    load(paste(direct,"Data/Model/",2022,"/",b,"/Results/", it, "/Projection_evaluation_realized_growth.RData",sep=''))
    out.tmp[["2021"]] <- out[["2021"]]
