...
for(b in banks){
  ...
  acfs <- apply(old.model[[b]][,c("I", "IR", "clappers", "clappersR", "CF", "l.bar")], MARGIN = 2, function(x) data.frame(acf=acf(x)$acf))
  acf_df <- do.call(cbind.data.frame, acfs)
  names(acf_df) <- names(acfs)
  acf_df <- pivot_longer(acf_df, cols=c("I", "IR", "clappers", "clappersR", "CF", "l.bar")) %>%
    group_by(name)%>%
    mutate(lag=1:n()) %>%
    arrange(name)
  
  survey$name <- factor(survey$name, levels=c("I", "IR", "clappers", "clappersR", "CF", "l.bar"))
  acf_df$name <- factor(acf_df$name, levels=c("I", "IR", "clappers", "clappersR", "CF", "l.bar"))
  
  TS[[b]] <- ggplot() + geom_line(data=survey, aes(year, value)) +
    facet_wrap(~name, scales="free_y", nrow=3) +
    theme_bw()
  
  ACF[[b]] <- ggplot() + geom_bar(data=acf_df, aes(x=lag, y=value), stat="identity") + 
