...
if(preprocessed==F)
  {
    ...
    if(file.exists(paste(direct,"Data/Framework/2017/Growth/Survey_all_results.Rdata",sep=""))==T)
    {
      load(paste(direct,"Data/Framework/2017/Growth/Survey_all_results.Rdata",sep=""))  
    ...
    save(mod.dat,cpue.dat,proj.dat,file=paste(direct,"Data/Framework/",2017,"/Growth/Model_input.RData",sep=""))
...
if(preprocessed==T) load(file=paste(direct,"Data/Framework/",2017,"/Growth/Model_input.RData",sep=""))
...
for(j in 1:num.banks)
{
  ...
  plotsGo <- paste(direct,2017,"/Framework/Growth/Figures/",sep="")
...
# If you need to run the model giver.
  if(run.mod == T)
  {
    ...
    names(DD.dat) <- c( "year","n","I","I.cv","IR",  "IR.cv", "IPR", "IPR.cv","N","N.cv","NR","NR.cv", "NPR", "NPR.cv",
                        "w.bar","l.bar", "l.k", "w.k","CF","clappers","clappersR","CS", "RS", "g","g.se","gR","gR.se","C","E","n.trips","U",
                        "U.var","U.se","LCI","UCI","U.cv") 
    ...
    cat(paste("*NOTE #2* See code for message about the CPUE CV being downweighted artificially, 
              this needs revised in next Framework as it ain't legit.\n",sep = " "))
    ...
    #Prepare priors for JAGS
      ...
      ifelse(is.null(parameters) == T, parameters <- c(names(DDpriors[[bnk]]),'K','P','B','R','mu','Imed','Ipred','Irep', 'IRmed','IRpred','IRrep',
                                                                              ...
                                                                              'IRresid','Presid',"Cresid","CRresid","sCresid","sCRresid","G","GR"),parameters)
      ...
      if(length(proj.catch[[bnk]]) == 0) 
      {
        ...
        writeLines("YO YO CHECK THIS OUT!!  The projected catch used in this model is 0, this should only happen in preliminary runs!!")
      }
      # The interim TAC is
      ...
      if(final.run == T) 
      {
        save(DD.lst, DDpriors,DD.out,mod.dat,cpue.dat,bank,proj.dat,yr,D.tab,manage.dat,proj.catch,
           ...
           file=paste(direct,"Data/Framework/",2017,"/Growth/Results/Final_model_results.RData",sep=""))
      } # end if(final.run == T) 
      ...
      if(final.run == F) 
      {
        save(DD.lst, DDpriors,DD.out,mod.out,mod.dat,cpue.dat,bank,proj.dat,yr,D.tab,manage.dat,proj.catch,
             URP,LRP,DD.dat,proj,bnk,TACi,yrs,
             file=paste(direct,"Data/Framework/",2017,"/Growth/Model_testing_results.RData",sep=""))
      } # if(final.run == F) 
  } # end if(run.mod==T)
...
# If we want the diagnostics  this is the section.  
  if(make.diag == T)
  {
    if(run.mod==F) # First load the data if we didn't just run the model.
    {
      ...
      if(file.exists(paste(direct,"Data/Framework/",2017,"/Growth/Final_model_results.RData",sep=""))==T && use.final==T)
      {
        load(file=paste(direct,"Data/Framework/",2017,"/Growth/Final_model_results.RData",sep=""))
      } # end if(file.exists(paste(direct,"Data/... == T
      ...
      if(file.exists(paste(direct,"Data/Framework/",2017,"/Growth/Model_testing_results.RData",sep=""))==T && use.final==F)
      {
        load(file=paste(direct,"Data/Framework/",2017,"/Growth/Model_testing_results.RData",sep=""))
    ...
    save(mort,TACI,BM.proj.1yr,B.quantiles,percent.B.change,prob.below.USR,FR.bm,FR.ltm,rec.bm,rec.ltm,neff,rhat,
         file=paste(direct,"Data/Framework/",2017,"/Growth/Model_results_and_diagnostics.RData",sep=""))
  } # end (if make.diag == T)
...
# Now if I want to make the figures both from the model output and for the update document do these...
  ...
  if(make.figs == T)
  {
    ...
    if(make.diag.figs == T)
    {
      if(run.mod==F && make.diag == F) load(file=paste(direct,"Data/Framework/",2017,"/Growth/Model_results_and_diagnostics.RData",sep=""))
