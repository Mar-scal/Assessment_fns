...
# Mar 9 2017: Heavily revised to use a gamm for the MW-SH relationship, model is now annual, really has only been
#             fully tested with the GBa data
################################################################################################################
####
################################################################################################################
...
#source(paste(direct,"Assessment_fns/Survey_and_OSAC/shwt.lme.r",sep="")) # No longer needed due to new MW-SH model
#source(paste(direct,"Assessment_fns/Survey_and_OSAC/condFac.r",sep=""))  # No longer needed due to new MW-SH model
source(paste(direct,"Assessment_fns/Framework/2017/Growth/survey.dat_growth_model.r",sep="")) 
source(paste(direct,"Assessment_fns/Framework/2017/Growth/simple.surv_growth_model.r",sep="")) 
...
#
    ...
    # Now save the data so you don't have to do all that every time
		save(list = ls(all.names = TRUE), file = paste(direct,"Data/Framework/",2017,"/Growth/Survey_preprocessed.Rdata",sep=""))
...
if(preprocessed == T) 
  {
    ...
    s.year <- survey.year
    ssn <- season
    load(paste(direct,"Data/Framework/",2017,"/Growth/Survey_preprocessed.Rdata",sep=""))  
...
years <- yr.start:yr
num.years <- length(years) # I am using this for the growth modelling section, if I make that it's own function this can probably go away...
...
bank.dat[[bnk]] <- subset(bank.dat[[bnk]] , year %in% years)
    ...
    #MEAT WEIGHT SHELL HEIGHT RELATIONSHIP in current year, this is the heart of the change in our calculation
    # methodology.
    # First get an object to predict on, this should cover all depths and shell heights of interest
    # It's a bit excessive but that's fine...
    # Covers depths from 0 meters to 150 meters depth based on using 80 meters as the "centering" below
    pred.dat <- data.frame(sh.log = rep(log(seq(2.5,197.5,by=2.5)),length(-80:90)),
                           sh = rep(seq(2.5,197.5,by=2.5),length(-80:90)),
                           depth.cen =rep( -80:90,length(seq(2.5,197.5,by=2.5))),
                           depth =rep( (-80:90)+80,length(seq(2.5,197.5,by=2.5))))
    mwsh.annual.fit <- NULL # I want to overwrite this object for each bank...
    surv.dat[[bnk]] <- bank.dat[[bnk]]
    surv.dat[[bnk]]$CF <- NA
    for(j in 1:length(years))
    {
      test.data <- mw.dat.all[[bnk]][mw.dat.all[[bnk]]$year == years[j],] # Pick the year
      test.data <- test.data[test.data$sh >=40,] # Pick the shells larger than 40 mm
      test.data$sh.log <-log(test.data$sh) # Get the log of the shell height
      #test.data$sh.log.cen <-test.data$sh.log-mean(test.data$sh.log)#
      #test.data$sh.cen <- scale(test.data$sh,scale=F,center=T)
      # IMPORTANT DECISION##  For all areas and years I am making 80 meters the "center" of the data
      # This will avoid any ambiguity between years on where we are predicting if looking for
      # when summarizing anything, the predictions will always be at a depth of 80 meters for all banks/years.
      test.data$depth.cen <- test.data$depth-80
      # Here the model, gamm with a smooth on the log-shell height and a smooth on depth.
      mwsh.annual.fit[[as.character(years[j])]] <-  gamm4(wmw ~ t2(sh.log) + t2(depth.cen) , 
                                                          random = ~ (1  |tow),data=test.data,family = Gamma(link="log"))
      # Get all the numbers starting with the mid-point of the smallest bin above 40 mm, and ending with the midpoint of the largest bin.
      # We will predict on all the depths found the current survey as well.
      pred.mwsh[[as.character(years[j])]] <- data.frame(pred.dat,
                   wmw = predict(mwsh.annual.fit[[as.character(years[j])]]$gam, pred.dat,type="response",re.form=NA),
                   wmw.se = predict(mwsh.annual.fit[[as.character(years[j])]]$gam,
                           pred.dat,type="response",re.form=NA,se=T)$se.fit,year= years[j])       
      # Pick the estimate at every depth for 100 mm scallop to get the expected "condition" for each tow.
      CF <- pred.mwsh[[as.character(years[j])]][pred.mwsh[[as.character(years[j])]]$sh == 100,]
      # This gets the depths for each tow from the the bank.dat object for the current year
      depths <- round(surv.dat[[bnk]]$depth[surv.dat[[bnk]]$year == years[j]])
      # There is a nicer way to do this, but this will get the MW of a 100 mm scallop for each tow...
      for(k in 1:length(depths)) 
      {
        surv.dat[[bnk]]$CF[surv.dat[[bnk]]$year == years[j]][k] <- CF$wmw[CF$depth == depths[k]] 
      } # end for(k in 1:length(depths))
      
    } # end for(j in 1:length(years))
    # Save the results...
    mw.pred.dat[[bnk]] <- do.call("rbind",pred.mwsh)
    mwsh.mod.fits[[bnk]] <-  mwsh.annual.fit  

		
		# Calculate the biomass of the Pre-recruits, Recruits and the Commerical Scallops in each tow on the bank
		# Revised by DK March 10, 2017, removed the function surv.by.tow and inserted this code
    # It might be nicer to just have this as a function, but for the moment I'm putting it inline...
    # This grabs the "bin' numbers for the shell height data
    bin<-na.omit(as.numeric(substr(names(surv.dat[[bnk]]),2,nchar(names(surv.dat[[bnk]])))))
    bin.name <- paste0("h",bin)
    wbin.name <- paste0("w",bin)
    #surv.dat.names <- names(surv.dat[[bnk]])
    weight.bins <- as.data.frame(matrix(NA,nrow=nrow(surv.dat[[bnk]]),ncol=length(wbin.name)))
    colnames(weight.bins) <- wbin.name
    ##  Add a whole bunch of columns for the estimated weight of each size bin.
    surv.dat[[bnk]] <- data.frame(surv.dat[[bnk]],weight.bins)
    #names(surv.dat[[bnk]])
    # Get the bins to pick the mid-point value in each bin, and make sure we have a RS and CS for each year...
    bin.pred <- bin-2.5
    if(length(RS) == 1) rep(RS,length(years))
    if(length(CS) == 1) rep(CS,length(years))

    for(j in 1:length(years))
    {
      # Here area the size bins I need to grab...
      pre.bin <- bin.name[bin <= RS[j]]
      rec.bin <- bin.name[bin > RS[j] & bin <= CS[j]]
      com.bin <- bin.name[bin > CS[j]]
      pred.pre <- bin.pred[bin <= RS[j]]
      pred.rec <- bin.pred[bin > RS[j] & bin <= CS[j]]
      pred.com <- bin.pred[bin > CS[j]]
      # Grab all the data from the smallest bin to the bin before the pre-recruit maximum and add those together.
      
      
      surv.dat[[bnk]]$pre[surv.dat[[bnk]]$year==years[j]] <- rowSums(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],pre.bin]) 
      
      # Grab all the recruit sized data and add those together
      surv.dat[[bnk]]$rec[surv.dat[[bnk]]$year==years[j]] <- rowSums(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],rec.bin]) 
      # Grab all the commerical sized data and add those together
      surv.dat[[bnk]]$com[surv.dat[[bnk]]$year==years[j]] <- rowSums(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],com.bin]) 
      # Add together all of the scallop data, irrespective of size.
      surv.dat[[bnk]]$tot[surv.dat[[bnk]]$year==years[j]] <- rowSums(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],bin.name]) 
      ...
      surv.dat[[bnk]]$l.bar[surv.dat[[bnk]]$year==years[j]] <-rowSums(sweep(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],com.bin],2,FUN='*',pred.com))/
                                                                              rowSums(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],com.bin])
      
      # To get biomass life is a bit more complex, but here we go
      # Run this loop for every row, I can't figure out how to avoid the for loop because we have to 
		  # predict on depth, this will work but if you know a tidier way let me know (sweep almost works but not quite...)
		  for(k in 1:nrow(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],]))
		  {
		  pred.tmp <- mw.pred.dat[[bnk]][mw.pred.dat[[bnk]]$year == years[j] &
		                                   mw.pred.dat[[bnk]]$sh %in% bin.pred &
		                                   mw.pred.dat[[bnk]]$depth == round(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],]$depth[k]),]
		  pred.tmp <- pred.tmp[order(pred.tmp$sh),]
		  
		  # Here we get the biomass by bin and then the biomass by classification...
		  surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,wbin.name] <- as.numeric(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,bin.name]* 
		                                                                        pred.tmp$wmw/1000)

		  surv.dat[[bnk]]$tot.bm[surv.dat[[bnk]]$year==years[j]][k] <- sum(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,bin.name]* 
		                                                                pred.tmp$wmw)/1000
		  surv.dat[[bnk]]$pre.bm[surv.dat[[bnk]]$year==years[j]][k] <- sum(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,pre.bin] * 
		                                                                pred.tmp$wmw[pred.tmp$sh %in% pred.pre])/1000
		  surv.dat[[bnk]]$rec.bm[surv.dat[[bnk]]$year==years[j]][k] <- sum(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,rec.bin] * 
		                                                                pred.tmp$wmw[pred.tmp$sh %in% pred.rec])/1000
		  surv.dat[[bnk]]$com.bm[surv.dat[[bnk]]$year==years[j]][k] <- sum(surv.dat[[bnk]][surv.dat[[bnk]]$year==years[j],][k,com.bin] * 
		                                                                pred.tmp$wmw[pred.tmp$sh %in% pred.com])/1000

		  } # end  for(k in 1:nrow(surv.dat$GBa[surv.dat$GBa$year==years[j],]))
    } # end for(j in 1:length(years))
		  
    # Now we can get the mean weight of commercial sized scallop
    surv.dat[[bnk]]$w.bar <- surv.dat[[bnk]]$com.bm*1000 / surv.dat[[bnk]]$com
    # The meat count of the different size classes as well
    surv.dat[[bnk]]$meat.count.tot <- 0.5* surv.dat[[bnk]]$tot / surv.dat[[bnk]]$tot.bm # Overall meat count
    surv.dat[[bnk]]$meat.count.com <- 0.5* surv.dat[[bnk]]$com / surv.dat[[bnk]]$com.bm # Meat count of fully recruited
    surv.dat[[bnk]]$meat.count.rec <- 0.5* surv.dat[[bnk]]$rec / surv.dat[[bnk]]$rec.bm # Meat count of recruits
    surv.dat[[bnk]]$meat.count.pre <- 0.5* surv.dat[[bnk]]$pre / surv.dat[[bnk]]$pre.bm # Meat count of pre-recruits.
		...
		## DK continuing to rip everything apart over here with these data....
		if(bnk != "Ger" && bnk != "Mid" && bnk != "GB") 
		  {
		    survey.obj[[bnk]] <- survey.dat(surv.Rand[[bnk]], RS=RS, CS=CS, bk=bnk, areas=strata.areas)	
		                                
		    clap.survey.obj[[bnk]] <- survey.dat(surv.Clap.Rand[[bnk]], RS=RS, CS= CS, bk=bnk, areas=strata.areas)	
		  ...
		  ######################################  GROWTH MODELLING GROWTH MODELLING GROWTH MODELLING GROWTH MODELLING   ###########################
		  ######################################  Now we do the growth modelling, this seciton will need a pretty big overhaul ####################
		  ######################################  once we get the growth modelling all sorted out for the moment we are ###########################
		  ######################################  Using growth data from the BoF because that is all the data we have   ###########################
		  ######################################  GROWTH MODELLING GROWTH MODELLING GROWTH MODELLING GROWTH MODELLING   ###########################
		  
		  #################  Now for the growth data#################  Now for the growth data#################  Now for the growth data
		  ###  Now load in the growth data, DK Note:  we'll need to do this differently once we get teh data up and running
		  ###  at a minimum I'm going to want this loaded at the front end of this document
		  growth.dat <- read.csv(paste(direct,"Data/Ageing/BoF_complete_ageing_2016.csv",sep=""),check.names=F) # This data comes from the xlsx (DRAFTageingSFA292016.xlsx)
		  names <- c("LY",as.character(na.omit(as.numeric(names(growth.dat))))) #
		  # Now I am going to subset the data, DK Note:  THIS LINE WILL NEED FIXED WHEN GBa Data becomes available.
		  growth.dat <- growth.dat[grep("29D",growth.dat$SPA),]
		  # I need to get a unique tow ID, so combine Tow number and area
		  growth.dat$Tow_ID <- paste(growth.dat$Cruise,growth.dat$Tow_No,sep="_")
		  ...
		  start.row <- which(names(growth.dat) =="LY")
		  ...
		  end.row <- which(names(growth.dat) == min(as.numeric(names(growth.dat)),na.rm=T))
		  ...
		  for(p in start.row:(end.row-1))
		  {
		    index <- 1+index # update the index
		    nam <- paste("G",names(growth.dat[p]),sep="") # get a good column name for the growth data
		    growth.dat[,(end.row+index)] <- growth.dat[,p] - growth.dat[,(p+1)] # calcuate the difference
		    names(growth.dat)[end.row+index] <- nam # and rename the new data column.
		  ...
		  growth.names <- names(growth.dat)[grep("G",names(growth.dat))]
		  growth.dat.long <- reshape(growth.dat,direction="long",varying=list(names),times=names)
		  
		  growth.dat.long <- growth.dat.long[,-which(names(growth.dat.long) %in% growth.names)]
		  # Just some temporaries bits to store the data.
		  growth.dat.g.long <- reshape(growth.dat,direction="long",varying=list(growth.names),times=growth.names)
		  ...
		  growth.dat.long$GLY <- c(growth.dat.g.long$GLY,rep(NA,nrow(growth.dat)))
		  growth.dat.long <- growth.dat.long[,-which(names(growth.dat.long)=="id")]
		  names(growth.dat.long) <- c(names(growth.dat.long[1:(which(names(growth.dat.long) =="time")-1)]),"year","size","growth")
		  # Finally, and new, the real "size" is the size that the scallop was before the growth was added, 
		  dat.long$size_start <- growth.dat.long$size-growth.dat.long$growth
		  
		  # Now standarized the data for the model amd select the minimum amount of data nece
		  growth.dat.long$std_size <- scale(growth.dat.long$size_start,center=T,scale=F)
		  #dat.tmp$log_size <- log(dat.tmp$size)
		  size_range <- seq((min(growth.dat.long$std_size,na.rm=T)-10),(max(growth.dat.long$std_size,na.rm=T)+10),by=0.1)
		  # Remove the Last year from the data as the chipped shells make that data on it's own very poor.
		  # If we move to more complex year based curves I'd probably want to change this up here to include a minimum 
		  # amount of data, see the orginal growth code for doing these kinda things if needed.
		  #useable.data <- aggregate(std_size~year,growth.dat,FUN=function(x) which(length(x) >= min.dat))
		  #useable.data <- as.character(na.omit(useable.data$year[useable.data$V1==1]))
		  #useable.data <- useable.data[useable.data != "LY"]
		  mod.tmp <- growth.dat.long[growth.dat.long$year != "LY",]
		  mod.tmp$ID <- paste(mod.tmp$Tow_No,mod.tmp$Scallop_No,sep="_")
		  #mod.tmp$ID <- paste(bnk,mod.tmp$Tow_No,mod.tmp$Scallop_No,sep="_") # Once we have growth data for GBa and BBn I might want to put in the bank
		  # ID but for the moment the tow-ID should be sufficient (don't want to confuse things more given this is already 29D data!!)
		  
		  # Run the model for each area
		  mod.growth <- gamm4(growth~ t2(std_size), random =~ (1 | ID),data=mod.tmp,family = "poisson")
		  # Get the predictions from the model.
		  growth.predictions <- data.frame(std_size = size_range) # Some data to predict on for the gam...
		  growth.predictions$size <- growth.predictions$std_size + attr(growth.dat.long$std_size,"scaled:center")
		  growth.predictions$growth <- predict(mod.growth$gam,growth.predictions,type="response",re.form=NA)
		  growth.predictions$se <- predict(mod.growth$gam,growth.predictions,type="response",re.form=NA,se=T)$se.fit
		  growth.predictions$bank <- bnk # Get the name of the area
		  growth.predictions$size.ny <- growth.predictions$size + growth.predictions$growth # mean size next year.
		  growth.predictions$LCI.ny <- growth.predictions$size + growth.predictions$growth - 2*growth.predictions$se 
		  growth.predictions$UCI.ny <- growth.predictions$size + growth.predictions$growth + 2*growth.predictions$se 
		  growth.predictions$size <- round(growth.predictions$size,digits=1) # Need to round it make sure we have clean numbers...
		  
		  ############## Step X, we now have everythign we need to predict growth for all years...
		  ...
		  mw.pred <- data.frame(com = rep(NA,num.years),rec = rep(NA,num.years),
		                        ...
		                        year = years,bank = rep(bnk,num.years))
		  sizes.com <- NA
		  sizes.rec <- NA
		  sizes.ny.com <- NA
		  sizes.ny.rec <- NA
		  # I am running this in a loop rather than using %in% b/c if we have the same size in different years it won't 
		  # select everything for us and could cause secret grief...
		  # Note that we make the predictions at 80 meters depth and the mean shell height for the year.
		  for(g in 1:num.years)
		  {
		    # The current size of scallop.
		    sizes.com[g] <- round(survey.obj[[bnk]]$model.dat$l.bar[g],digits=1)
		    sizes.rec[g] <- round(survey.obj[[bnk]]$model.dat$l.k[g],digits=1)
		    # The predicted size of scallop given the current size and our growth predictions.  
		    sizes.ny.com[g] <- round(growth.predictions[growth.predictions$size == sizes.com[g],]$size.ny,digits=1)
		    sizes.ny.rec[g] <- round(growth.predictions[growth.predictions$size == sizes.rec[g],]$size.ny,digits=1)
		    # The wmw of the average fully-recruited scallop next year along with the se of that estimate based on our growth model above.
		    mw.pred$com.ny[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                              data.frame(depth.cen = 0, sh.log = log(sizes.ny.com[g])),
		                              type="response",re.form=NA)
		    mw.pred$com.ny.se[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                                 data.frame(depth.cen = 0, sh.log = log(sizes.ny.com[g])),
		                                 type="response",re.form=NA,se=T)$se.fit
		    # How much we expect a recruit this year to weigh next year and it's standard error...
		    mw.pred$rec.ny[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                                 data.frame(depth.cen = 0, sh.log = log(sizes.ny.rec[g])),
		                                 type="response",re.form=NA)
		    mw.pred$rec.ny.se[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                                 data.frame(depth.cen = 0, sh.log = log(sizes.ny.rec[g])),
		                                 type="response",re.form=NA,se=T)$se.fit
		    # The current weight of fully recruited scallop and it's standard error.
		    mw.pred$com[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                              data.frame(depth.cen = 0, sh.log = log(sizes.com[g])),
		                              type="response",re.form=NA)
		    mw.pred$com.se[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                              data.frame(depth.cen = 0, sh.log = log(sizes.com[g])),
		                              type="response",re.form=NA,se=T)$se.fit
		    # And the wmw of the current recruits...
		    mw.pred$rec[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                             data.frame(depth.cen = 0, sh.log = log(sizes.rec[g])),
		                             type="response",re.form=NA)
		    mw.pred$rec.se[g] <- predict(mwsh.mod.fits[[bnk]][[as.character(years[g])]]$gam,
		                              data.frame(depth.cen = 0, sh.log = log(sizes.rec[g])),
		                              type="response",re.form=NA,se=T)$se.fit
		  }  # end for(g in 1:num.years)
		  
		  # Now I can get CI's around all of these estimates...
		  mw.pred$com.ny.LCI <- mw.pred$com.ny - 2 * mw.pred$com.ny.se
		  mw.pred$com.ny.UCI <- mw.pred$com.ny + 2*mw.pred$com.ny.se
		  mw.pred$rec.ny.LCI <- mw.pred$rec.ny - 2 * mw.pred$rec.ny.se
		  mw.pred$rec.ny.UCI <- mw.pred$rec.ny + 2*mw.pred$rec.ny.se
		  mw.pred$com.LCI <- mw.pred$com - 2 * mw.pred$com.se
		  mw.pred$com.UCI <- mw.pred$com + 2*mw.pred$com.se
		  mw.pred$rec.LCI <- mw.pred$rec - 2 * mw.pred$rec.se
		  mw.pred$rec.UCI <- mw.pred$rec + 2*mw.pred$rec.se
		  
		  # Now we can get the growth parameter for each year
		  mw.pred$g.com <- mw.pred$com.ny/mw.pred$com
		  # This is the low growth scenario
		  mw.pred$g.com.LCI <- mw.pred$com.ny.LCI/mw.pred$com
		  # This is an estimate of the SE of this situation, gives us a measure of uncertainty...
		  # I neeed to think much more about this measure of uncertainty, but it's something...
		  ...
		  # And output into the surv.dat object....
		  survey.obj[[bnk]]$model.dat$g <- mw.pred$g.com
		  survey.obj[[bnk]]$model.dat$g.se <- mw.pred$g.com.se 
		  survey.obj[[bnk]]$model.dat$gR <- mw.pred$g.rec
		  survey.obj[[bnk]]$model.dat$gR.se <- mw.pred$g.rec.se
		  ######################################  END GROWTH MODELLING END GROWTH MODELLING END GROWTH MODELLING   ###########################
		  ######################################  END GROWTH MODELLING END GROWTH MODELLING END GROWTH MODELLING   ###########################
		  ######################################  END GROWTH MODELLING END GROWTH MODELLING END GROWTH MODELLING   ###########################
		...
		### DK March 10th 2017, don't see any reason why I couldn't toss this object out entirely...  
		### I will need to revisit the FIGURES that use this so don't delete this yet.
		### I will just use the CF from the surv.dat object instead of this, downside is
		### that does use some inferred relationships for the spatial CF, but our whole GD 
		### model does that anyways!!
...
save(list = ls(all.names = TRUE), file = paste(direct,"Data/Framework/2017/Growth/Survey_all_results.Rdata",sep=""))
...
return(list(survey.obj = survey.obj,
            ...
            mw.pred.dat = mw.pred.dat,
            mwsh.mod.fits = mwsh.mod.fits,
