                        ...
                        refs=c("URP","LRP","zones"),index,URP,LRP,URP2=NULL,LRP2=NULL,out.data2,lab='',alpha=c(0.05,0.2),path='')
{
  ...
  if(is.null(refs) == F)
  {
    text(min(years)+5, LRP*0.1, "CRITICAL", col='firebrick1', cex=1.5)
    text(min(years)+5, LRP*1.2, "CAUTIOUS", col='goldenrod1',cex=1.5)
    text(min(years)+5, 0.9*Bymax, "HEALTHY", col='chartreuse2',cex=1.5)
