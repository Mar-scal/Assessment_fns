  ...
  # Stick a y axis label on here, hopefully at a nice location...
	...
	if(multi==T)
	{
	  ...
	  multiposts<-model.out$sims.list[lapply(model.out$sims.list,ncol)>1]
		...
		if(graphic=='pdf') pdf(paste0(path,"post_annual.pdf"), width = wd, height = ht, pointsize = 12,onefile=T)
		...
		for (i in 1:length(multiposts))
		{
		  ...
		  par(mfrow = c(ceiling(length(years)/ceiling(sqrt(length(years)))), 
			...
			for(y in 1:ncol(multiposts[[i]]))
			{
			  ...
			  if(xl.type==1) xl<-range(multiposts[[i]],na.rm=T) ;  x<-seq(xl[1], xl[2], l = length(multiposts[[i]][,y]))
			  # If we set axis range to 2 then the range for each plot is the maxium for the year.
			  if(xl.type==2) xl<-range(multiposts[[i]][,y],na.rm=T) ;  x <-seq(xl[1], xl[2], l = length(multiposts[[i]][,y]))
	  ...
	  if(graphic=="pdf")dev.off()	 # Turn off the pdf device
	} # end if(multi==T)
}# end function
