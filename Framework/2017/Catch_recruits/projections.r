...
# Aug 2017 - This has been revised to include some catch coming from recruits along with a non-clapper natural mortality term
#####################################  Function Summary ########################################################
####  
...
projections<-function(model.out,C.p=c(200,300))
{
  # Add the projected Catch from the start of survey year to the end of the calendar year
	...
	for(i in 1:length(C.p))
	{
		...
		Pmed.p[[i]]<- log(exp(-d$m[,d$NY]-d$nm[,d$NY])*(d$g[d$NY])*(d$P[,d$NY]-(d$C.prop.FR[,d$NY]*C.p[i])/d$K)+ 
		                                                  (exp(-d$mR[,d$NY]-d$nmR[,d$NY])*(d$gR[d$NY])*(d$r[,d$NY]-((1-d$C.prop.FR[,d$NY])*C.p[i])/d$K)))
		...
		# Caluclate the exploitation rate of the fully recruited scallop for next year using the projected catch. 
		# We assume that all of the catch next year comes from this population, i.e. we don't have information on the
		# recruits coming next year so we don't include those in this calculation.  I'm not sure this is all quite right
		# at the moment and need to stew over this a bit, but should be close...
		...
		Pmed.p2[[i]]<-log(exp(-d$m[,d$NY]-d$nm[,d$NY])*(d$g[d$NY])*P.p[[i]]+exp(-d$mR[,d$NY]-d$nmR[,d$NY])*(d$gR[d$NY])*d$r[,d$NY])
		# Get P
